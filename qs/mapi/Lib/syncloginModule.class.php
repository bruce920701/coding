...
function Wechat()
{
		...
		if($user_data)
		{
			...
			else
			{
				//同步登录
				require_once APP_ROOT_PATH."system/model/user.php";
				auto_do_login_user($user_data['user_name'],$user_data['user_pwd'],$from_cookie = false,true);
		...
		else
		{
			...
			if($result['status']){
				...
				//同步登录
				require_once APP_ROOT_PATH."system/model/user.php";
				auto_do_login_user($s_user_info['user_name'],$s_user_info['user_pwd'],$from_cookie = false,true);
