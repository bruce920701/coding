	...
	public function index()
	{
		...
		if($indexs_list===false)
		{
			...
			foreach($indexs as $k=>$v)
			{
				...
				$v['data']['cate_id'] = $v['id'];
				$v['ctl'] = "tuan";
				$v['icon_name'] = $v['m_iconfont'];//图标名 http://fontawesome.io/icon/bars/
				$v['color'] = $v['m_iconcolor'];//颜色
				$v['bg_color'] = $v['m_iconbgcolor'];//背景颜色
				$v['img'] = $v['app_icon_img'];
				$v['type'] = 11;
		...
		$root['xzt_html'] = load_xzt_unit($page=2);
		//推荐团购
		...
		$root['page_title']=$GLOBALS['lang']['WAP_MAIN_INDEX_TITLE'];
		if($root['city_name']!=''){
			if($root['page_title']!=''){
				$root['page_title'].='·'.$root['city_name'];
			}else{
				$root['page_title'].=$root['city_name'];
			}
		}
