...
class biz_presell_orderApiModule extends MainBaseApiModule
{
    ...
    public function index(){
        ...
        $condition=" from ".DB_PREFIX."deal_order_item as doi left join ".DB_PREFIX."deal as d on doi.deal_id=d.id left join ".DB_PREFIX."deal_order as do on doi.order_id=do.id LEFT JOIN ".DB_PREFIX."deal_location_link AS l ON doi.deal_id = l.deal_id
			WHERE l.location_id IN (".implode(",",$account_info['location_ids']).") AND do.is_presell_order=1 and doi.is_shop=1 and do.supplier_id = ".$supplier_id;
        if($type==0){ //全部订单

            $order_sql = "select distinct(doi.order_id) as order_id , do.order_sn,do.pay_status,d.presell_delivery_time ".$condition." AND do.pay_status > 0  order by doi.order_id desc limit ".$limit;
            $order_count_sql = "select count(distinct(doi.order_id)) ". $condition;
        }elseif($type==1){//未发货订单

            $order_sql = "select distinct(doi.order_id) as order_id , do.order_sn,do.pay_status,d.presell_delivery_time ".$condition." AND do.pay_status =2  and doi.delivery_status=0 and doi.is_pick=0 order by doi.order_id desc limit ".$limit;
            $order_count_sql = "select count(distinct(doi.order_id)) ". $condition." and doi.delivery_status=0 and doi.is_pick=0";

        }elseif($type==2){//已发货订单
            $order_sql = "select distinct(doi.order_id) as order_id , do.order_sn,do.pay_status,d.presell_delivery_time ". $condition." and doi.delivery_status=1 and doi.is_pick=0 order by doi.order_id desc limit ".$limit;
        ...
        foreach($order_arr as $k=>$v){
            ...
            foreach($order_item as $kk=>$vv){
                if($v['order_id']==$vv['order_id']){
                    $vv['presell_delivery_time']=$v['presell_delivery_time'];
                    $vv['pay_status']=$v['pay_status'];
    ...
    public function view(){
        ...
        $order_item_sql = "select doi.*,d.balance_price,do.pay_status,d.presell_delivery_time from ".DB_PREFIX."deal_order_item as doi LEFT JOIN ".DB_PREFIX."deal_location_link AS l ON doi.deal_id = l.deal_id
            left join ".DB_PREFIX."deal as d on d.id = doi.deal_id left join ".DB_PREFIX."deal_order do on do.id=doi.order_id
        ...
        if ($order['invoice_info']) {
            $order['invoice_info'] = unserialize($order['invoice_info']);
        }
        ...
        $fee_detail['symbol'] = 1;
        $fee_detail['value'] = round($total_balance_price-($order['presell_discount_money']-$order['presell_deposit_money']),2);
        $feeinfo[] = $fee_detail;
...
//            echo $dnSql;exit;
    ...
    public function delivery(){
        ...
        $sql = "select do.id,do.delivery_status,do.delivery_id,do.region_lv1,do.region_lv2,do.region_lv3,do.region_lv4,do.consignee,do.address,do.zip,do.mobile from ".
            ...
            " where do.order_status=0 and do.type = 8 and do.pay_status = 2 and do.id=".$order_id;
        ...
        if($order_info && $order_info['delivery_status']!=2){
            ...
            $sql = "select doi.id,doi.deal_id,doi.deal_icon,doi.name,doi.number,doi.unit_price,doi.total_price,doi.is_delivery,doi.attr_str  from ".$order_item_table_name." as doi left join ".
                ...
                " where do.type = 8 and (doi.refund_status=0 or doi.refund_status=3) and doi.is_shop = 1 and do.pay_status = 2 and doi.delivery_status=0 ".$condition;
    ...
    public function get_order_status($order_item,$pick_location_id)
    {
        ...
        $track_url = wap_url("biz",'presell_order#logistics',array('data_id'=>$order_id));
        $delivery_url = wap_url("biz",'presell_order#delivery',array('data_id'=>$order_id));
        if ($status & 1) {
            if($order_item[0]['pay_status']>1){
                if($delivery_count==0){
                    if(NOW_TIME>$order_item[0]['presell_delivery_time']){
                        $op_array=array(
                    ...
                    $op_array[]= array('js_obj'=>'js_track','name'=>'查看物流','url'=>$track_url);
                    if(NOW_TIME>$order_item[0]['presell_delivery_time']){
                        $op_array[]=array('js_obj'=>'js_delivery','name'=>'发货','url'=>$delivery_url);
                    }
                }
            }
        } else {
    ...
    public function get_order_item_status($order_item,$pick_location_id){
        ...
        if($order_item){
            if($order_item['refund_status']==1){
                ...
                $order_status='已退款';
            }elseif($order_item['pay_status']==1){
                $order_status='待付尾款';
