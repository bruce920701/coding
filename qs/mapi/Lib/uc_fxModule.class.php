...
class uc_fxApiModule extends MainBaseApiModule
{
    ...
    public function my_fx(){
		...
		else
		{
			...
			if($user['is_fx']==1){
				...
				//$result=get_fx_withdraw_money($user_id);
				$meney=$user['fx_money'];// - $fx_money-$result;
    			$user_data['fx_money'] = round($meney>0?$meney:0,2);
        		...
        		foreach ($deal_list as $K=>$v){
                    ...
                    if(APP_INDEX=='app'){
						...
						$temp_data['url']='javascript:App.app_detail(414,"'.$json_parma.'");';
					}else{
						$temp_data['url']=$temp_data['share_url'];
					}
		...
		$root['page_title']=$GLOBALS['lang']['WAP_UC_FX_TITLE'];
		$root['lang']['wap_uc_fx_account_balance']=$GLOBALS['lang']['WAP_UC_FX_ACCOUNT_BALANCE'];
		$root['lang']['uc_fx_fx_name']=$GLOBALS['lang']['UC_FX_FX_NAME'];
		return output($root);
	}	
	...
	public function deal_fx(){
        ...
        $tid = intval($GLOBALS['request']['tid']);//商品分类ID
	    $user_login_status = check_login();
	    if($user_login_status!=LOGIN_STATUS_LOGINED){
	        // 未登录
	    } elseif ($user['is_fx'] == 0) {
            ...
            $ext_condition = " d.is_fx = 2 and d.buy_type <> 1 and is_shop=0 ".$condition;

            $orderby =' d.id desc ';
            $condition_param = array("cid"=>$catalog_id,"tid"=>$tid);
    		$deal_result = get_deal_list($limit,array(DEAL_ONLINE,DEAL_NOTICE),$condition_param,'',$ext_condition,$orderby);
	...
	public function income(){
	    ...
	    else
	    {
			...
			require_once(APP_ROOT_PATH."system/model/fx.php");
			$result=get_fx_withdraw_money($user_id);
			$meney=$user_data['fx_money'];// - $fx_money-$result;
			$root['fxmoney']=format_price($meney>0?$meney:0);			
			...
			$root['lang']['wap_uc_fx_account_balance']=$GLOBALS['lang']['WAP_UC_FX_ACCOUNT_BALANCE'];
			$root['lang']['wap_uc_fx_income_accumulated_commission']=$GLOBALS['lang']['WAP_UC_FX_INCOME_ACCUMULATED_COMMISSION'];
	    }
	    return output($root);
	}
	...
	public function vip_buy(){
	    ...
	    else
	    {
	        ...
	        $fx_buy=$GLOBALS['db']->getRow("select id,pay_fee,pay_agreement,phone_description,phone_pay_agreement from ".DB_PREFIX."fx_qualification");
	        if($fx_buy['pay_fee']){
	...
	public function qrcode(){
		...
		else
		{
			...
			if($user['is_fx']==1){
                ...
                if($user_data['qrcode_type']==0){ $json['type']=602; $qrcode_url=wap_url("index","uc_fx#mall",array("r"=>base64_encode($user_id)));
                }elseif($user_data['qrcode_type']==1){ $json['type']=1; $qrcode_url=wap_url("index","index",array("r"=>base64_encode($user_id)));
                }elseif($user_data['qrcode_type']==2){ $json['type']=102; $qrcode_url=wap_url("index","shop",array("r"=>base64_encode($user_id)));
                }elseif($user_data['qrcode_type']==3){ $json['type']=103; $qrcode_url=wap_url("index","main",array("r"=>base64_encode($user_id)));
                }
                ...
                //echo $savename_bg;exit;
                if(!file_exists($savename_bg)){
                    if(ENTRY_NAME=='nzy_o2o'){
                        require_once APP_ROOT_PATH."system/utils/es_imagecls.php";
                        $image = new es_imagecls();
                        @unlink($savename_bg);
                        //生成背景
                        $image->thumb(APP_ROOT_PATH.'public/font_bg/user_qrcode_background.png',$width=1500,$height=2252,$thumb_type=0,true,$savename_bg);
//                  //二维码
                        $qrcode = $dir_invite."/invite_qrcode_".$user_id.".jpg";
                        copy($user_data['share_mall_qrcode'], $qrcode);
                        $thumbname=$dir_invite."/invite_".$user_id."_188x188.jpg";
                        //生成二维码缩略图
                        $thumb=$image->thumb($qrcode,$width=188,$height=188,$thumb_type=0,true,$thumbname);
                        @unlink($qrcode);
                        ...
                        // 图片大小设置,生成头像缩略图
                        $user_logo_circular=$image->thumb($user_logo_circular,$width=160,$height=160,$thumb_type=0,true,$user_logo_circular);

                        // 把二维码加入背景图中
                        $new_pic::water($savename_bg, $thumb['path'], $savename_bg,$alpha=100,$position="12");
                        @unlink($thumb['path']);
                        // 把头像加入背景图中
                        $new_pic::water($savename_bg, $user_logo_circular['path'] , $savename_bg,$alpha=100,$position="13");
    ...
    public function shop_fx(){
        ...
        if($user_login_status!=LOGIN_STATUS_LOGINED){
            ...
            $ext_condition = " d.is_fx = 2 and d.buy_type <> 1  and d.is_shop=1 ".$condition;

            $orderby =' d.id desc ';

            $condition_param = array("cid"=>$catalog_id);
            $deal_result = get_goods_list($limit,array(DEAL_ONLINE,DEAL_NOTICE),$condition_param,'',$ext_condition,$orderby);
