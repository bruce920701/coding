...
class cartApiModule extends MainBaseApiModule
{
	...
	public function addcart($outputReturn=true,$param=array())
	{
        ...
        $buy_num =  intval($_REQUEST['num'])<=0?1:intval($_REQUEST['num']);
        ...
        if($deal_info['current_price']*$buy_num > $user_info['register_credits'])
        {
            $res['status'] = 0;
            $res['info'] = "积分不足";
            ajax_return($res);
...
// 				}
				...
				if (!$cart_item || $cart_data['in_cart'] == 0) {
					...
					if ($deal_attr) {
						...
						$add_price_sql = 'SELECT price, add_balance_price,presell_deposit_money,presell_discount_money FROM ' . DB_PREFIX . 'attr_stock WHERE deal_id=' . $id . ' AND attr_key="' . $attr_key . '"';
...
// 		}
	...
	public function check()
	{
		...
		if ($show_payment) {
			...
			foreach ($cart_list as $k => $v) {
				...
				$v['unit_price_format'] = " <i>" . $bai . "</i>." . $fei;
	...
	public function count_buy_total()
	{
		...
		if( $result['is_presell']){
		    $presell_discount_price = $result['presell_discount_money'] - $result['presell_deposit_money'];
		    if($presell_discount_price > 0){
		        $presell_title = "订金：".format_price($result['presell_deposit_money'],2)."（抵".format_price($result['presell_discount_money'],2)."）";
		    }else{
		        $presell_title = "订金：".format_price($result['presell_deposit_money'],2);
		    }
			...
			$feeinfo[] = array(
					"name" => "商品尾款：".format_price($result['presell_left_price']),
					"class"=>"total_price",
					"symbol" => 3,
					"value" => '未开始',
			);
		...
		if($result['is_presell']==1){
			...
			$result['user_discount']=0;//预售商品不参与任何优惠
...
// 		}
	...
	public function order_done()
	{
		...
		$goods_list = $GLOBALS['db']->getAll("select doi.* , d.allow_user_discount ,d.presell_deposit_money,d.presell_discount_money,d.presell_type,d.presell_delivery_time,d.is_presell,d.presell_begin_time,d.presell_end_time from ".DB_PREFIX."deal_order_item as doi left join ".DB_PREFIX."deal as d on doi.deal_id=d.id where doi.order_id = ".$order['id']);
...
// 			return output("",-2,"没有可以购买的产品");
	...
	public function get_cart_deal_attr(){
	    ...
	    $user_discount_percent = 1;
	    if (!empty($user_info['id']) && $deal_info_all['allow_user_discount']) {
	        $user_discount_percent = getUserDiscount($user_info['id']);
	    }
	    $deal_info_all['current_price'] = $deal_info_all['current_price'] * $user_discount_percent;
		$deal_info['deal_attr_stock_json']=$deal_info_all['deal_attr_stock_json'];
		$deal_info['current_price']=$deal_info_all['current_price'];
		$deal_info['f_current_price']=round($deal_info_all['current_price'],2);
	...
	public function pay()
	{
		...
		//处理购物车输出
		$cart_list = $GLOBALS['db']->getAll("select doi.*,d.id as did,d.icon,d.uname as duname ,  d.allow_user_discount ,do.presell_deposit_money,do.presell_discount_money,do.presell_type,d.presell_delivery_time,d.is_presell,d.presell_begin_time,do.presell_end_time from ".DB_PREFIX."deal_order_item as doi left join ".DB_PREFIX."deal as d on doi.deal_id = d.id left join ".DB_PREFIX."deal_order as do on doi.order_id=do.id where doi.order_id = ".$order_info['id']);
