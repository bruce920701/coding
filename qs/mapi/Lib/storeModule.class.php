...
and y.supplier_id = ".$supplier_id." and ( ( y.youhui_type = 1 and yll.location_id = ".$store_id." ) OR y.youhui_type = 2 ) order by y.create_time desc,y.id desc";
        ...
        if ($store_info['open_store_payment'] == 1) {
            $promotes = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."promote where supplier_id=".$store_info['supplier_id']." and class_name<>'Freebyprice'");
			// 优惠信息
        ...
        $json['type']=430;
        $json['data_id']=$store_info['id'];

        if($GLOBALS['user_info'])
        {
            $json['user_id']=intval($GLOBALS['user_info']['id']);
            $store_info['json_url']  = SITE_DOMAIN.wap_url("index","store",array('data_id'=>$store_info['id'],'r'=>base64_encode(intval($GLOBALS['user_info']['id'])),'json'=>base64_encode(json_encode($json))));
        }else{
            $store_info['json_url']  = SITE_DOMAIN.wap_url("index","store",array('data_id'=>$store_info['id'],'json'=>base64_encode(json_encode($json))));
        }
