...
class uc_fxwithdrawApiModule extends MainBaseApiModule
{
	...
	public function wap_index(){
	    ...
	    if($user_login_status!=LOGIN_STATUS_LOGINED){
	        ...
	        //可提现金额
	        $inWithdrawSql = "select sum(money) as money, sum(fee) as fee from ".DB_PREFIX."fx_withdraw where user_id=".$user['id']." and is_paid=0 and is_delete=0";
	        $inWithdraw = $GLOBALS['db']->getRow($inWithdrawSql);
	        $fx_money = $inWithdraw['money'] + $inWithdraw['fee'];
	        ...
	        //echo $money;exit;
			require_once(APP_ROOT_PATH."system/model/fx.php");
			$result=get_fx_withdraw_money($user['id']);
			$money=$money-$result;
			
	        $root['fxmoney']=$money>0?round($money,2):0;
	    ...
	    $root['page_title'].=$GLOBALS['lang']['WAP_USER_CENTER_UC_FXWITHDRAW_TITLE'];
		$root['lang']['wap_uc_fx_balance']=$GLOBALS['lang']['WAP_UC_FX_BALANCE'];
	    //print_r($root);exit;
	    return output($root);
	}
	...
	public function detail()
	{
		...
		if($user_login_status!=LOGIN_STATUS_LOGINED){	
			...
			{	
				...
				$list[] = $item;
				if ($v['fee'] > 0 && $v['is_paid'] == 1) {
					$feeItem = $item;
					$feeItem['money'] = format_price($v['fee']);
					$feeItem['title'] = '提现手续费';
					$feeItem['type'] = 2;
					$list[] = $feeItem;
				}
