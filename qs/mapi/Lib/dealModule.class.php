	...
	public function index()
	{
		...
		if($data['buy_type']==1){ //积分商品才判断登录状态和会员积分是否足够兑换
		    ...
		    if($data['user_login_status']==1){
		        if( $GLOBALS['user_info']['avalible_consume_credits'] + $return_score >=0){//足够兑换
		...
		$data['collect_count']=$user_id>0?$GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal_collect where user_id=".$user_id):0;
        ...
        if($GLOBALS['user_info'])
        {
            ...
            $data['json_url']= SITE_DOMAIN.wap_url("index","deal",array('data_id'=>$data['id'],'r'=>base64_encode(intval($GLOBALS['user_info']['id'])),'json'=>base64_encode(json_encode($json))));

        }else{
            $data['json_url']= SITE_DOMAIN.wap_url("index","deal",array('data_id'=>$data['id'],'json'=>base64_encode(json_encode($json))));
        }
        ...
        if (APP_INDEX == 'app' && isOpenXN() && $data['buy_type'] != 1) {
	        ...
	        if ($data['supplier_id']) {
	        	$xnInfo = $GLOBALS['db']->getRow('SELECT xn_talk_login_id, xn_talk_custom_id , open_xn_talk FROM '.DB_PREFIX.'supplier WHERE id='.$data['supplier_id']);
