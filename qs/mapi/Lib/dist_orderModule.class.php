...
class dist_orderApiModule extends MainBaseApiModule
{
    ...
    public function do_delivery(){
        ...
        AppPush::singleUserPush("dist_do_delivery",$order_info['user_id'],lang("APP_PUSH_ORDER_DIST_DO_DELIVERY",$order_info['order_sn'],$dist_info['name'],$coupon['sn'] ),$order_id);
        //开始同步订单的发货状态
		...
		if(1){
            ...
            foreach($order_info_arr as $v){
                $distribution_fee += $v['distribution_fee'];
            }
            Distribute::distribution_order_fee($type=1,$distribution_fee,$order_info);
		}
        require_once(APP_ROOT_PATH."system/model/deal_order.php");
        update_order_cache($order_id);
