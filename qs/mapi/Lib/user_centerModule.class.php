...
class user_centerApiModule extends MainBaseApiModule
{
	...
	public function wap_index()
	{
		...
		$is_visiting_service = defined('IS_VISITING_SERVICE') && IS_VISITING_SERVICE;
		
		foreach($mobileuccenter['OrderManager']['menu'] as $k=> &$v){
			if (($v['module'] == 'vs_order' && !$is_visiting_service) || ($v['module'] == 'uc_review' && $is_visiting_service)) {
				unset($mobileuccenter['OrderManager']['menu'][$k]);
			}
        ...
        $mobileuccenter['OrderManager']['menu'] = reset_key_array($mobileuccenter['OrderManager']['menu']);
	...
	private function baseData()
	{
		...
		if($user_login_status == LOGIN_STATUS_LOGINED){			
			...
			// 未使用优惠券
			$youhuiSql = 'SELECT COUNT(*) FROM '.DB_PREFIX.'youhui_log AS yl INNER JOIN '.DB_PREFIX.'youhui as y ON yl.youhui_id = y.id WHERE yl.confirm_time=0 and y.id>0 and y.is_effect=1 and yl.user_id='.$user_id.' and ( yl.expire_time > '.NOW_TIME.' or yl.expire_time = 0)';
			...
			if(IS_OPEN_DISTRIBUTION==1){
			    $dist_count_sql="select count(*) from ".DB_PREFIX."distribution_coupon where user_id=".$user_id." and confirm_time=0 and is_delete=0";
        ...
        $direct_num = $this->get_user_direct_user_num($user_data['id']);
        ...
        if($direct_num < $fx_0_members_0)
        {
            $level_0_fx = 0;
        }
        else if($direct_num < $fx_0_members_1)
        {
            $level_0_fx = 1;
        }
        else if($direct_num < $fx_0_members_2)
        {
            $level_0_fx = 2;
        }
        else
        ...
        $root['level_0_fx'] = $level_0_fx;
		$root['user_login_status'] = $user_login_status;
		$root['coupon_name'] = app_conf("COUPON_NAME");
		$root['lang']['fx_name'] = $GLOBALS['lang']['WAP_USER_CENTER_FX_NAME'];
		$root['lang']['uc_fx'] = $GLOBALS['lang']['WAP_USER_CENTER_UC_FX'];
		$root['lang']['uc_fxwithdraw'] = $GLOBALS['lang']['WAP_USER_CENTER_UC_FXWITHDRAW'];
		return $root;
	}
