...
class goodsApiModule extends MainBaseApiModule
{
	...
	public function index()
	{
		...
		$condition_param = array("cid"=>$catalog_id,"bid"=>$brand_id,'buy_type'=>2);
		...
		$brand_list_bak=$GLOBALS['cache']->get("GOODS_INDEX_BRAND_LIST_".intval($catalog_id));
		if($brand_list_bak === false){
			if($catalog_id){
				$shop_cate=$GLOBALS['db']->getRow("select pid from ".DB_PREFIX."shop_cate where id=".$catalog_id);

				if($shop_cate['pid']!=0){
					$brand=$GLOBALS['db']->getAll("select distinct brand_id from ".DB_PREFIX."deal where brand_id<>0 and find_in_set({$catalog_id},shop_cate_id)");
				}else{
					$shop_cate_array=$GLOBALS['db']->getAll("select id from ".DB_PREFIX."shop_cate where pid=".$catalog_id);

					if(empty($shop_cate_array)){
						$brand=$GLOBALS['db']->getAll("select distinct brand_id from ".DB_PREFIX."deal where brand_id<>0 and find_in_set({$catalog_id},shop_cate_id)");


					}else{

						foreach($shop_cate_array as $k=>$v){
							$arr=explode(',',$v['id']);
							foreach($arr as $vv){
								$shop_cate_id_key[]+=$v['id'];
							}
						}
						array_push($shop_cate_id_key,$catalog_id);
						$brand=$GLOBALS['db']->getAll("select distinct brand_id from ".DB_PREFIX."deal where brand_id<>0 and shop_cate_id in(".implode(",",$shop_cate_id_key).")");
					}
			...
			sort($brand_list);
			$GLOBALS['cache']->set("GOODS_INDEX_BRAND_LIST_".intval($catalog_id),$brand_list,600);
		}else{
			$brand_list=$brand_list_bak;
		}
		...
		$bcate_list_bak=$GLOBALS['cache']->get("GOODS_INDEX_BCATE_LIST_".implode(",",$brand_id));
		if($bcate_list_bak === false){
			$time = NOW_TIME;
			...
			sort($bcate_list);
			$GLOBALS['cache']->set("GOODS_INDEX_BCATE_LIST_".implode(",",$brand_id),$bcate_list,600);
		}else{
			$bcate_list=$bcate_list_bak;
		}
		//二级分类的个数计算
		foreach($bcate_list as $k=>$v ){
			$bcate_list[$k]['active'] = 0;
		...
		/*输出分类名 */
		$catalog_info=$GLOBALS['cache']->get("GOODS_INDEX_CATALOG_INFO_".intval($catalog_id));
		if($catalog_info === false){
			$catalog_info = $GLOBALS['db']->getRow("select name from ".DB_PREFIX."shop_cate where id=".$catalog_id);
			$GLOBALS['cache']->set("GOODS_INDEX_CATALOG_INFO_".intval($catalog_id),$catalog_info,600);
		}
