...
class uc_collectApiModule extends MainBaseApiModule
{
	...
	public function wap_index(){
	       ...
	       //分页
			...
			if($status==0){
    			...
    			foreach($list as $k=>$v)
    			{
    			    ...
    			    $list[$k]['icon'] = get_abs_img_root(get_spec_image($v['icon'],280,280,1));
    			    $list[$k]['origin_price']= round($v['origin_price'],2);
    			    $list[$k]['current_price']=round($v['current_price']*$user_discount,2);
			...
			/* else if ($status==1){
				...
				//exit;
			    $result = get_dc_location_collect($limit,$user_id,$ypoint,$xpoint);
			    $list=$result['list'];
			    foreach($list as $k=>$v)
			    {
			        //获取商圈，订餐分类
	    ...
	    $collect_arr = array();
	    $collect_item = array();
	    $collect_item['name'] = '商品&团购';
	    $collect_item['sc_status'] = 0;
	    $collect_arr[] = $collect_item;
	    $collect_item['name'] = '活动';
	    $collect_item['sc_status'] = 1;
	    $collect_arr[] = $collect_item;
	    if(defined('DC')){
	        $collect_item['name'] = '店铺';
	        $collect_item['sc_status'] = 2;
	        $collect_arr[] = $collect_item;
	    }
	    $root['collect_arr'] = $collect_arr;
	    $root['user_login_status'] = $user_login_status;
	    $root['item'] = $list;
