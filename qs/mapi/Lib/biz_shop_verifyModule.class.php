...
//             $deal_location_id = array_column($deal_location_id, 'location_id');
    ...
    public function qrcode(){
		...
		if($location_list===false){
			...
			$img_arr=array();
			$user_id=$GLOBALS['db']->getOne("select user_id from ".DB_PREFIX."supplier_account where supplier_id=".$account_info['supplier_id']." and is_main=1");
			foreach($location_list as $k=>$v){
				$location_id=$v['id'];
                $json['type']=601;
                $json['data_id']=$location_id;
                $json['user_id']=intval($user_id);
                $store_pay_url=wap_url("index","store_pay",array('id'=>$location_id,'r'=>base64_encode(intval($user_id)),'json'=>base64_encode(json_encode($json))));
