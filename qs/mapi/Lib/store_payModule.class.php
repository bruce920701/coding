...
class store_payApiModule extends MainBaseApiModule {
    ...
    public function pay() {
        ...
        */
        //商户数据
        $location_info = $GLOBALS['db']->getRow("select id,name,supplier_id from " . DB_PREFIX . "supplier_location where id=" . $location_id.' and open_store_payment = 1');
        if(!$location_info){
        	
        	return output($root,-1,'无此商家');
        }
        ...
        //获取促销规则 (重新编辑)
        $sql = 'SELECT p.* from '.DB_PREFIX.'promote p inner join '.DB_PREFIX.'supplier_location sl on p.supplier_id=sl.supplier_id AND sl.id='.$location_id.' WHERE sl.open_store_payment = 1  and p.class_name<>"Freebyprice"';
        $promote = $GLOBALS['db']->getAll($sql);
        ...
        if($promote){
	        foreach($promote as $k=>$v){
                ...
                $promote[$k]['config'] = unserialize($v['config']);
	        }
        }
        $root['promote']=$promote;
		$root['score_purchase_switch']=app_conf("SCORE_PURCHASE_SWITCH");
 ...
 /**
    ...
    public function make_order() {
    	...
    	$other_money = floatval($GLOBALS['request']['other_money']);
    	...
    	if ((check_login() == LOGIN_STATUS_TEMP && $GLOBALS['user_info']['money'] > 0) || check_login() == LOGIN_STATUS_NOLOGIN) {
            ...
            if (empty($user_info['mobile'])) {
                $root['nomobile'] = 1;
                return output($root, 0, '请先绑定手机号码');
            }

    		$root['location_id']=$location_id;
    		if($money==0){
    			return output($root, 0, "请输入消费金额");
    		}
    		 
    		if($other_money > 100000){
    		    return output($root, 0, "不可优惠金额不能大于10万");
    		}
    		
    		if($money > 100000){
    		    return output($root, 0, "消费金额不能大于10万");
    		}
    		...
    		if(empty($location_info)){
    			return output($root, 0, "本店不支持到店买单");
    		}
    		
    		$total_price = $money;
    		
    		// 可以优惠的金额 = 总消费金额 - 不可以优惠金额
    		$can_pro_money = $money - $other_money;
    		...
    		$order_data['create_ym'] = date('Ym',NOW_TIME) ;
    		$order_data['pay_status'] = 0 ;
    		$order_data['total_price'] =  $total_price;
    		$order_data['other_money'] = $other_money;
    		...
    		$count_data = count_store_pay_total($location_id, $can_pro_money);
			...
			//计算积分抵现
			if($all_score==1&&app_conf("SCORE_PURCHASE_SWITCH")==1){

				require_once(APP_ROOT_PATH."system/model/cart.php");
				$score_purchase_counts=score_purchase_count($user_info['score'],$can_pro_money,($money-$count_data['discount_price']));
				if($score_purchase_counts['exchange_money']>0){
					$order_data['exchange_money'] = $score_purchase_counts['exchange_money'];
					$order_data['score_purchase'] = serialize($score_purchase_counts);
				}
			}
			...
			//积分抵扣
			if($all_score==1&&app_conf("SCORE_PURCHASE_SWITCH")==1){
				if($score_purchase_counts['score_purchase_switch']==1&&$score_purchase_counts['exchange_money']>0){
					store_pay_score_purchase_paid($score_purchase_counts,$order_id);
				}
    ...
    public function check() {
        ...
        $order_info['discount_price'] += $order_info['exchange_money'];
        $root['pay_price'] = round($order_info['total_price'] - $order_info['discount_price'], 2);
        $order_info['discount_price'] = round($order_info['discount_price'], 2);

        $order_info['other_money'] = round($order_info['other_money'], 2);
        $order_info['pay_amount'] = round($order_info['pay_amount'], 2);
        $order_info['payment_fee'] = round($order_info['payment_fee'], 2);
        ...
        foreach ($payment_list as $k => $v) {
            ...
            $payment_list[$k]['fee_amount'] = round($payment_list[$k]['fee_amount'], 2);
        }
    	...
    	if(check_login() == LOGIN_STATUS_TEMP || check_login() == LOGIN_STATUS_NOLOGIN) {
    		...
    		if ($rs) {
    			...
    			{   //付款单号重复支付,当前支付的退到会员帐户
    				...
    				return output($root);
    			}// elseif (APP_INDEX == 'app') { // app支付
                    $data_pay = call_api_core("store_payment","third_pay_interface",array("order_id"=>$order_id));
                    $root['data_pay'] = $data_pay;
                    $root['sdk_code'] = $data_pay['payment_code']['sdk_code'];
                    $root['online_pay'] = $data_pay['online_pay'];
    ...
    public function promote() {
    	...
    	$money = floatval($GLOBALS['request']['money']);
    	$other_money = floatval($GLOBALS['request']['other_money']);
    	$can_dis_money = $money - $other_money;
    	...
    	if($promote_row){
    		foreach($promote_row as $k=>$v){
    			...
    			if (file_exists($file)) {
    				...
    				$tmp_arr = array();
    				$tmp_arr = $promote_object->count_store_pay($can_dis_money,$v['config']);
    ...
    public function score_purchase_count(){
		...
		$final_pay = floatval($GLOBALS['request']['final_pay']);
		$user_info = $GLOBALS['user_info'];
        require_once(APP_ROOT_PATH."system/model/cart.php");
		$root=score_purchase_count($user_info['score'],$pay_money,$final_pay);
