...
class dc_rsorderApiModule extends MainBaseApiModule
{
	...
	public function index()
	{
		...
		if(check_save_login()!=LOGIN_STATUS_LOGINED)
		{
			...
			foreach($list as $k=>$v)
			{
				...
				$list[$k]['preview']=get_abs_img_root(get_spec_image($v['preview'],360,270,1));
	 ...
	 * dc_coupon:电子券，只有商家接单了，会员才能看电子券信息，其中：sn为预订电子券的序列号，end_time为电子券过期时间，is_expired为电子券是否过期，0为未过期，1为已过期，is_used为是否已经验证消费过，confirm_time为验证消费的时间
	 * coupon_state:电子券状态和动作,其中电子券动作中有个字段has_reason，是用来判断该动作是否需要跳到填写原因的，退款申请中用到该字段
	...
	public function view()
	{
		...
		if(check_save_login()!=LOGIN_STATUS_LOGINED)
		{
			...
			$supplier_location_info=$GLOBALS['db']->getRow("select tel,preview,address from ".DB_PREFIX."supplier_location where id=".$order_info['location_id']);
			$order_info['address'] = $supplier_location_info['address'];
			$order_info['location_tel'] = $supplier_location_info['tel'];
			$order_info['preview'] = get_abs_img_root(get_spec_image($supplier_location_info['preview'],360,270,1));
			...
			if($order_info['id']>0)
			{
				...
				if($order_info['payment_id']==1){
				    $order_info['payment_id']='货到付款';
				}else{
				    $order_info['payment_id']='在线支付';
				}
				$order_info['order_state']=get_order_state($order_info,'wap','view');
	 ...
	 * 输出：
	 * user_login_status:用户登录状态，等于1为已登录，等于0为未登录
	 * status：为预订电子券短信发送操作的状态，status=0,电子券短信发送失败，status=1,电子券短信发送成功
	 * info:返回的提示信息
	 *
	 */
			...
			{	require_once(APP_ROOT_PATH."system/model/dc.php");
				...
				if($order_info['order_status']==1 || $order_info['type_del']==1 || $order_info['is_cancel']!=0){
					...
					{	//订单已付款成功，商家未接单，直接取消订单
						...
						$tip = "商户已经接单，如需取消订单请联系商家，点击确定拨打电话";
						$root['location_tel']=$order_info['location_tel'];
						return output($root,0,$tip);
	 ...
	 * @return $order_state：array:array,返回电子券状态和可操作动作
	 */
	public function get_dc_coupon_state($order_info){
		$order_state=array();
		if($order_info['refund_status']==0){
			if($order_info['dc_coupon']['end_time']==0 || $order_info['dc_coupon']['end_time'] > NOW_TIME || $order_info['dc_coupon']['is_used']==1){
				if($order_info['is_cancel']==0 && $order_info['order_status']==0){
					...
					$order_state['state']='已消费 ';
				}elseif($order_info['is_cancel']==1 || $order_info['is_cancel']==2 ||$order_info['is_cancel']==3){
					$order_state['state']='无效';
				}
