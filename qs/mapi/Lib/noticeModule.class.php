...
class noticeApiModule extends MainBaseApiModule
{
	...
	public function index()
	{
		...
		$result = $GLOBALS['db']->getAll("select id,name,create_time from ".DB_PREFIX."m_notice where is_effect=1 order by sort limit ".$limit);
		foreach ($result as $k => $v){
		    $result[$k]['create_time']=to_date($result[$k]['create_time'],"Y-m-d");
		}
		//$result[0]['create_time']=to_date($result[0]['create_time'],"Y-m-d");
        //print_r($result);exit;
		...
		$page_title=app_conf('MNOTICE_TITLE');
		$root['page_title'].=$page_title?$page_title:'知奥学院';
	...
	public function detail()
	{
		...
		if($result){
			...
			//$result['content'] = get_abs_img_root(format_html_content_image($result['content'],150));
		}		
