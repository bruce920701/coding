        ...
        else
        {
            ...
            if($GLOBALS['request']['bank_name']){   //银行卡表单提交并且提现
             ...
             /*   if(md5($check_pwd)!==$user['user_pwd']){
                ...
                $mobile_data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."sms_mobile_verify where mobile_phone = '".$user['mobile']."'");
            ...
            $data['money']="-".$money;
            $log_msg="提现金额：".format_price($money)."元，等待后台打款";
            modify_account($data,$user['id'],$log_msg);
