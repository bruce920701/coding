...
// 	    }
	    ...
	    if($result['status'])
	    {
	        ...
	        if (APP_INDEX == 'app'){
                ...
                $GLOBALS['db']->autoExecute(DB_PREFIX."distribution", $data, 'UPDATE','id = '.intval($root['dist_info']['id']));
            }
//	        $root['m_dist_nav_list'] = assign_dist_nav_list();
    ...
    public function dev_token_save(){
		if (APP_INDEX == 'app'){
			...
			$data['dev_type'] = strim($GLOBALS['request']['dev_type']);
			$data['device_token'] = strim($GLOBALS['request']['device_token']);
			$data['dist_id']=$GLOBALS['dist_info']['id'];
			if($data['device_token']!=''){
				$GLOBALS['db']->autoExecute(DB_PREFIX."distribution", $data, 'UPDATE','id = '.intval($GLOBALS['dist_info']['id']));
				return output($data,1,'保存成功');
			}
		}
		return output($data,0,"保存失败");
	}
    /**
     * 注销接口
     * 传入
