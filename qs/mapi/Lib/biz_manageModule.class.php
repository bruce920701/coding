...
class biz_manageApiModule extends MainBaseApiModule
{
	...
	public function index(){
        ...
        $presell_order_count_sql="select count(distinct(doi.order_id)) from ".DB_PREFIX."deal_order_item as doi left join ".DB_PREFIX."deal_order as do on doi.order_id=do.id LEFT JOIN ".DB_PREFIX."deal_location_link AS l ON doi.deal_id = l.deal_id
			WHERE l.location_id IN (".implode(",",$account_info['location_ids']).") AND do.is_delete = 0 AND do.type = 8 AND do.pay_status = 2  and doi.delivery_status=0 and doi.is_pick=0 and ( doi.refund_status=0 or doi.refund_status=3 ) and doi.is_shop=1";
        $root['presell_order_count']=intval($GLOBALS['db']->getOne($presell_order_count_sql));
        /* 退款维权商品数量统计 */
        ...
        foreach($locations as $k=>$v){
            $location_dcorder_count=intval($GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."dc_order where supplier_id=".$supplier_id." and is_delivery_cancel=0 and confirm_status=0 and is_cancel=0 and refund_status=0 and order_status=0 and type_del=0 and is_rs=0 and ((payment_id=0 and pay_status=1) or payment_id=1) and location_id =".$v['id']));
