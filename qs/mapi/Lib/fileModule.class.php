...
class fileApiModule extends MainBaseApiModule
{
    ...
    public function upload_img(){
        $data_result['user_login_status'] = check_login();

        if(!$data_result['user_login_status'])
        {
            $data_result['error'] = 1000;  //未登录
            $data_result['msg'] = '';
            return output($data_result,0);
        }
        ...
        $data_result['absolute_url']=get_abs_img_root(get_spec_image($data_result['url'],200,200,1));
        $data_result['path'] = $img_result['file']['path'];
        return output($data_result,1);
    }
}