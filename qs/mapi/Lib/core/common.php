...
//获取服务分类
function getServiceCateList(){
    $cate_list_rs = load_auto_cache("cache_cate_tree",array("type"=>6));
...
function getYouhuiCateList(){
	$youhui_ids = $GLOBALS['db']->getCol("select DISTINCT deal_cate_id from ".DB_PREFIX."youhui where (valid_type = 1 OR ( valid_type = 2 AND ( use_end_time = 0 OR use_end_time > ".NOW_TIME."))) and is_effect=1 and (begin_time < ".NOW_TIME." or begin_time = 0) 
...
{	
	...
	$deal['brief'] = $v['brief'];
	$deal['buy_count'] = $v['buy_count'];//总销量(已包含预售销量)
	$deal['current_price']=round($v['current_price'],2);
	...
	$deal['presell_begin_time_format']=to_date($v['presell_begin_time']);
	$deal['presell_end_time_format']=to_date($v['presell_end_time']);
	$deal['presell_begin_time'] = $v['presell_begin_time'];
	$deal['presell_end_time'] = $v['presell_end_time'];
	$deal['presell_buy_count'] = $v['presell_buy_count'];
...
{	
	...
	if(ENTRY_NAME=='nzy_o2o'){
		$store['distance'] = 0;//$v['distance'] >0 ? floatval($v['distance']) : '';
	}else{
...
//     echo 90572744%86400>0?intval(90572744/86400)."天以上":90572744/86400;exit;
    ...
    if(check_deal_has_discount($deal)){
        $group_id = $GLOBALS['user_info']['group_id'];
        $group_info=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_group where id = ".$group_id);
 ...
 * @param unknown $deal
 */
function check_deal_has_discount($deal){
    $group_id = $GLOBALS['user_info']['group_id'];
    $has_discount = false;
    if($group_id && $deal['allow_user_discount'] && ( $deal['is_presell']==0 || ( $deal['is_presell']==1 && ($deal['presell_begin_time'] > NOW_TIME || $deal['presell_end_time'] < NOW_TIME)))){
        $has_discount = true;        
    }
    return $has_discount;
}
...
function format_short_deal_item($deal)
{
    ...
    if($group_id && $deal['allow_user_discount'] && !IS_PRESELL){
        $group_info=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_group where id = ".$group_id);
...
function load_zt_unit($zt_moban,$page=1)
{
    ...
    if(APP_INDEX=="app")
    {
        $sql = " select * from ".DB_PREFIX."m_zt where mobile_type = '0' and city_id in (0,".intval($city_id).") and status = 1  and zt_moban='".$zt_moban."' and instr(page,{$page})>0 and z_type=0 order by sort asc limit 1";
...
//         }
    ...
    else
    {
        ...
        $sql = " select * from ".DB_PREFIX."m_zt where mobile_type = '1' and city_id in (0,".intval($city_id).") and status = 1 and zt_moban='".$zt_moban."' and instr(page,{$page})>0 and z_type=0 order by sort asc limit 1";
        $zt_unit = $GLOBALS['db']->getRow($sql);
    }
...
function load_xzt_unit($page=1)
{
    ...
    if(APP_INDEX=="app")
    {
        $sql = " select * from ".DB_PREFIX."m_zt where mobile_type = '0' and city_id in (0,".intval($city_id).") and status = 1  and instr(page,{$page})>0 and z_type=1 order by sort asc";
        $zt_info = $GLOBALS['db']->getAll($sql);
    }
    else
    {

        $sql = " select * from ".DB_PREFIX."m_zt where mobile_type = '1' and city_id in (0,".intval($city_id).") and status = 1 and instr(page,{$page})>0 and z_type=1 order by sort asc";
        $zt_info = $GLOBALS['db']->getAll($sql);
    }
    $html = $GLOBALS['cache']->get("MOBILE_INDEX_XZT_".intval($city_id)."_".APP_INDEX);
    ...
    if($html===false)
    {
        ...
        if($zt_info){
            ...
            $zt_css_file = APP_ROOT_PATH.'/admin/Tpl/default/X_MZt/set_zt_html_css.html';
            $zt_css = file_get_contents($zt_css_file);
            $html.= $zt_css;
            
            foreach($zt_info as $k=>$zt_unit){
                ...
                $html_unit =  $zt_unit['zt_html'];

                //开始输出每个广告位的变量
                foreach($zt_layout_list as $kk=>$vv)
                {
                    $vv['data'] = unserialize($vv['data']);
                    $a_link = 'href="'.getHtmlUrl($vv).'"';
                    $search = 'key="'.$vv['zt_position'].'"';
                    $html_unit = str_replace($search, $a_link, $html_unit);                   
                }
    
                $html.=$html_unit;
            }
            $html = '<body style="margin:0px;">'. get_abs_img_root($html).'</body>';
        }
        
        $GLOBALS['cache']->set("MOBILE_INDEX_XZT_".intval($city_id)."_".APP_INDEX,$html);
    }
...
function mindex_cate_menu($city_id,$page=1){
    ...
    if($indexs_list===false)
    {
        ...
        {
            $v['icon_name'] = $v['vice_name'];//图标名 http://fontawesome.io/icon/bars/
			$v['color'] = $v['desc'];//颜色
			$v['bg_color'] = $v['iconbgcolor'];//背景颜色
            $v['data'] = unserialize($v['data']);
			$indexs_list[$k]=format_indexs_i($v);
        }
...
function format_indexs_i($v)
{
	$indexs_i['id'] = $v['id'];
	$indexs_i['name'] = $v['name'];
	$indexs_i['icon_name'] = $v['icon_name'];//图标名 http://fontawesome.io/icon/bars/
	$indexs_i['color'] = $v['color'];//颜色
	$indexs_i['bg_color'] = $v['bg_color'];//背景颜色
	$indexs_i['data'] = $v['data'];
	$indexs_i['ctl'] = $v['ctl'];
	$indexs_i['img'] = get_abs_img_root(get_spec_image($v['img'], 100, 100,1));
	$indexs_i['type'] = $v['type'];
	$indexs_i['url'] = SITE_DOMAIN.getWebAdsUrl($v);
	$is_login_arr=array(41,305);
	if(in_array($v['type'],$is_login_arr)){
		$indexs_i['is_login']=1;
	}else{
		$indexs_i['is_login']=0;
	}
...
if(!function_exists('wx_info_bind')){
    function wx_info_bind($wx_info){
         if(!$wx_info['openid'])
		{
		...
		if($wx_user_info){
			require_once(APP_ROOT_PATH."system/model/user.php");
			$result=bind_wx_user_info_check($GLOBALS['user_info'],$wx_user_info,$wx_info);
			if($result['status']==0){
				return array("status"=>0,"info"=>$result['info']);
			}
			if($wx_user_info['mobile']==''){
				bind_wx_user_info($GLOBALS['user_info'],$wx_user_info,$wx_info,1);
			}else{
