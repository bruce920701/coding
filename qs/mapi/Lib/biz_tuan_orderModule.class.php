...
class biz_tuan_orderApiModule extends MainBaseApiModule
{
    ...
    public function get_order_status($order_item){
        ...
        $deal_coupon_num = count($deal_coupon);  //订单中团购券的数量
        foreach($order_item as $k=>$v){
        ...
        if($dp_count==count($order_item)){//团购券全部使用,并且全部点评完，订单状态为已完成
            $order_status='已完成';
            $order_status_num=2;
        }
        $op_array=array();
        ...
        if($confirm_count + $refund_count == $deal_coupon_num){//团购券 使用数量 + 已退款数量 = 团购总数，订单状态才为 待评价
            $order_status='待评价';
            $order_status_num=1;
        }
    ...
    public function get_order_item_status($order_item){
        ...
        $deal_coupon = $GLOBALS['db']->getAll($deal_coupon_sql);
        $deal_coupon_num = count($deal_coupon);  //商品团购券的数量
        ...
        if($confirm_count + $refund_count ==$deal_coupon_num){//团购券 使用数量 + 已退款数量 = 团购总数，商品状态才为 待评价
            $order_status='待评价';
            $order_status_num=1;
        }
