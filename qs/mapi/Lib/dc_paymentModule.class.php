...
class dc_paymentApiModule extends MainBaseApiModule
{
	...
	public function done()
	{
		...
		if($pay_status==1){  //正常支付，支付完成
			...
			$root['page_title']= '支付结果'; // $GLOBALS['lang']['PAY_SUCCESS'];
			return output($root);
	...
	public function get_payment_code()
	{
		...
		if($pay_status==0){ //订单未完成
			$payment_notice_id = intval($GLOBALS['request']['payment_notice_id']);
			$payment_notice_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."payment_notice where id = ".$payment_notice_id);
			$payment_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."payment where id = ".$payment_notice_info['payment_id']);

			if(APP_INDEX == 'wap') {
				if (!in_array($payment_info['online_pay'], array(2,4,5))) {
					return output(array(),0,"该支付方式不支持wap支付");
				}
			} else {
				if (!in_array($payment_info['online_pay'], array(3,4,5))) {
					return output(array(),0,"该支付方式不支持手机支付");
				}
			}
	...
	public function wap_done()
	{
		$order_id = intval($GLOBALS['request']['order_id']);
		$order_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."dc_order where id = ".$order_id);
		if (empty($order_info) || empty($order_info['pay_status'])) {
		...
		if($pay_status==1){  //正常支付，支付完成
			...
			$root['order_sn']=$order_info['order_sn'];
			if ($root['is_rs'] == 0) {
				$root['mobile'] = $order_info['mobile'];
				$root['consignee'] = $order_info['consignee'];
				$root['address'] = $order_info['api_address'].$order_info['address'];
			}
