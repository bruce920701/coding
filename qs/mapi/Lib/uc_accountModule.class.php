...
class uc_accountApiModule extends MainBaseApiModule
{
	...
	public function wap_index()
	{
		...
		if ($user_login_status == LOGIN_STATUS_LOGINED) {
			...
			$return_user_data = array(
				...
				'm_openid' => $user_data['m_openid']
			);
			if($return_user_data['union_id']==''&&$return_user_data['wx_openid']==''&&$return_user_data['m_openid']==''){
				$return_user_data['is_bind_wx']=0;
			}else{
				$return_user_data['is_bind_wx']=1;
			}
			...
			if (empty($add_time)) {
				...
				if ($request['step'] == 2) {
					...
					if($user_info){
						...
						}elseif($GLOBALS['user_info']['is_merchant']==1){
							return output($data,0,"微信会员已绑定商家，无法合并");
						}
						if($GLOBALS['user_info']){
							if($GLOBALS['user_info']['mobile']==''){
								require_once(APP_ROOT_PATH."system/model/user.php");
								$wx_info=array();
								$wx_info['wx_openid']=$GLOBALS['user_info']['wx_openid'];
								$wx_info['m_openid']=$GLOBALS['user_info']['m_openid'];
								$wx_info['union_id']=$GLOBALS['user_info']['union_id'];
								bind_wx_user_info($user_info,$GLOBALS['user_info'],$wx_info,2);
	...
	function weixin_is_user() {
		...
		if(!$openid)
		{
			return output($root,0,"微信的唯一ID未传递");
		}
		if($unionid)
		{
	...
	function get_wx_app_userinfo()
    {
		...
		if($is_login)
		{
			if($is_login['status']==1){
				...
				return output($root,0,$is_login['info']);
			}
		}
		else
		{
			return output($root,0,"绑定失败");
		}
