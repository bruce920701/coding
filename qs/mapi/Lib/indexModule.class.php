...
class indexApiModule extends MainBaseApiModule
{
	...
	public function wap()
	{
		...
		$adv_list = $GLOBALS['cache']->get("WAP_INDEX_ADVS_".intval($city_id).'_'.APP_INDEX);
		...
		{		
			...
			$GLOBALS['cache']->set("WAP_INDEX_ADVS_".intval($city_id).'_'.APP_INDEX,$adv_list,300);
		}
		$root['advs'] = $adv_list?$adv_list:array();

        // 广告2
		$adv_list2 = $GLOBALS['cache']->get("WAP_INDEX_ADVS2_".intval($city_id).'_'.APP_INDEX);
		if($adv_list2===false)
		{
			if(APP_INDEX=='app')
			{
		    ...
		    $GLOBALS['cache']->set("WAP_INDEX_ADVS2_".intval($city_id).'_'.APP_INDEX, $adv_list2, 300);
		}
		...
		//首页菜单列表		
		$indexs_list = mindex_cate_menu($city_id,$page=1);
		...
		$indexs_deal = $GLOBALS['cache']->get("WAP_INDEX_LIKE_".intval($city_id));
		if($indexs_deal === false)
		{
			...
			$GLOBALS['cache']->set("WAP_INDEX_LIKE_".intval($city_id),$indexs_deal,300);
		}
		...
		if(!$GLOBALS['m_config']['close_index_tuan'])
		{
			...
			if($indexs_deal === false)
			{
				...
				require_once(APP_ROOT_PATH."system/model/deal.php");
				$result = get_deal_list(10,$type=array(DEAL_ONLINE,DEAL_NOTICE),$param=array("cid"=>0,"tid"=>0,"aid"=>0,"qid"=>0,"city_id"=>$city_id),""," d.is_recommend = 1 and d.buy_type <> 1 and d.is_shop = 0 AND d.is_location=1 ");
		...
		//推荐位
		$root['zt_html3'] = load_zt_unit('index_zt3.html',$page=1);
		$root['zt_html4'] = load_zt_unit('index_zt4.html',$page=1);
		$root['zt_html5'] = load_zt_unit('index_zt5.html',$page=1);
		$root['zt_html6'] = load_zt_unit('index_zt6.html',$page=1);
		
		$root['xzt_html'] = load_xzt_unit($page=1);
		

		$is_weixin = isWeixin();
		$root['is_weixin']=$is_weixin;
		$root['page']=1;
		$root['has_next']=1;
	...
	public function more()
	{
		...
		$root['page_title'] = "更多";//$GLOBALS['m_config']['program_title'];
		return output($root);
	}
	...
	public function load_index_list_data()
	{
        ...
        $result = get_deal_list($limit,$type=array(DEAL_ONLINE,DEAL_NOTICE),$param=array("cid"=>0,"tid"=>0,"aid"=>0,"qid"=>0,"city_id"=>$city_id),""," d.is_recommend = 1 and d.buy_type <> 1 and ( d.is_shop = 1 or ( d.is_shop = 0 AND d.is_location=1 and ((is_coupon = 1 AND (coupon_end_time >= ".NOW_TIME." or coupon_end_time=0)) or is_coupon=0) ))");
        $indexs_deal_rs = $result['list'];
