	...
	public function index()
    {
        ...
        if(!$account_info){
			...
			$sql = "select distinct(doi.id),dc.message_id as msg_id,doi.*,do.memo,do.create_time,do.order_sn,do.total_price,do.pay_amount,doi.refund_status from ".$order_item_table_name." as doi 
	    		...
	    		" where l.location_id in (".implode(",",$account_info['location_ids']).") and do.is_delete = 0 and (do.type = 5 or do.type = 6 or do.type=8) and do.pay_status > 0 and (( doi.refund_status > 0 and doi.is_coupon=0) or (dc.refund_status <> 0 and doi.is_coupon=1)) order by ".
			...
			$sql_count = "select count(*) from (select DISTINCT (doi.id),dc.message_id AS msg_id from ".$order_item_table_name." as doi 
				...
				LEFT JOIN ".DB_PREFIX."deal_coupon AS dc ON doi.id = dc.order_deal_id 
				left join ".DB_PREFIX."deal_location_link as l on doi.deal_id = l.deal_id ".
	    		" where l.location_id in (".implode(",",$account_info['location_ids']).") and do.is_delete = 0 and (do.type = 5 or do.type = 6 or do.type=8) and do.pay_status > 0 and (( doi.refund_status > 0 and doi.is_coupon=0) or (dc.refund_status <> 0 and doi.is_coupon=1))) aa";
	...
	*/
	public function view()
    {
        ...
        if(!$account_info){
			...
			$order_item_table_name = get_supplier_order_item_table_name($supplier_id);
			$order_table_name = get_supplier_order_table_name($supplier_id);
			$sql = "select doi.*,do.memo,do.create_time,do.presell_deposit_money,do.presell_discount_money,do.type,do.order_sn,do.pay_status,do.total_price,do.pay_amount,doi.refund_status from ".$order_item_table_name." as doi left join ".
			...
			if($item){
				...
				if($item['type']==8){
                    $item['s_total_price']=$item['s_total_price']-($item['presell_discount_money']-$item['presell_deposit_money']);
                }
			}
			$root['item']=$item;
