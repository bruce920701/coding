...
class dcorderApiModule extends MainBaseApiModule
{
	...
	public function old_make_order()
	{
		...
		if(check_save_login()!=LOGIN_STATUS_LOGINED)
		{
			...
			*/
			...
			if($dc_type==-1){
				$order['rs_price'] = 0;   //定金
				$order['is_rs'] = 0;   //是否为预订定单，0:否 1:是，预订定单不用选配送，发团购券
					
			}else{
				$order['rs_price'] = $location_dc_table_cart['total_data']['total_price'];   //定金
				$order['is_rs'] = 1;   //是否为预订定单，0:否 1:是，预订定单不用选配送，发团购券
			...
			if($order_idx){
				...
				// 订单生成成功后，生成代金券支付付款单号，同时更新代金券使用状态
	...
	private function dcHandle($request)
	{
		...
		do {
			...
			$ecvid = 0;
			$ecvsn = '';
			if (isset($request['ecvsn'])) { // 获取代金券ID
