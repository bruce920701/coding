...
class withdrawalModule extends BizBaseModule
{
	...
	{		
		...
		require_once(APP_ROOT_PATH."system/model/supplier.php");
		$result=get_supplier_withdraw_money($s_account_info['supplier_withdraw_cycle'],$s_account_info['supplier_id']);
		...
		if($s_account_info['supplier_withdraw_cycle']>=0){
			$supplier_withdraw_cycle=$s_account_info['supplier_withdraw_cycle'];
		}else{
			$supplier_withdraw_cycle=app_conf("SUPPLIER_WITHDRAW_CYCLE");
		}
	    //分页
	    $page_size = 10;
		...
		$money=$supplier_info['money']-$result-$submitted_money;
		$GLOBALS['tmpl']->assign("withdraw_money",$money>0?$money:0);
		$GLOBALS['tmpl']->assign("supplier_withdraw_cycle",intval($supplier_withdraw_cycle));
	...
	public function withdraw_done()
	{
		...
		require_once(APP_ROOT_PATH."system/model/supplier.php");
		$result=get_supplier_withdraw_money($s_account_info['supplier_withdraw_cycle'],$s_account_info['supplier_id']);
		if($submitted_money+$money>$supplier_info['money']-$result)
		{
			$data['status'] = 0;
			$data['info'] = "提现超额";
			ajax_return($data);
		...
		$withdraw_data['bank_name'] = strim($_REQUEST['bank_name']);//开户行
		$withdraw_data['bank_info'] = strim($_REQUEST['bank_info']);//银行卡帐号
		$withdraw_data['bank_user'] = strim($_REQUEST['bank_user']);//持卡人姓名
