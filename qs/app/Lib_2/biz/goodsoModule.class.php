...
// 	    }
	...
	{			
		...
		$condition = " l.location_id IN (".implode(",",$s_account_info['location_ids']).") and do.type = 6 and do.is_presell_order=0 and doi.is_shop = 1 and do.pay_status = 2";
		if($name!="") {
			$condition .=" and (doi.name like '%".$name."%' or doi.sub_name like '%".$name."%') ";
		}
		if($begin_time_s) {
	    ...
	    $sql = 'SELECT do.id as doid,do.order_status,do.delivery_status as dstatus,do.create_time,do.order_sn,do.total_price as dtotal,do.user_name,do.invoice_info,doi.*,d.id AS did,d.uname AS duname FROM '.$order_table_name.' AS do LEFT JOIN '.$order_item_table_name.' AS doi ON do.id = doi.order_id LEFT JOIN '.DB_PREFIX.'deal AS d ON doi.deal_id = d.id LEFT JOIN '.DB_PREFIX.'deal_location_link AS l ON doi.deal_id = l.deal_id WHERE do.supplier_id = '.$supplier_id.' AND '.$condition.' GROUP BY doi.id ORDER BY do.id DESC LIMIT '.$limit;
	    $sql_count = 'SELECT count(distinct(doi.id)) FROM '.$order_table_name.' AS do LEFT JOIN '.$order_item_table_name.' AS doi ON do.id = doi.order_id LEFT JOIN '.DB_PREFIX.'deal_location_link AS l ON doi.deal_id = l.deal_id WHERE do.supplier_id = '.$supplier_id.' AND '.$condition;
	    $list = $GLOBALS['db']->getAll($sql);
