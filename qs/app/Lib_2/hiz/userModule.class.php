...
// 	    }
    ...
    public function user_list(){
        ...
        $list=$GLOBALS['db']->getAll("select u.id,u.user_name,u.mobile,u.point,u.score,u.total_score,u.money,p.user_name p_user_name,u.login_time,u.is_effect from ".DB_PREFIX."user u LEFT JOIN ".DB_PREFIX."user p on u.pid=p.id where u.is_delete=0 and u.agency_id=".$hiz_info['id'].$where."order by u.id desc".$limit);
        $count=$GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user u where u.is_delete=0 and u.agency_id=".$hiz_info['id'].$where);
    ...
    public function user_log(){
        ...
        $user_info=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id=".$user_id." and agency_id=".$hiz_info['id']." and is_delete=0");
        if(!$user_info){
            showErr("该会员不存在",0,url("hiz","user#user_list"));
        }
