...
class dcajaxModule extends MainBaseModule
{
	...
	public function load_dc_cart_list()
	{
		global_run();
		require_once(APP_ROOT_PATH."system/model/dc.php");
		$location_id=intval($_REQUEST['location_id']);
		...
		$menu_cart_num = array();
		foreach ($dc_cart_result['cart_list'] as $cart) {
			$menu_cart_num[$cart['menu_id']] = $cart['num'];
		}

		$res['menu_num'] = $menu_cart_num;
		$res['status'] = 1;
		ajax_return($res);
	...
	*/
	 ...
	 */
	static function set_dc_cart_menu_status($is_ajax_return=1,$lid_info=array()){
		global_run();
		$user_id=isset($GLOBALS['user_info']['id'])?$GLOBALS['user_info']['id']:0;
