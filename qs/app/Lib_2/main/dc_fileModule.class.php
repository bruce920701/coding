...
class dc_fileModule extends MainBaseModule
{
	...
	function dc_wxpay_qr_code()
	{
		$payment_notice_id = intval($_REQUEST['notice_id']);
		$url = SITE_DOMAIN.APP_ROOT."/cgi/dc_payment/wwxjspay/redirect.php?notice_id=".$payment_notice_id."&from=".$GLOBALS['request']['from'];
		gen_qrcode($url,app_conf("QRCODE_SIZE"),true);
	}
	 ...
	 * 微信端为跨号支付发起的二维码
	 */
	function dc_wxpay_qr_code_transapp()
	{
		...
		require_once(APP_ROOT_PATH."system/dc_payment/Wxjspay/WxPay.NativePay.php");
		require_once(APP_ROOT_PATH."system/model/dc.php");
		$payment_notice = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."payment_notice where id = ".$payment_notice_id);
		$payment_notice_id = make_dcpayment_notice($payment_notice['money'],$payment_notice['order_id'],$payment_notice['payment_id']);
		$payment_notice = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."payment_notice where id = ".$payment_notice_id);
	
		$sql = "select name ".
				"from ".DB_PREFIX."dc_order_menu ".
				"where order_id =". intval($payment_notice['order_id'])." limit 1";
		$title_name = $GLOBALS['db']->getOne($sql);
	
		$order_sn = $GLOBALS['db']->getOne("select order_sn from ".DB_PREFIX."dc_order where id = ".$payment_notice['order_id']);
		$money = round($payment_notice['money'],2);
		$money_fen=intval($money*100);
		...
		$payment_info['config'] = unserialize($payment_info['config']);
		$wx_config=$payment_info['config'];
		$data_notify_url = SITE_DOMAIN.APP_ROOT.'/callback/dc_payment/wwxjspay_notify.php';
