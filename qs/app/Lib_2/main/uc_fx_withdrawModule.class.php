...
class uc_fx_withdrawModule extends MainBaseModule
{
	...
	public function index()
	{
		...
		//可提现金额
		$inWithdrawSql = "select sum(money) as money, sum(fee) as fee from ".DB_PREFIX."fx_withdraw where user_id=".$user_info['id']." and is_paid=0 and is_delete=0";
		...
		$result=get_fx_withdraw_money($user_info['id']);
		$money=$money-$result;
	
		$GLOBALS['tmpl']->assign("withdraw_money",$money>0?round($money,2):0);
