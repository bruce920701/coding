	...
	{	
		...
		if($wx_payment)
		{
			...
			if($wx_payment['config']['scan']==1)
			{
				...
				$file = $directory. '/Dc_' .$wx_payment['class_name']."_payment.php";
				if(file_exists($file))
				{
					require_once($file);
					$payment_class = 'Dc_'.$wx_payment['class_name']."_payment";
 ...
 */
	...
	public function load_user_consignee($id){
		...
		// 如果没有默认地址，选第一个为默认地址
		if (empty($consignee_info_new['is_main']) && empty($id)) {
			$consignee_info_new['list'][0]['is_main'] = 1;
		}
		require_once(APP_ROOT_PATH."system/model/dc.php");
	...
	{ 
		...
		$ecv_pass = intval($_REQUEST['ecv_pass']);   //代金券是否需要验证，0为不验证，1为要验证
		...
		if (empty($consignee_id)) {
			showErr('请选择一个配送地址',$ajax,url("index","dcorder",array('lid'=>$location_id)));
		}
		if($address!=''){
		...
		if($result['location_delivery_info']['start_price'] > $result['pay_total_price']){
			showErr('起送价不足',$ajax);  //起送价不足
	...
	public function order_done()
	{
		...
		check_form_verify();
		//享受的优惠措失，带换行的字符串
		$order['promote_str']=$order_info['promote_str'];
		foreach($result['dc_promote'] as $k=>$v){
			//$order['promote_str'].=$v['promote_description']."<br/>";
			$order['promote_amount'] += $v['discount_amount'];
