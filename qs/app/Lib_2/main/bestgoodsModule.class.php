...
class bestgoodsModule extends MainBaseModule
{
	...
	{		
		...
		if (!$GLOBALS['tmpl']->is_cached('mall.html', $cache_id))
		{
			...
			foreach($index_cates as $k=>$v)
			{
				...
				$deal_result  = get_goods_list(8,array(DEAL_ONLINE,DEAL_NOTICE),array("city_id"=>$GLOBALS['city']['id'],"cid"=>$v['id']),""," d.buy_type = 2 ");
				
				foreach ( $deal_result['list'] as $kk => $vv ){
					if($vv['is_presell']==1){
						require_once(APP_ROOT_PATH."system/model/deal.php");
						$deal_result['list'][$kk]['name'] = format_deal_name($vv['name']);
						$deal_result['list'][$kk]['on_presell'] = 1;//正在参与预售活动
					}else if($group_info && $vv['allow_user_discount'] && $group_info['discount']<1){
				        $deal_result['list'][$kk]['current_price'] = round($vv['current_price']*$group_info['discount'],2);
				    }
				}
		...
		$GLOBALS['tmpl']->display("bestgoods.html",$cache_id);
	}
