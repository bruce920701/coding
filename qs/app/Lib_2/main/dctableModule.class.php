...
class dctableModule  extends MainBaseModule 
{
    ...
    function index() {
		...
		{
			$date_num=to_timespan($date);
			$now = NOW_TIME;
			
			$now=to_date($now ,"Y-m-d");
			$now=to_timespan($now);
    	...
    	/*url重写模式下日历获取的时间戳*/
    	$unix_date=$_REQUEST['unix_date'];
		
		if($unix_date){
			$GLOBALS['tmpl']->assign('unix_date',$unix_date);
		}else{
			//初始化当前时间戳
			$GLOBALS['tmpl']->assign('unix_date',NOW_TIME);
		}
    	/*url重写模式下日历获取的时间戳end*/
    ...
    public function add_table_cart(){
    	...
    	else
    	{
    		...
    		showErr("预订额已满",$ajax,url("index","dctable",array('lid'=>$location_id)));
