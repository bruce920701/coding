...
class indexModule extends MainBaseModule
{
	...
	{		
		global_run();
		load_auto_cache("cache_area",array("city_id"=>$GLOBALS['city']['id']));	 //商圈缓存
		...
		if (!$GLOBALS['tmpl']->is_cached('index.html', $cache_id))
		{
			...
			//输出首页推荐的分类
			$index_cates = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_cate where is_delete = 0 and is_effect = 1 and recommend = 1 order by sort");
			foreach($index_cates as $k=>$v)
			{
				...
				foreach ( $deal_result['list'] as $kk => $vv ){
				    if($group_info && $vv['allow_user_discount'] && $group_info['discount']<1){
