...
class youhuisModule extends MainBaseModule
{
	...
	{		
		...
		$cate_sql="select dc.id,dc.name,count(yl.id) as yl_count from ".DB_PREFIX."deal_cate as dc left join ".DB_PREFIX."youhui as yl on dc.id=yl.deal_cate_id and yl.is_effect=1 and (yl.youhui_type=2 or (yl.youhui_type=1 and yl.city_id=".$city_id.")) and (yl.valid_type = 1 OR ( yl.valid_type = 2 AND ( yl.use_end_time = 0 OR yl.use_end_time > ".NOW_TIME."))) where dc.is_delete=0 and dc.is_effect=1 group by dc.id";
		$cate_info=$GLOBALS['db']->getAll($cate_sql);
		
		$new_cate=array();

		$all_count="select count(*) from ".DB_PREFIX."youhui as y where y.is_effect=1  and (y.begin_time < ".NOW_TIME." or y.begin_time = 0) and (y.valid_type=1 or (y.valid_type=2 and (y.use_end_time=0 or y.use_end_time>".NOW_TIME."))) and  (y.youhui_type=2 or (y.youhui_type=1 and y.city_id=".$city_id."))";
		$total_count = $GLOBALS['db']->getOne($all_count);
		if($total_count>0){
    		$new_cate[0]['id']=0;
    		$new_cate[0]['name']="精选";
