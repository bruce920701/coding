...
class uc_orderModule extends MainBaseModule
{
	public function index()
	{
		...
		foreach($list as $k=>$v)
		{
			...
			$list[$k]['pay_amount_format'] = $v['pay_amount'];
			
			$pay_price = $v['total_price'] - $v['ecv_money'] - $v['youhui_money'] - ($v['presell_discount_money'] - $v['presell_deposit_money']);
            
			$list[$k]['total_price_format'] = $pay_price;
			$list[$k]['delivery_fee_format'] = $v['delivery_fee'];
			...
			{
				$list[$k]['deal_order_item'][$kk]['total_price'] = $vv['total_price'];
				$list[$k]['deal_order_item'][$kk]['unit_price'] = $vv['discount_unit_price'];
				...
				if($v['is_presell_order']==1){
					$list[$k]['deal_order_item'][$kk]['name'] = format_deal_name($vv['name']);
				}
			}
		}
		...
		else
		{
			...
			if($order_info)
			{
			    if(($order_info['is_delete'] && $order_info['is_cancel']) || $order_info['order_status']==1){
			        $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set is_delete = 1,is_cancel = 0 where (order_status = 1 or pay_status < 2) and ( ( is_delete = 1 and is_cancel=1 ) or is_delete = 0)  and user_id = ".$GLOBALS['user_info']['id']." and id = ".$id);
			        ...
			        $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set is_delete = 1,is_cancel = 1 where pay_status < 2 and is_delete = 0 and user_id = ".$GLOBALS['user_info']['id']." and id = ".$id);
			        require_once(APP_ROOT_PATH."system/model/deal_order.php");
			        if($GLOBALS['db']->affected_rows())
			        {
	...
	public function view()
	{
		...
		if($order_info)
		{
			...
			require_once(APP_ROOT_PATH . "system/model/deal_order.php");
			$result = check_order_allow_pay($order_info);

			$order_info['count_time']=$result['count_time'] - NOW_TIME;  //用于手机端倒计时
			$order_info['count_type']=$result['count_type'];
			$order_info['allow_pay'] = $result['allow_pay'];
			$order_info['count_time_format'] = to_date($result['count_time'],'Y/m/d H:i:s');
			...
			$fee['order_pay_price'] = $order_info['total_price'] - $order_info['ecv_money'] - $order_info['youhui_money'] - ($order_info['presell_discount_money'] - $order_info['presell_deposit_money']);
			$order_info['fee']=$fee;
        	//获取商家
			...
			{
				$order_info['deal_order_item'][$kk]['discount_unit_price'] = $vv['discount_unit_price'];
				$deal_info = load_auto_cache("deal",array("id"=>$vv['deal_id']));
				...
				if($order_info['is_presell_order'] == 1){
					$order_info['deal_order_item'][$kk]['name'] = format_deal_name($deal_info['name']);
				}
				//获取商品状态
				if($order_info['pay_status']==2){
				    $order_info['deal_order_item'][$kk]['status']=$this->order_item_status($vv, $order_info['order_status']);
					//尾款已付，预售发货时间
					$presell_delivery_time = $GLOBALS['db']->getOne("select presell_delivery_time from ".DB_PREFIX."deal where id=".$vv['deal_id']);

					$count_time = $presell_delivery_time - NOW_TIME;//时间差：预售商品发货时间-当前时间
					if($order_info['is_presell_order'] == 1 && $count_time > 0){//如果是全部付款完成，并且是预售订单
						$order_info['count_time'] = $count_time;
						$order_info['count_time_format'] = to_date($presell_delivery_time,'Y/m/d H:i:s');
					}
			...
			if($order_info['cod_money']>0){
				...
				$order_info['payment_info']['money']=$order_info['payment_info']['money'];
			}
