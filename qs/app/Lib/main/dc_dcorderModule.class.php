...
class dc_dcorderModule extends MainBaseModule
{
	...
	public function view()
	{
		...
		if($order_info)
		{
			...
			if(IS_OPEN_DADA){
			    //达达配送信息
			    $order_info['IS_OPEN_DADA'] = IS_OPEN_DADA;//是否开启达达配送
			    $dada_delivery = unserialize($order_info['dada_delivery']);
			    if($dada_delivery['order_status'] == 2 || $dada_delivery['order_status'] == 3 || $dada_delivery['order_status'] == 4){
			        require_once(APP_ROOT_PATH."system/delivery/DaDaDelivery.php");
			        $DaDaDelivery = new DaDaDelivery();
			        $dada_info = $DaDaDelivery->queryOrder($order_info['order_sn']);
			        require_once(APP_ROOT_PATH."system/common.php");
			        $distance = getDistance($dada_info['transporterLng'],$dada_info['transporterLat'],$order_info['xpoint'],$order_info['ypoint']);
			        $order_info['distance'] = $distance / 1000;
			        $order_info['dada_order_status'] = $dada_delivery['order_status'];
			        if($dada_delivery['order_status'] == 2){
			            $order_info['send_info']="达达骑手正赶往取餐";
			        }elseif($dada_delivery['order_status'] == 3){
			            $order_info['send_info']="达达骑手正努力向您飞奔";
			        }elseif($dada_delivery['order_status'] == 4){
			            $order_info['send_info']="已送达";
			        }
			    }
			}
			...
			$payment_notice_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."payment_notice where order_id = ".$order_info['id']." and is_paid = 1 and order_type = 1 order by create_time desc");
	...
	public function dc_reminder(){
		...
		if(check_save_login()!=LOGIN_STATUS_LOGINED)
		{
			...
			if($order_info)
			{
			    $time = 2700;//下单大于45分钟才可以催单
			    if(NOW_TIME - $order_info['create_time']<$time){
			        $result['status'] = 0;
			        $result['info'] = "下单后45分钟才可以催单哦";
			        ajax_return($result);
			    }else{
