...
class uc_fxModule extends MainBaseModule
{
	...
	public function my_fx(){
        ...
        //二维码链接
        $json['type']=602;
        $json['user_id']=intval($user_info['id']);
        $GLOBALS['tmpl']->assign('qrcode_url',SITE_DOMAIN.url("index","uc_home#mall",array("r"=>base64_encode($user_info['id']),"json"=>base64_encode(json_encode($json)))));
	    //通用模版参数定义
