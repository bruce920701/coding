...
// 		            es_session::delete("verify");
		...
		if(jh_send_verify_sms($mobile_phone,$code)) {
            $data['status'] = true;
            $data['info'] = "发送成功";
	...
	*/
	...
	public function addcart($ajaxReturn=true,$param=array())
	{
        ...
        $consume_max = $global_config['consume_max'];
        ...
        $left_buy = $consume_max - $total_consume;
        $buy_num =  intval($_REQUEST['number'])<=0?1:intval($_REQUEST['number']);

        $total_consume_1 = $deal_info['current_price']*$buy_num + $total_consume;
        if($total_consume_1 > $consume_max)
        {
            $res['status'] = 0;
            $res['info'] = "总共可以购买".$consume_max."，您已经购买了".$total_consume."，总共还能购买".$left_buy;
            ajax_return($res);
        }
...
/*
		if($deal_info['buy_type'] == 2)
        {
            //$user_credits = intval($GLOBALS['db']->getOne("select avalible_consume_credits  from ".DB_PREFIX."user where id = ".intval($GLOBALS['user_info']['id'])));
            if($deal_info['return_total_score'] < $GLOBALS['user_info']['avalible_consume_credits'])
            {
                $res['status'] = 0;
                $res['info'] = "消费积分不足";
                ajax_return($res);
...
*/
		...
		else
		{
			...
			{
				//需要积分兑换
				$user_score = intval($GLOBALS['db']->getOne("select avalible_consume_credits from ".DB_PREFIX."user where id = ".intval($GLOBALS['user_info']['id'])));
...
// 		$total_money = $cart_result['total_data']['return_total_money'];
	...
	public function count_buy_total()
	{
		...
		if($result['presell_deposit_money']>0)
		{
		    $format=format_price_html(round($result['presell_deposit_money'],2),2);
		    $feeinfo[] = array(
		        "name" => "- 应付订金：",
		        "symbol" => 4,
		        "value" => $format['bai'].'.'.$format['fei']
		...
		if($result['presell_left_price']>0)
		{
		    $format=format_price_html(round($result['presell_left_price'],2),2);
		    $feeinfo[] = array(
		        "name" => "- 商品尾款：",
		        "symbol" => 4,
		        "value" => $format['bai'].'.'.$format['fei']
		...
		$result['money']=$GLOBALS['user_info']['register_credits'];
		...
		if($data['is_pick']==1){
		    ...
		    $delivery_list = get_express_fee($goods_list,intval($address_id));
		    
		    
		    foreach($delivery_list as $k=>$v){
		        if($v['is_delivery_free']==1){
		            $delivery_info = "满".round($v['delivery_free_money'],2)."免运费";
		        }else{
		            if($v['total_fee'] > 0){
		                $delivery_info = round($v['total_fee'],2)."元";
		            }else{
		                $delivery_info= '包邮';
		            }
		        }
		        $delivery_list[$k]['delivery_info'] = $delivery_info;
		    }
		    
		    $data['delivery_info']=$id == 0?$delivery_list:array();
		}
	...
	public function count_order_total()
	{
		...
		$goods_list = $GLOBALS['db']->getAll("select doi.* , d.allow_user_discount , do.presell_deposit_money,do.presell_discount_money,do.presell_type,d.presell_delivery_time,d.is_presell,d.presell_begin_time,do.presell_end_time from ".DB_PREFIX."deal_order_item as doi left join ".DB_PREFIX."deal as d on doi.deal_id=d.id left join ".DB_PREFIX."deal_order as do on doi.order_id=do.id where doi.order_id = ".$order_id);
		...
		$GLOBALS['tmpl']->assign("is_delivery",$is_delivery);
		if($order_info['type']==8 && $order_info['pay_status']>0){
		...
		$presell_discount = $result['presell_discount_money'] - $result['presell_deposit_money'];
		if($presell_discount >0)
		{
		    $format=format_price_html(round($presell_discount,2),2);
		    $feeinfo[] = array(
		        "name" => "订金抵扣优惠：",
		        "symbol" => -1,
		        "value" => $format['bai'].'.'.$format['fei']
		...
		$data = $result;
	    $delivery_list = get_express_fee($goods_list,intval($order_info['consignee_id']));
	    ...
	    $data['delivery_info']=$delivery_list;
...
// 			ajax_return($rs);
	...
	public function verification_code(){
	    ...
	    //获取验证码
	    require_once APP_ROOT_PATH."system/utils/verification_code.php";
	    $verification_code=verification_code::get_verification_code();
	    $GLOBALS['tmpl']->assign("sms_ipcount",load_sms_ipcount());
	    $GLOBALS['tmpl']->assign("verification_code",$verification_code['verification_code']);
	    $GLOBALS['tmpl']->assign("is_verification_code",app_conf("ICON_VERIFICATION_CODE"));
	    $GLOBALS['tmpl']->assign("key",$verification_code['key']);//生成的答案
	    $data['status']=1;
	    $data['html'] = $GLOBALS['tmpl']->fetch("inc/verification_code.html");
