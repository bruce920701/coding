...
class cartModule extends MainBaseModule
{
	...
	public function check()
	{
		...
		$delivery_list=get_express_fee($cart_list,intval($consignee_id));
		...
		foreach ($group_list as $k=>$v){
		    ...
		    foreach($v['goods_list'] as $kk=>$vv){
				...
				if($is_presell==1){
					$group_list[$k]['goods_list'][$kk]['name'] = format_deal_name($vv['name']);
				}
		    }
			...
			if($delivery_list){
				if($delivery_list[$k]['is_delivery_free']==1){
					...
					if($delivery_list[$k]['total_fee'] > 0){
						$delivery_info = round($delivery_list[$k]['total_fee'])."元";
					}else{
						$delivery_info= '包邮';
					}
				}
			...
			$group_list[$k]['delivery_fee']=$delivery_info;
			if($buy_type == 0 && $is_presell==0){  //积分商品兑换不使用优惠券和红包
		...
		foreach($payment_list as $k=>$v)
		{
			if($v['class_name']=="Voucher"||$v['class_name']=="Account"||$v['class_name']=="Otherpay")
			{
				...
				if($v['class_name']=="Voucher" && $is_presell==0)
				{
					$directory = APP_ROOT_PATH."system/payment/";
					$file = $directory. '/' .$v['class_name']."_payment.php";
					if(file_exists($file))
				...
				{
				    $disp_paylist[] = $v;
				}elseif($v['class_name']=="Voucher" && $is_presell==0){
				    $disp_paylist[] = $v;
				}
	...
	public function done()
	{
		...
		$order['xpoint']	=	$consignee_info['consignee_info']['xpoint'];
		$order['ypoint']	=	$consignee_info['consignee_info']['ypoint'];
...
*/
	...
	public function order_done()
	{
		...
		if($order['is_presell_order']==1 && $order['pay_status']>0){
		    $order['account_money'] =  $order['pay_amount'];
		}
