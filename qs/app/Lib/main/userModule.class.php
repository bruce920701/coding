...
class userModule extends MainBaseModule
{
	...
	public function login()
	{
		...
		{
		    app_redirect(url("index","uc_myinfo"));
		}
		$fanwe_mobile = es_cookie::get("fanwe_mobile");
		$GLOBALS['tmpl']->assign("fanwe_mobile",$fanwe_mobile);
		$GLOBALS['tmpl']->assign("sms_lesstime",load_sms_lesstime());
	...
	public function dologin()
	{
		...
		*/
		...
		if($result['status'])
		{
			...
			if(substr($_SERVER['HTTP_REFERER'],-9)=="act=login" || 
			    substr($_SERVER['HTTP_REFERER'],-12)=="act=register")
			{
			    $return['jump']=url("index","uc_myinfo");
			    $return['page']=1;
			}
			$return['tip'] = $tip;
            $this->_checkUserFx();
	...
	public function doregister()
	{
		...
		$this->_checkAddress();
        $user_data['city_id']=$_REQUEST['city_id'];
        $user_data['province_id']=$_REQUEST['province_id'];
        $user_data['city_code']=$GLOBALS['db']->getOne("select code from ".DB_PREFIX."delivery_region where id=".$_REQUEST['city_id']);
        $user_data['agency_id']=$GLOBALS['db']->getOne("select id from ".DB_PREFIX."agency where city_code=".$user_data['city_code']);
	...
	public function dophregister()
	{
		...
		if($user_name == ''){
            ...
            $data['info'] = "请输入用户名";
            ajax_return($data);
        }elseif(mb_strlen($user_name,'utf8')<4 || mb_strlen($user_name,'utf8')>20){
            $data['status'] = false;
            $data['info'] = '必须 4-20 个字符';
            ajax_return($data);
        }elseif(preg_match("/^[0-9]+$/i", $user_name, $matches)){
            $data['status'] = false;
            $data['info'] = '不能为纯数字';
            ajax_return($data);
        }
	...
	public function dogetpassword()
	{
		...
		$iconcode = strim($_REQUEST['iconcode']);
		if(app_conf("ICON_VERIFICATION_CODE")){
		    $data = check_field("verify_code",$iconcode,0);
		}else{
		    $data = check_field("verify_code",$verify,0);
		}
   ...
   private function _checkAddress(){
    ...
    private function _checkUserFx(){
        ...
        $city_id=$GLOBALS['user_info']['city_id'];
        if(!$city_id){
            $data['status'] = 1;
            $data['info']	=  "本系统全新升级,为了让我们更好的提供服务,请完善您的个人信息资料~";
            $data['jump'] =url("index","uc_account#index");
            ajax_return($data);
        }
        if(!$update['city_code']){
            $city_code=$GLOBALS['db']->getOne("select code from ".DB_PREFIX."delivery_region where id=".$city_id);
