	...
	{	
		...
		if($history_ids)
		{
			...
			foreach($history_ids as $k=>$v)
			{
				...
				{
				    if($group_info && $history_item['allow_user_discount'] && $group_info['discount']<1){
		...
		$cate_cache = load_auto_cache("cache_shop_cate",array("all"=>1)); //商城分类缓存
		...
		*/
		...
		foreach($price_level as $k=>$v)
		{
			...
			if($v['min']==0&&$v['max']>0)
			{
				$range_name = $v['max']."积分以下";
			}
			else if($v['max']==0&&$v['min']>0)
			{
				$range_name = $v['min']."积分以上";
			}
			else 
			{
				$range_name = $v['min']."-".$v['max']."积分";
			}
...
// 			$kw_arr = div_str($GLOBALS['kw']);
// 			foreach($kw_arr as $kw)
// 			{
// 				$kw_unicode .= " +".str_to_unicode_string($kw)." ";
// 			}

// 			if($kw_unicode)
// 				$ext_condition .=" and (match(d.name_match) against('".$kw_unicode."' IN BOOLEAN MODE)) ";
		...
		foreach ($deal_list as $k => $v){
		    if($group_info && $v['allow_user_discount'] && $group_info['discount']<1){
		        $deal_list[$k]['current_price'] = round($v['current_price']*$group_info['discount'],2);
		    }
		}
		...
		foreach($side_deal_list as $k=>$v)
		{
		    if($group_info && $v['allow_user_discount'] && $group_info['discount']<1){
		        $side_deal_list[$k]['current_price'] = round($v['current_price']*$group_info['discount'],2);
		    }
		}
