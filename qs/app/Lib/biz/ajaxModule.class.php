...
class  ajaxModule extends BizBaseModule{
    ...
    public function attr_table_td($attr_stock,$key){
        ...
        $html .='<td><input  type="hidden" name="presell_discount_money[]" value="' . $attr_stock[$key]['presell_discount_money'] . '" /><input  type="hidden" name="presell_deposit_money[]" value="' .$attr_stock[$key]['presell_deposit_money'] .'" /><input type="hidden" name="stock_buy_count[]" value="'.intval($attr_stock[$key]['buy_count']).'" /><span>'.intval($attr_stock[$key]['buy_count']).'</span></td>';
        return $html;
    }
    ...
    public function refund_init()
    {
        ...
        if($order_item) {
            ...
            // 当前商品实际支付价格
            $real_total_price         = $order_item['discount_unit_price']*$order_item['number'];
            ...
            $total_f=get_pay_amount_in_deal_order($order_info);
