...
class userModule extends BizBaseModule{
    public function login(){
        ...
        if($GLOBALS['user_info']){
            if($GLOBALS['user_info']['is_merchant']==0)
                app_redirect(url('biz','user#register'));
        }


        init_app_page();
        $GLOBALS['tmpl']->assign("biz_login_qrcode",gen_qrcode(SITE_DOMAIN.wap_url("biz","user#login"),app_conf("QRCODE_SIZE")));
    ...
    public function register(){
        ...
        if($GLOBALS['user_info']){
            ...
            $supplier_data = $GLOBALS['db']->getRow("SELECT * FROM ".DB_PREFIX."supplier_submit WHERE user_id=".$GLOBALS['user_info']['id']);
            if($supplier_data){
                $step = 4;
        ...
        if ($step==2){
            ...
            $province_id=array();
            foreach ($deal_city_list as $t => $v){
                foreach ($v as $tt => $vv){
                    if(!in_array($vv['pid'], $province_id)){
                        $province_id[]=$vv['pid'];
                    }
                }
            }
            ...
            $province_list = $GLOBALS['db']->getAll("select id,name from ".DB_PREFIX."deal_city where is_effect = 1 and is_delete = 0 and pid = 0  order by sort desc");

            foreach ($province_list as $t => $v){
                if(!in_array($v['id'], $province_id))
                    unset($province_list[$t]);
            }
            ...
            if($_POST)
            {
                ...
                $data['share_code'] = strim($_REQUEST['share_code']);
    ...
    function do_login(){
        ...
        $iconcode = md5(strim($_POST['iconcode']));
        $session_verify = es_session::get('verify');

        if(app_conf("ICON_VERIFICATION_CODE")){
		    //验证码
		    $iconcode = strim($_REQUEST['iconcode']);
		    if(es_session::get("verification_code")!=md5("&#x".$iconcode.";"))
		    {
		        $data['status'] = false;
		        $data['info'] = "验证码错误";
		    ...
		    es_session::delete("verification_code");
		}else{
        ...
        //2017-5-12 商户登录流程修改,先登录会员账户自动登录商户账户,两个账户合并
        ...
        {
            //判断是否已经开通商户
            if($result['user']['is_merchant']==0){
                $jump_url = url("biz","user#register");
                showErr("暂未成为商户",1,$jump_url,1);
            }else{
                global_run();
                //获取权限
                ...
                if(empty($biz_account_auth)){
                    showBizErr("此商户账户权限不足,请更换账户登录!",1,'',0);
                }else{
    ...
    function do_register(){
        ...
        //$account_name = strim($_POST['account_name']);
        $account_name = get_round_supplier_name();//随机生成商户账户
        $account_mobile = strim($_POST['account_mobile']);
        $sms_verify = strim($_POST['sms_verify']);
        //$account_password = strim($_POST['account_password']);
        $account_password = md5($account_name);//随机生成商户账户密码
...
//		}
        ...
        if($GLOBALS['db']->getRow("select * from ".DB_PREFIX."supplier_submit where is_publish=0 and (account_mobile = '".$account_mobile."' or account_name='".$account_name."')")
            || $GLOBALS['db']->getRow("select * from ".DB_PREFIX."supplier_account where is_delete=0 and (mobile = '".$account_mobile."' or account_name='".$account_name."')")
        )
        {
            $data['status'] = false;
            $data['info'] = "该商户或手机已被注册或正在申请中";
            $data['field'] = "account_name";
            ajax_return($data);
        }


        if($sms_verify=="" && app_conf("SMS_ON") == 1 &&  app_conf("BIZ_REGISTER_SMS") ==1)
        {
            $data['status'] = false;
            $data['info']	=	"请输入收到的验证码";
        ...
        //查找所属城市代理商
        $ins_data['city_code']=$GLOBALS['db']->getOne("select code from ".DB_PREFIX."deal_city where id=".$base_data['city_id']);
        $agency_id=$GLOBALS['db']->getOne("select id from ".DB_PREFIX."agency where city_code=".$ins_data['city_code']);
        if($agency_id)
            $ins_data['agency_id'] = $agency_id;
     ...
     * @return array
     */
    function check_register_field($param_data)
    {
        $data = array();
        $data['status'] = true;
        $data['info'] = "";

        if(strim($param_data['account_name']))
        {
            if($GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."supplier_account where account_name = ".strim($param_data['account_name']))>0 ||
                $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."supplier_submit where is_publish <> 2 and account_name = ".strim($param_data['account_name']))>0){
                $data['status'] = false;
                $data['info'] = "该商户已被注册或正在申请中";
                $data['field'] = "account_name";
                ajax_return($data);
            }
        }

        if(strim($param_data['account_mobile']))
        {
            if(!check_mobile($param_data['account_mobile']))
            {
                $data['status'] = false;
                $data['info'] = "手机号格式不正确";
                $data['field'] = "account_mobile";
        ...
        if(strim($param_data['verify_code']) && app_conf("SMS_ON") == 1)
        {

            $verify = md5($param_data['verify_code']);
            $session_verify = es_session::get('verify');
            if($verify!=$session_verify)
            {
                $data['status'] = false;
    ...
    public function load_sub_city()
    {
        $pid = intval($_REQUEST['pid']);
        $html='';
        if($pid>0){
            $city_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_city  where is_effect = 1 and is_delete = 0 and pid = ".$pid);

            $html .= "<select name='city_id'  class='ui-select'>";;
    ...
    public function recheck(){
        global_run();
        if($GLOBALS['user_info']){
            $GLOBALS['db']->query("delete from ".DB_PREFIX."supplier_submit where user_id= ".$GLOBALS['user_info']['id']);
            $data['jump'] = url("biz","user#register");
            $data['info'] = "重新填写申请资料";
            $data['status'] = 1;
        } else{
            $data['info'] = "操作错误";
