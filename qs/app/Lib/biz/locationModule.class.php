...
class locationModule extends BizBaseModule
{
	...
	{		
        ...
        foreach ($list as $k => $v) {
            ...
            $list[$k]['update_url'] = url("biz","location#update",array("id"=>$v['id']));
            if($v['deal_cate_id'])
	...
	public function publish(){
	    ...
	    $defined_is_dc=$is_dc;//是否开启外卖
	    
	   /* 系统默认 */
	    $GLOBALS['tmpl']->assign("vo", $defined_is_dc);//是否开启外卖
	    $GLOBALS['tmpl']->assign("ajax_url", url("biz", "location"));
	...
	public function do_save_publish(){
	    ...
	    $area_id[] = intval($_REQUEST['p_area_id']);
	    ...
	    $data['district']=strim($_REQUEST['district']);//地区
	    $data['brief'] = btrim(no_xss($_REQUEST['brief'])); // 部门简介
	    $data['mobile_brief'] = btrim(no_xss($_REQUEST['mobile_brief']));
	...
	public function edit()
	{
	    ...
	    if ($edit_type == 1) {//管理员发布
	        ...
	        if($location_info['location_id']>0){$is_online = 1;}
	    }                                  
	    /*外卖*/
	    if(defined("DC")){
	        $is_dc=1;
	    ...
	    $location_info['defined_is_dc']=$is_dc;
	     /* 数据 */
	    $GLOBALS['tmpl']->assign("is_online", $is_online); // 是否为在线数据
	...
	public function update(){
	    ...
	    /* 系统默认 */
	    $GLOBALS['tmpl']->assign("ajax_url", url("biz", "location"));
	    $GLOBALS['tmpl']->assign("page_title", "门店资料更新");
	    $GLOBALS['tmpl']->display("pages/location/update.html");
	}
	...
	public function do_update(){
	    ...
	    if(!preg_match($isMob,$tel)/* && !preg_match($isTel,$tel)*/){
	        $result['status'] = 0;
	        $result['info'] = "联系电话格式不正确";
	        ajax_return($result);
	        exit;
	    }

	    if(strim($_REQUEST['contact'])==''){
	        $result['status'] = 0;
	        $result['info'] = "联系人不能为空";
	        ajax_return($result);
	        exit;
	    ...
	    if(strim($_REQUEST['xpoint'])=='' || strim($_REQUEST['ypoint'])==''){
	        $result['status'] = 0;
	        $result['info'] = "请地图定位";
	        ajax_return($result);
	        exit;
	    ...
	    if($id>0){
	        ...
	        if($location_info){
	            ...
	            if($location_submit_info){ //存在数据
	                ...
	                }elseif($location_submit_info['biz_apply_status']==3){
	                    $result['status'] = 0;
	...
	public function load_area_list_box()
	{
	    ...
	    $area_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."area where city_id = ".$city_id." and pid=0");
	...
	private function check_location_publish_data($data){
		...
		do {
		    ...
		    if (empty($data['xpoint']) || empty($data['ypoint'])) {
		    	$info = '必须在地图上定位一个坐标';
		    	break;
		    }
