$(document).ready(function(){
	...
	//清空时间
	$(".clear_input_btn").bind("click",function(){
	...
	if($(".verification_code").length>0)
	{
		$.ajax({
			...
			success:function(obj){
				if(obj.status){
					$(".verification_code").html(obj.html);
					$(".icon-list").click(function(){
						//选择验证码图标
						$(this).addClass('active').siblings().removeClass('active');
						var iconcode = $(this).find(".iconcode").attr("rel");
						$("input[name='iconcode']").val(iconcode);
					...
					$(".refresh_verify").live("click",function(){
...
window.onresize=function(){  
    init_side_nav_height();
};  
/*
function init_biz_nav()
{
	$(".nav_item").find("dd").hide();
	$(".nav_item").find("dd div.current").parent().parent().find("dd").show();
	$(".nav_item").find("dd div.current").parent().parent().find("dt").addClass("open");
	
	$(".nav_item").find("dt").bind("click",function(){
		$(".nav_item").find("dd").hide();
		$(".nav_item").find("dt").removeClass("open");
		$(this).parent().find("dd").show();
		$(this).parent().find("dt").addClass("open");
		location.href = $(this).parent().find("dd:first div a").attr("href");
	});
}
*/
//以下是处理UI的公共函数
...
function init_side_nav_height(){
    $(".left_box").css("height",$(window).height()-60);
}
/*选中事件*/
function init_side_nav_cur(){
    $(".j-nav-tit").bind('click',function() {
    	var nav_length=$(this).parent().find('dd').length;
    	if ($(this).parent().hasClass('j-open')) {
	    	$(this).parent().css('height', '40px');
	    	$(this).parent().removeClass('j-open');
    	}
    	else{
    		$('.main-nav').removeClass('j-open').css('height', '40px');
	    	$(this).parent().css('height', (nav_length+1)*40);
	    	$(this).parent().addClass('j-open');
    	}
    });
}
