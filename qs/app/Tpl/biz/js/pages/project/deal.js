$(function(){
	...
	/*日期控件*/
	$("input[name='begin_time']").datetimepicker({format: "Y-m-d H:i",step:10});
	$("input[name='end_time']").datetimepicker({format: "Y-m-d H:i",step:10});
	
	$("input[name='coupon_begin_time']").datetimepicker({format: "Y-m-d H:i",step:10});
	$("input[name='coupon_end_time']").datetimepicker({format: "Y-m-d H:i",step:10});
	...
	$("select[name='coupon_time_type']").bind("change",function(){
		coupon_time_is_hide_show();
	});
	coupon_time_is_hide_show();
	function coupon_time_is_hide_show(){
		var cur_type = $("select[name='coupon_time_type']").val();
		if(cur_type == 1){
...
/*JQUERY END*/
//new
	$("label[name='is_coupon']").bind("click",function() { 
		if($("input[name='is_coupon']").prop("checked")){
			$(".is_coupon").each(function(k,v){
				if($(v).hasClass("coupon_time_type_day")){
					if($("input[name='coupon_time_type']").val()==1){
						$(v).show();
					}
				}else if($(v).hasClass("coupon_time_type_datetime")){
					if($("input[name='coupon_time_type']").val()!=1){
						$(v).show();
					}
				}else{
...
function check_deal_form_submit(){
	...
	//分类
	if($.trim($("input[name='cate_id']").val())==''&&$.trim($("input[name='deal_cate_type_id']").val())==''){
		$.showErr("请选择分类");
		return false;
	}
	//支持门店
