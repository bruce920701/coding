...
$(document).ready(function() {
	...
	load_sub_cate();
	// 省份改变的时候
	$("select[name='city_pv_id']").bind("change", function() {
		load_city_list();
	});
	// 城市改变的时候
	...
	$("form[name='user_register_2']").bind("submit", function() {
		...
		if($("#city_pv_id").val() == "0" && is_error == 0) {
			console.log(0);
			$("dl[name='city_pv_id']").click();
			obj = $("#city_pv_id");
			is_error = 1;
			error_msg = "请选择所属城市！";
			alert(error_msg);
		...
		if(!$.checkMobilePhone($("#tel").val()) && is_error == 0) {
			$("#tel").focus();
			is_error = 1;
			error_msg = "商户手机号格式不正确";
		}
	...
	$("div.h_license").ui_upload({multi:false,FilesAdded:function(files){
		...
		if(responseObject.error==0)
		{
			...
			$(".h_license_img").attr("src",responseObject.small_url);
			$("#h_license_big_img .big_img_item").html("<img  src=\'"+responseObject.big_url+"\'/>");
		}
		else
		{
	...
	$("div.h_other_license").ui_upload({multi:false,FilesAdded:function(files){
		...
		if(responseObject.error==0)
		{
			...
			$(".h_other_license_img").attr("src",responseObject.small_url);
			$("#h_other_license_big_img .big_img_item").html("<img  src=\'"+responseObject.big_url+"\'/>");
		}
		else
		{
			$.showErr(responseObject.message);
	...
	$("div.h_supplier_logo").ui_upload({multi:false,FilesAdded:function(files){
		...
		if(responseObject.error==0)
		{
			...
			$(".h_supplier_logo_img").attr("src",responseObject.small_url);
			$("#h_supplier_logo_big_img .big_img_item").html("<img  src=\'"+responseObject.big_url+"\'/>");
		}
		else
		{
			$.showErr(responseObject.message);
	...
	$("div.h_supplier_image").ui_upload({multi:false,FilesAdded:function(files){
		...
		if(responseObject.error==0)
		{
			...
			$(".h_supplier_image_img").attr("src",responseObject.small_url);
			$("#h_supplier_image_big_img .big_img_item").html("<img  src=\'"+responseObject.big_url+"\'/>");
		}
		else
		{
			$.showErr(responseObject.message);
...
function load_city_list() {
	var pv_id = $("select[name='city_pv_id']").val();

	var ajaxurl = APP_ROOT + "/biz.php?ctl=user&act=load_sub_city&pid=" + pv_id;
	$.ajax({
		url : ajaxurl,
		success : function(html) {
			if(html != "") {
...
function recheck() {
	ajaxurl = APP_ROOT + "/biz.php?ctl=user&act=recheck";
	$.ajax({
		url : ajaxurl,
		dataType : "json",
		success : function(data) {
