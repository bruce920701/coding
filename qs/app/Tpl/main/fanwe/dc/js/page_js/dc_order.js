$(document).ready(function(){
	...
	$('.dc_con_del').bind('click',function(event){
		var query = new Object();
		var o=$(this);
		query.id = parseInt($(this).attr('data-parmas'));	
...
function save_weebox_consignee(){
	
var obj=$('.dc_save_con').parents('.dc_action');
	...
	if($("input[id='q_text2']").length > 0){
		if(!check_empty($("input[id='q_text2']"),'请输入地理位置')){	
			return false;
		}
	}
...
function save_dc_consignee(o){
	...
	var url = "//api.map.baidu.com/place/v2/search?ak="+op_ak+"&output=json&query="+op_q+"&page_size="+op_page_size+"&page_num="+op_page_num+"&scope=1&region="+op_region;
	var query=new Object();
