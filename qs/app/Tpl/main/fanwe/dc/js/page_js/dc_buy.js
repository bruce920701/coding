	...
	$.ajax({
		...
		success:function(data){
			if(data.status==1){
				$('#dc_cartsection').html(data.html);
				if (data.menu_num) {
					for (var menu_id in data.menu_num) {
						var obj = $('.add_to_cart_span').find('i[data-i="'+menu_id+'"]');
						change_menu_buy_count(obj, data.menu_num[menu_id]);
					}
				}
			}	
		}
