	...
	$.ajax({ 
		...
		success: function(data){
			$.each(data.delivery_info,function (index, obj) {
				...
				$(".memo_"+index).parent(".order-tip").append("<p class='logistics-way f_r'>运送方式：普通配送 快递<span class='price price_"+index+"'>"+obj.delivery_info+"</span></p>");
			});
			$(".logistics-way").show();
			if(data.is_pick){
			...
			data.pay_price = parseFloat(data.pay_price);

			
			if(data.money<data.pay_price){
