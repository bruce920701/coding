	...
	$(".j-activeopen").click(function(){
		...
		if(childlengh > 1){
			if($(this).hasClass("isClick")){
				...
				$(this).find(".tag_box .iconfont").first().show();
				$(this).find(".tag_box .iconfont").last().hide();
			}else{
				$(this).addClass("isClick");
				$(this).height((childheight * childlengh)  + (childmargin * (childlengh + 1)));
				$(this).find(".tag_box .iconfont").first().hide();
				$(this).find(".tag_box .iconfont").last().show();
			}
		}
	});

	$(".J-gz_detail").click(function(){
		$(this).toggleClass("active");
		$(".guize_detail").toggleClass("active");
	});
	...
	$.init_buy_ui = function(){
		...
		{			
			...
			if(attr_spec_stock_cfg)
			{
				...
				deal_show_price+=parseFloat(attr_spec_stock_cfg['price']);
			}
			else
			{
		...
		if(is_choose_all&&is_stock)
		{
			//更新价格
			if(buy_type!=1)
			$("#deal_price").html(Math.round(deal_show_price*100)/100);
		...
		else
		{
			...
			{
				if(buy_type!=1)
				$("#deal_price").html(Math.round(deal_price*100)/100);
			}
...
function init_kf_qq()
{
	...
	$(window).scroll(function(){		
		...
		if($.browser.msie && $.browser.version =="6.0")
		{
			$("#s_kf_qq").css("top",$(document).scrollTop()+$(window).height()-80);
		}	
