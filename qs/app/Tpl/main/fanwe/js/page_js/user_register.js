...
function init_province(){
   var html="<option value='0'>所在省份</option>";
   for(var i in regionConf){
       html+="<option value='"+regionConf[i].i+"'>"+regionConf[i].n+"</option>";
   }
   $(".settings_province_id").html(html);
   $(".settings_province_id").each(function(){
       $(this).ui_select({refresh:true});
   });
}
//选择省份修改市级
function load_city($city,city_id)
{
    var id = city_id;
    var city=regionConf["r"+city_id]['c'];
    if(id==0)
    {
    ...
    else
    {
        ...
        for(var key in city)
        {
            html+="<option value='"+city[key].i+"'>"+city[key].n+"</option>";
        }
    }
    $city.html(html);
    $city.ui_select({refresh:true});
...
function init_register_panel()
{
    $("select[name='province_id']").bind("change",function(){
        load_city($(this).parent().next().find("select"),$(this).val());
    });
		...
		if(!$(register_panel).find("input[name='user_name']").attr("bindblur"))
		{
			...
			$(register_panel).find("input[name='user_name']").bind("blur",function(){
				...
				if($.trim(txt)=="")
				{
					...
					form_err($(this),"请输入用户名");
				}
				else
				{
					//（支持中文、字母、数字、“_” 的组合，4-20个字符 ，且不能为纯数字）
					var reg = new RegExp("^(?!_)(?!.*?_$)[a-zA-Z0-9_\u4e00-\u9fa5]+$");
					var reg_num = new RegExp("^[0-9]*$");
					if(!reg.test(txt)){
						form_err($(this),'支持中文、字母、数字、“_” 且不能以"_" 开头和结尾');
					}else if(txt.length<4 || txt.length>20){
						form_err($(this),'必须 4-20 个字符');
					}else if(reg_num.test(txt)){
						form_err($(this),'不能为纯数字');
					}else{
						//验证用户名唯一性
	...
	$(".register-panel").each(function(k,register_panel){
		if(!$(register_panel).find("div.ph_verify_btn").attr("bindclick"))
		{
			...
			$(register_panel).find("div.ph_verify_btn").bind("click",function(){		
				...
				query.iconcode = $.trim($(form).find("input[name='iconcode']").val());
	...
	$(".register-panel").find(".register").each(function(i,form){
		if(!$(form).attr("bindsubmit"))
		{
			...
			$(form).bind("submit",function(){
				...
				{
					form_err(user_pwd_confirm,"您两次输入的密码不一致");
					return false;
				}
				var $province=$(form).find("select[name=province_id]");
                if($.trim($province.val())==0){
                    form_tip($province,"请选择省份");
                    return false;
                }
	...
	$(".register-panel").find(".ph_register").each(function(i,form){
		if(!$(form).attr("bindsubmit"))
		{
			...
			$(form).bind("submit",function(){
                ...
                if($.trim($province.val())==0){
                    form_tip($province,"请选择省份");
                    console.log($province.parent().parent().find(".form_tip"));
                    return false;
                }
                var $city=$(form).find("select[name=city_id]");
                if($.trim($city.val())==0){
