...
class cartModule extends MainBaseModule
{
	...
	public function addcart(){
		...
		$is_relate = false;
		$ids = $_REQUEST['id'];
		$is_presell = $_REQUEST['is_presell'];
	    ...
	    if( !empty($ids)&&(is_array($ids)) ){
			...
			$data = call_api_core("cart","addcart",array("id"=>$id,"deal_attr"=>$deal_attr,"num"=>$num,"is_presell"=>$is_presell));
		}
...
// 	        }
	         ...
	         $cart_data = get_cart_type($id,'wap',$is_presell);
...
//		print_r($data);exit;
	...
	public function done()
	{
		...
		$param['is_presell'] = $_REQUEST['is_presell'];
	...
	public function pay()
	{
		...
		$arr=array('data_id'=>$param['id'],'is_done'=>1);
		if(APP_INDEX=='app'){
			$json_parma=addslashes(json_encode($arr));
			$back_url ='javascript:App.app_detail(308,"'.$json_parma.'");';
			
		}else{
			$back_url = SITE_DOMAIN.wap_url("index","uc_order#view",$arr);
		}
		$GLOBALS['tmpl']->assign("back_url",$back_url);
