...
class uc_addressModule extends MainBaseModule
{
	...
	public function index()
	{
		...
		*/
		...
		$is_service = intval($_REQUEST['is_service']);
		$tid = intval($_REQUEST['tid']);
		$rs_date = strim($_REQUEST['rs_date']);
		$consignee_id = intval($_REQUEST['consignee_id']);


		if($check){
			if ($is_service) {
				$back_url = wap_url('index', 'vs_cart', array('consignee_id'=>$consignee_id,'id'=>$id));
			} else {
		...
		foreach($data['consignee_list'] as $k=>$v){
		    if($check){
		    	if ($is_service) {
		    		$data['consignee_list'][$k]['carturl']= wap_url("index","vs_cart",array("id"=>$id,"consignee_id"=>$v['id'],"tid"=>$tid,'id'=>$id, 'rs_date'=>$rs_date));
			        $data['consignee_list'][$k]['url']= wap_url("index","uc_address#add",array("id"=>$v['id'],"check"=>"check","tid"=>$tid,'vsid'=>$id,'rs_date'=>$rs_date));
		    	} else {
