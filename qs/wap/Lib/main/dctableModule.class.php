...
class dctableModule extends MainBaseModule
{
	...
	{	
		...
		$data = call_api_core("dctable","index",$param);
	...
	public function detail()
	{
		...
		$data = call_api_core('dctable', 'detail', $param);
		if (empty($data['location_info']) || $data['location_info']['is_reserve'] == 0) {
			app_redirect(wap_url('index', 'dctable'));
		}
		$data['location_info']['avg_point'] = round($data['location_info']['avg_point'],1);
		$data['location_info']['point_percent'] = (floatval($data['location_info']['avg_point']) / 5) * 100;
		$data['dp_list']['avg_point'] = round($data['dp_list']['avg_point'],1);
		$data['dp_list']['point_percent'] = ($data['dp_list']['avg_point'] / 5) * 100;
		// print_r($data);exit;
		$GLOBALS['tmpl']->assign('dclocation', $data['location_info']);
		$GLOBALS['tmpl']->assign('data', $data);
		$GLOBALS['tmpl']->display('dc/dc_table.html');
	}
	...
	public function index()
	{
        ...
        $data = call_api_core("dc","table_index",$param);
        ...
        $data['page_title'] = '预约订座';
        $GLOBALS['tmpl']->assign('s_info',$s_info);
        $GLOBALS['tmpl']->assign("data",$data);
        
        $GLOBALS['tmpl']->display("dc/dc_table_index.html");
	...
	public function lists()
	{
		...
		if(isset($GLOBALS['geo']['address'])){
            ...
            $param['qid'] = intval($_REQUEST['qid']); // 商圈id
            
            $data = call_api_core("dctable","lists",$param);
            ...
            $GLOBALS['tmpl']->assign("data",$data);
            $GLOBALS['tmpl']->display("dc/dc_table_list.html");
		}
		else
		{
		    app_redirect(wap_url('index','dcposition'));
