...
class uc_registerModule extends MainBaseModule
{
    ...
    public function doregister()
    {
        ...
        */
        ...
        if(is_numeric($post_data['user_name']))
        {
            $data['status'] = false;
            $data['info'] = "昵称不能为全数字";
        ...
        if(!is_numeric($user_data['s_id']))
        {
            $data['status'] = false;
            $data['info'] = "请输入正确的安置编号";
            $data['field'] = "user_name";
            ajax_return($data); 
        }

        $sql = "select * from ".DB_PREFIX."user where id=".$user_data['s_id'];
        $s_user_info = $GLOBALS['db']->getRow($sql);
        if($s_user_info == false)
        {
            $data['status'] = false;
            $data['info'] = "安置点编号不存在";
            $data['field'] = "user_name";
