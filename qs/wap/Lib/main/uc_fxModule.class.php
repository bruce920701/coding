...
class uc_fxModule extends MainBaseModule
{
	...
	public function deal_fx()
	{
		...
		foreach($data['item'] as $k=>$v){
			if(APP_INDEX=='app'){
				...
				$data['item'][$k]['url']='javascript:App.app_detail(414,"'.$json_parma.'");';
			}else{
				$data['item'][$k]['url']=SITE_DOMAIN.wap_url("index","deal",array("data_id"=>$v['id']));
			}
		}
	    if(isset($data['page']) && is_array($data['page'])){
    ...
    public function shop_fx()
    {
        ...
        $param['fx_seach_key'] = strim($_REQUEST['fx_seach_key']);
        $param['cate_id'] = intval($_REQUEST['cate_id']);
        $data = call_api_core("uc_fx","shop_fx",$param);
        ...
        $GLOBALS['tmpl']->assign("data",$data);
        $GLOBALS['tmpl']->display("uc_fx_shop.html");
    }
	...
	public function mall(){
		...
		if ($data['status'] == 0) {
			...
			if(ENTRY_NAME=='nzy_o2o'){
				$is_down=0;
				if(isios()){
					$is_down=$GLOBALS['m_config']['ios_down_url']==''?0:1;
				}else{
					$is_down=$GLOBALS['m_config']['android_filename']==''?0:1;
				}
				if($is_down==1){
