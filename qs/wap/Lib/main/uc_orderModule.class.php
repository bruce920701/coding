...
class uc_orderModule extends MainBaseModule
{
	...
	public function index()
	{
		...
		$param['is_user_center']=intval($_REQUEST['is_user_center']);
		if (isset($_REQUEST['tuan']) && intval($_REQUEST['tuan']) == 1) {
			$param['tuan'] = 1;
		}
		if ($param['pay_status']==9){
			if($param['is_user_center']==1){
				if(APP_INDEX=='app'){
...
// 		print_r($data);exit;
		foreach ($data['item'] as $k=>$v){
			$data['item'][$k]['format_total_price']=$v['total_price'];
			if(APP_INDEX=='app'){
			...
			foreach ($v['deal_order_item'] as $kk=>$vv){
				if($vv['count']==1){
					foreach ($vv['list'] as $kkk=>$vvv){
						$data['item'][$k]['deal_order_item'][$kk]['list'][$kkk]['unit_price'] = $vvv['unit_price'];
						$data['item'][$k]['deal_order_item'][$kk]['list'][$kkk]['discount_unit_price'] = $vvv['discount_unit_price'];
						if($vvv['buy_type']==1){
							$data['item'][$k]['deal_order_item'][$kk]['list'][$kkk]['return_score'] = $vvv['return_score'];
						}
						if($v['type']==8){
							require_once(APP_ROOT_PATH."system/model/deal.php");
							$data['item'][$k]['deal_order_item'][$kk]['list'][$kkk]['name'] = format_deal_name($vvv['name']);
						}
					}
				}
...
// 		print_r($data);exit;
		...
		else
		{
			...
			$arr['tuan']=$data['tuan'];
			$data['jump'] = wap_url('index', 'uc_order',$arr);
			ajax_return($data);
		}		
	}
	...
	public function view()
	{
	    ...
	    $data = call_api_core("uc_order","wap_view",array("data_id"=>$data_id,'is_done'=>$is_done));
	    // echo "<pre>";print_r($data);exit;
	    if (!$data['item']) {
	    	// 订单不存在或被删除时
	    	...
	    	$data['item']['format_total_price']=$data['item']['total_pay_price'];
	    	$data['item']['format_pay_amount']=$data['item']['pay_amount'];
    	       ...
    	       || substr($url,$tp,16)=="ctl=cart&act=pay")
	    	{
	    	    ...
	    	    if($data['buy_type']==1){
	    	        ...
	    	        $GLOBALS['tmpl']->assign("back_url",wap_url("index","uc_order",array('pay_status'=>9,'is_user_center'=>$is_user_center)));
	    	    }
	    	}
	    	...
	    	foreach ($data['item']['deal_order_item'] as $kk=>$vv){
	    		foreach ($vv['list'] as $kkk=>$vvv){
	    			$data['item']['deal_order_item'][$kk]['list'][$kkk]['unit_price'] = $vvv['unit_price'];
	    			$data['item']['deal_order_item'][$kk]['list'][$kkk]['discount_unit_price'] = $vvv['discount_unit_price'];
	    			if($vvv['buy_type']==1){
	    				$data['item']['deal_order_item'][$kk]['list'][$kkk]['return_score'] = $vvv['return_score'];
	    			}
					if($data['item']['is_presell_order']==1){
						$data['item']['deal_order_item'][$kk]['list'][$kkk]['name'] = format_deal_name($vvv['name']);
					}
	    		}
	    	}
		    $GLOBALS['tmpl']->assign("data",$data);
	...
	public function order_refund()
	{
	    ...
	    //echo "<pre>";print_r($data);exit;
	    $data['item']['format_total_price']=$data['item']['total_price'];
	    $data['item']['format_pay_amount']=$data['item']['pay_amount'];
	    //echo "<pre>";print_r($data);exit;
	    foreach ($data['item']['deal_order_item'] as $kk=>$vv){
	        if($vv['count']==1){
