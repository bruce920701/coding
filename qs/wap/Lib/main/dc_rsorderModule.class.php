...
class dc_rsorderModule extends MainBaseModule
{
	...
	public function index()
	{
		...
		$data = call_api_core("dc_rsorder","index",$param);
		init_app_page();
		if($data['user_login_status']==1)
		{
			...
			if($is_ajax==1){
			    ...
			    $ajaxobj['html'] = $GLOBALS['tmpl']->fetch("dc/inc/rs_order.html");
			    ajax_return($ajaxobj);
			}else{
			    $GLOBALS['tmpl']->assign("data",$data);
                if(APP_INDEX=='app')
	...
	public function view()
	{
		...
		$data = call_api_core("dc_rsorder","view",$param);
		...
		if($data['user_login_status']==1)
		{
			if($data['is_order_exists']==1){
				...
				$GLOBALS['tmpl']->assign("data",$data);
				$GLOBALS['tmpl']->assign("back_url",wap_url("index","dc_rsorder"));
	...
	public function rend_coupon_sms()
	{
		...
		$param['id']=intval($_REQUEST['id']);
		$data = call_api_core("dc_rsorder","rend_coupon_sms",$param);
	...
	public function cancel()
	{
		...
		$param['id']=intval($_REQUEST['id']);
		$data = call_api_core("dc_rsorder","cancel",$param);
	...
	public function do_refund()
	{
		...
		$data = call_api_core("dc_rsorder","do_refund",$param);
