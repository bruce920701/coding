...
class MainBaseModule{
	...
	{		
		...
		if(
    		    ...
    		    MODULE_NAME=="shop"&&ACTION_NAME=="index"||
    		    MODULE_NAME=="main"&&ACTION_NAME=="index"||
    ...
    protected  function success($info,$data=array(),$jump=""){
        ajax_return(array("status"=>1,"info"=>$info,"data"=>$data,"jump"=>$jump));
    }
    protected  function error($info,$data=array(),$jump=""){
        $is_ajax=$_REQUEST['is_ajax'];
        if(intval($is_ajax)){
            ajax_return(array("status"=>0,"info"=>$info,"data"=>$data,"jump"=>$jump));
        }else{
            showErr($info);
        }
    }
    protected function assign($key,$value=""){
        if($value){
            $GLOBALS['tmpl']->assign($key,$value);
        }else{
            $GLOBALS['tmpl']->assign($key);
        }

    }
    protected function display($path){
        $GLOBALS['tmpl']->display($path);
    }
	 ...
	 * @return null        
	 */
	protected function _pageFormat($pages)
	{
		if(!empty($pages) && is_array($pages)){
			$page = new Page($pages['data_total'],$pages['page_size']);   //初始化分页对象
			$p  =  $page->show();
	...
	/**
	 * 检查是否授权开启分销功能, 无则跳转至会员中心
	 * @return  
	 */
	protected function fxCheck()
	{
		if (!defined('FX_LEVEL')) {
			if (stripos(MODULE_NAME, 'uc_fx') !== false) {
