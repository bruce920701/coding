	...
	if(empty($user_info))
	{
				...
				{		
					...
					else
					{
						...
						if($wx_info&&$wx_info['wx_openid']&&$wx_info['openid'])
						{
							...
							{
								auto_do_login_user($platform_user_info['user_name'],$platform_user_info['user_pwd'],false,true);
								$user_info = es_session::get('user_info');
							}
			...
			{		
				...
				if(WEIXIN_TYPE=="platform")
				{
					...
					}else
					{
						...
						if($weixin_login==0){
							//进入微信端自动授权登录，授权登录后返回的链接
							$params['from'] = '//'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'];
							//进入微信端自动授权登录,未绑定手机前往绑定手机页，绑定成功后返回的链接
							es_session::set("bind_mobile_return_url",$params['from']);
						}
		...
		else
		{
			...
			if($cookie_uname!=''&&$cookie_upwd!=''&&!es_session::get("user_info"))
			{
				...
				auto_do_login_user($cookie_uname,$cookie_upwd,true,true);
				$user_info = es_session::get('user_info');
			}
		}		
	}
...
//		}
	...
	if(!(($_REQUEST['ctl']=="uc_account"&&($_REQUEST['act']=='phone'||$_REQUEST['act']=='bindPhone'))||($_REQUEST['ctl']=='ajax'||$_REQUEST['act']=='send_sms_code'))&&es_session::get("bind_mobile_return_url")){
		es_session::delete("bind_mobile_return_url");
	}
	//关于微信绑定
        ...
        if($is_weixin) {
			...
			if($weixin_conf['platform_status']==1&&WEIXIN_TYPE!="platform"){
				...
				if($wx_code&&$wx_status){
					...
					$weixin=new weixin($weixin_account['authorizer_appid'],"",SITE_DOMAIN.$url);
					$weixin_user_info = $weixin->scope_get_userinfo_platform($wx_code); //通过授权获取到用户信息
					if($weixin_user_info&&$weixin_user_info['openid']){
						es_session::set("user_wx_info", $weixin_user_info);
					}
				}else
				{
					//跳转至微信的授权页
...
function init_app_page()
{
    ...
    $GLOBALS['tmpl']->assign("IS_PRESELL",IS_PRESELL);
...
function wx_info_login($wx_info,$type=0)
{
	...
	if($wx_user_info){
		...
		//如果会员存在，直接登录
		auto_do_login_user($wx_user_info['user_name'],$wx_user_info['user_pwd'],false,true);
	}else{
		//会员不存在进入自动创建流程
		$user_data = array();
		...
		auto_do_login_user($user_data['user_name'],$user_data['user_pwd'],false,true);
	}
	global $user_info;
	$user_info = es_session::get('user_info');
	if($user_info['mobile']==''){
	...
	//能跳到这，说明这个账号已经绑定过手机，删除此链接
	if(es_session::get("bind_mobile_return_url")){
		es_session::delete("bind_mobile_return_url");
	}
	return true;
}
...
function wx_info_bind($wx_info,$type=0)
{
    ...
    if($wx_user_info){
        ...
        if($wx_user_info['mobile']==''){
			bind_wx_user_info($GLOBALS['user_info'],$wx_user_info,$wx_info,0);
        }else{
...
function dist_global_run () {
    ...
    if(empty($dist_info))
    {
        ...
        if($cookie_dist_uname!=''&&$cookie_dist_upwd!=''&&!es_session::get("dist_info"))
        {
            ...
            auto_do_login_dist($cookie_dist_uname, $cookie_dist_upwd);
