 ...
 * @desc      
 * @author    吴庆祥
 * @since     2017-08-16 11:18  
 */
class biz_presell_orderModule extends MainBaseModule
{
    ...
    public function index()
    {
        ...
        $data = call_api_core("biz_presell_order","index",array("page"=>$page,'type'=>$type));
        ...
        if($data['status']==0){
            ...
            $GLOBALS['tmpl']->display("biz_presell_order.html");
        }
    ...
    public function view()
    {
        ...
        $data = call_api_core("biz_presell_order","view",array("data_id"=>$data_id));
        if($data['biz_user_status']!=LOGIN_STATUS_LOGINED){
            app_redirect(wap_url("biz","user#login"));
        }
    ...
    public function logistics()
    {
        global_run();
        $data_id = intval($_REQUEST['data_id']);
        $data = call_api_core("biz_presell_order","logistics",array("data_id"=>$data_id));
        ...
        $GLOBALS['tmpl']->assign("data",$data);
        $GLOBALS['tmpl']->display("biz_presell_order_logistics.html");
    ...
    public function delivery(){
        ...
        $data = call_api_core("biz_presell_order","delivery",$param);
        ...
        $data['page_type'] = "o";
        $GLOBALS['tmpl']->assign("data",$data);
        $GLOBALS['tmpl']->assign("ajax_url",wap_url("biz","biz_presell_order#do_delibery",$param));
        $GLOBALS['tmpl']->display("biz_presell_order_delivery.html");
    }
    ...
    public function do_delivery(){
        ...
        $data = call_api_core("biz_presell_order","do_delivery",$param);
        if ($data['is_auth']==0){ //没有操作权限
            $data['jump']= wap_url("biz","shop_verify");
        }

        if ($data['status'] == 1){
            $data['jump']= wap_url("biz","presell_order#view",array('data_id'=>$data['order_id']));
        }
