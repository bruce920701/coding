...
INSERT INTO `%DB_PREFIX%conf` VALUES ('6','DB_VERSION','6.8','0','0','','1','0','0');
...
INSERT INTO `%DB_PREFIX%delivery_region` VALUES ('20', '1', '宁夏', '2', '640000');
...
INSERT INTO `%DB_PREFIX%delivery_region` VALUES ('29', '1', '新疆', '2', '650000');
INSERT INTO `%DB_PREFIX%delivery_region` VALUES ('30', '1', '云南', '2', '530000');
INSERT INTO `%DB_PREFIX%delivery_region` VALUES ('31', '1', '浙江', '2', '330000');
...
PRIMARY KEY (`deal_id`),
KEY `deal_id` (`deal_id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='产品库存事务表';
...
`id` int(11) NOT NULL auto_increment,
`sn` varchar(255) NOT NULL COMMENT ' 预订电子券的序列号',
`begin_time` int(11) NOT NULL COMMENT ' 预订电子券生效时间',
`end_time` int(11) NOT NULL COMMENT ' 预订电子券过期时间',
...
PRIMARY KEY  (`id`),
UNIQUE KEY `sn` (`sn`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='商家预订电子券表';
...
`ecv_money` decimal(20,4) NOT NULL COMMENT '代金券',
...
`id` int(11) NOT NULL auto_increment,
`create_time` int(11) NOT NULL,
`type` tinyint(1) NOT NULL COMMENT '0.营业额 1.结算额 2.在线支付额 3.活动补贴 4.代金券 5.退款,取消订单金额 6.佣金',
`money` decimal(20,4) NOT NULL,
`log_info` text NOT NULL,
PRIMARY KEY  (`id`)
...
INSERT INTO `%DB_PREFIX%conf` VALUES ('','ADMIN_FEE_RATE','10','7','0','','1','0','1');
...
ALTER TABLE `%DB_PREFIX%deal_order`
ADD COLUMN `youhui_log_id`  int(11) NOT NULL DEFAULT 0 COMMENT '优惠券ID',
ADD COLUMN `youhui_money`  decimal(20,4) NOT NULL DEFAULT 0 COMMENT '优惠券优惠的金额';
...
INSERT INTO `%DB_PREFIX%m_config` VALUES (null, 'is_ios_upgrading', 'ios是否在审核中', '0', '0', '手机端升级设置', '4');
ALTER TABLE `%DB_PREFIX%user` CHANGE COLUMN `user_name` `user_name` varchar(255) DEFAULT NULL COMMENT '会员名（支持中文、字母、数字、“_” 的组合，4-20个字符 ，且不能为纯数字）', CHANGE COLUMN `user_pwd` `user_pwd` varchar(255) NOT NULL COMMENT '会员密码（长度只能在6-20个字符之间）';

ALTER TABLE `%DB_PREFIX%user`
ADD COLUMN `path`  varchar(255) NOT NULL DEFAULT 0 COMMENT '用户三级关系';

ALTER TABLE `%DB_PREFIX%fx_qualification`
ADD COLUMN `phone_pay_agreement` text COMMENT '手机购买协议';

ALTER TABLE `%DB_PREFIX%fx_statements`
ADD COLUMN `recommend_money`  decimal(20,4) NOT NULL COMMENT '推荐商品返佣的累计佣金';

ALTER TABLE `%DB_PREFIX%deal_submit`
ADD COLUMN `coupon_time_type` tinyint(1) NOT NULL COMMENT '0：指定时间过期 1:按下单日起xx天过期',
ADD COLUMN `coupon_day` int(11) NOT NULL COMMENT '下单后xx天内失效';

ALTER TABLE `%DB_PREFIX%m_zt`
ADD COLUMN `z_type`  tinyint(1) NOT NULL DEFAULT 0 COMMENT '专题类型,0:固定模版专题，1.动态专题';

ALTER TABLE `%DB_PREFIX%m_zt`
ADD COLUMN `zt_html`  text NULL COMMENT '动态专题模版';

ALTER TABLE `%DB_PREFIX%m_index`
MODIFY COLUMN `type`  int(11) NULL DEFAULT 0 COMMENT 'system/mobile/mobile_cfg/main/webnav_cfg';

ALTER TABLE `%DB_PREFIX%deal`
ADD COLUMN `presell_begin_time`  int(11) NOT NULL COMMENT '预售开始时间',
ADD COLUMN `presell_end_time`  int(11) NOT NULL COMMENT '预售结束时间',
ADD COLUMN `presell_data`  text NOT NULL COMMENT '预售商品数据 序列化格式';

ALTER TABLE `%DB_PREFIX%deal`
ADD COLUMN `is_presell`  tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否为预售' ;
CREATE TABLE `%DB_PREFIX%deal_presell` (
`id` int(11) NOT NULL AUTO_INCREMENT,
`type` tinyint(1) NOT NULL DEFAULT '0' COMMENT '0.订金1.定金',
`deposit_money` decimal(20,4) NOT NULL DEFAULT '0.0000' COMMENT '预付定金或订金',
`discount_money` decimal(20,4) NOT NULL DEFAULT '0.0000' COMMENT '折扣金额',
...
ALTER TABLE `%DB_PREFIX%deal_order`
MODIFY COLUMN `type`  tinyint(1) NOT NULL COMMENT '订单类型(0:商品订单 1:用户充值单,2:积分兑换订单，3:平台自营物流配送订单，4:平台自营驿站配送订单,5:商家团购订单,6:商家商品订单,7:积分充值订单,8.预售的定金订单，9.预售的订金订单，10.预售的尾款订单)';

ALTER TABLE `%DB_PREFIX%deal_presell`
ADD UNIQUE INDEX `unique_deal_id` (`deal_id`) USING BTREE ;


ALTER TABLE `%DB_PREFIX%deal`
ADD COLUMN `presell_deposit_money`  decimal(20,4) NOT NULL DEFAULT 0 COMMENT '预售订金' ,
ADD COLUMN `presell_discount_money`  decimal(20,4) NOT NULL DEFAULT 0 COMMENT '抵扣金额' ,
ADD COLUMN `presell_type`  tinyint(1) NOT NULL DEFAULT 0 COMMENT '0.订金1.定金' ,
ADD COLUMN `presell_delivery_time`  int(11) NOT NULL DEFAULT 0 COMMENT '预售配送时间' ;

ALTER TABLE `%DB_PREFIX%attr_stock`
ADD COLUMN `presell_deposit_money`  decimal(20,4) NOT NULL DEFAULT 0 COMMENT '订金' ,
ADD COLUMN `presell_discount_money`  decimal(20,4) NOT NULL DEFAULT 0 COMMENT '订金抵扣金额';

ALTER TABLE `%DB_PREFIX%deal`
ADD COLUMN `presell_buy_count`  int(11) NOT NULL DEFAULT 0 COMMENT '预售销量';
ALTER TABLE `%DB_PREFIX%attr_stock`
ADD COLUMN `presell_buy_count`  int(11) NOT NULL DEFAULT 0 COMMENT '预售销量';

INSERT INTO `%DB_PREFIX%conf` VALUES ('','MNOTICE_TITLE','知奥学院',3,0,'',1,1,1498806074);

ALTER TABLE `%DB_PREFIX%event_biz_submit`
ADD COLUMN `img` varchar(255) NOT NULL COMMENT 'wap活动列表展示图';

UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='INVITE_REFERRALS');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='INVITE_REFERRALS_TYPE');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='REFERRAL_LIMIT');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='REFERRALS_DELAY');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='REFERRAL_IP_LIMIT');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_REGISTER_MONEY');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_REGISTER_SCORE');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_REGISTER_POINT');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_LOGIN_MONEY');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_LOGIN_SCORE');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_LOGIN_POINT');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_LOGIN_KEEP_MONEY');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_LOGIN_KEEP_SCORE');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_LOGIN_KEEP_POINT');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_ACTIVE_MONEY');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_ACTIVE_SCORE');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_ACTIVE_POINT');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_ACTIVE_MONEY_MAX');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_ACTIVE_SCORE_MAX');
UPDATE `%DB_PREFIX%conf` SET `is_conf`='0' WHERE (`name`='USER_ACTIVE_POINT_MAX');

CREATE TABLE `%DB_PREFIX%verification_code` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `iconfont` varchar(255) NOT NULL COMMENT 'iconfont图标字符串',
  `iconname` varchar(255) NOT NULL COMMENT 'iconfont图标名字',
  `iconcode` varchar(255) NOT NULL COMMENT 'iconfont图标初始值',
  UNIQUE KEY `id` (`id`) USING BTREE,
  UNIQUE KEY `iconfont` (`iconfont`) USING BTREE
) ENGINE=MyISAM AUTO_INCREMENT=12 DEFAULT CHARSET=utf8;


INSERT INTO `%DB_PREFIX%conf` ( `name`, `value`, `group_id`, `input_type`, `value_scope`, `is_effect`, `is_conf`, `sort`) VALUES ( 'ICON_VERIFICATION_CODE', '1', '1', '1', '0,1', '1', '1', '0');

ALTER TABLE `%DB_PREFIX%m_adv`
MODIFY COLUMN `type`  int(11) NOT NULL DEFAULT 0 COMMENT '1分类标签广告\r\n2URL广告\r\n3分类排行\r\n4达人页\r\n5搜索页\r\n6拍照\r\n7热门\r\n8分享详细\r\n9团购列表\r\n10商品列表\r\n11活动列表\r\n12优惠列表\r\n13代金券列表\r\n14团购明细\r\n15商品明细\r\n16活动明细\r\n17优惠明细\r\n18代金券明细\r\n19关于我们\r\n20优惠券主页面\r\n21公告列表';

INSERT INTO `%DB_PREFIX%conf` ( `name`, `value`, `group_id`, `input_type`, `value_scope`, `is_effect`, `is_conf`, `sort` ) VALUES ( 'PRESELL_SEND_MESSAGE', '0', '5', '1', '0,1', '1', '1', '73' );

ALTER TABLE `%DB_PREFIX%deal_order`
ADD COLUMN `presell_deposit_money`  decimal(20,4) NOT NULL DEFAULT 0 COMMENT '预售订金' ,
ADD COLUMN `presell_discount_money`  decimal(20,4) NOT NULL DEFAULT 0 COMMENT '预售抵扣金额' ,
ADD COLUMN `presell_left_price`  decimal(20,4) NOT NULL DEFAULT 0 COMMENT '预售尾款' ,
ADD COLUMN `presell_type`  tinyint(1) NOT NULL DEFAULT 0 COMMENT '0.订金1.定金' ;
...
ALTER TABLE `%DB_PREFIX%deal_order`
ADD COLUMN `is_presell_order`  tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否为预售订单';

ALTER TABLE `%DB_PREFIX%deal_order`
MODIFY COLUMN `type`  tinyint(1) NOT NULL COMMENT '订单类型(0:商品订单 1:用户充值单,2:积分兑换订单，3:平台自营物流配送订单，4:平台自营驿站配送订单,5:商家团购订单,6:商家商品订单,7:积分充值订单)';

INSERT INTO `%DB_PREFIX%m_config` VALUES ('', 'android_dist_master_secret', '驿站android推送友盟AppMasterSecret', '', '0', '手机推送配置', '27');
INSERT INTO `%DB_PREFIX%m_config` VALUES ('', 'android_dist_app_key', '驿站android推送友盟AppKey', '', '0', '手机推送配置', '28');
INSERT INTO `%DB_PREFIX%m_config` VALUES ('', 'ios_dist_app_key', '驿站ios推送友盟AppKey', '', '0', '手机推送配置', '29');
INSERT INTO `%DB_PREFIX%m_config` VALUES ('', 'ios_dist_master_secret', '驿站ios推送友盟AppMasterSecret', '', '0', '手机推送配置', '30');

ALTER TABLE `%DB_PREFIX%distribution`
ADD COLUMN `dev_type` varchar(20) DEFAULT 'android' COMMENT 'android,ios 客户手机类型,一个客户只绑定一个最新的手机',
ADD COLUMN `device_token` varchar(255) DEFAULT NULL COMMENT '推送device_token一个客户只绑定一个最新的手机';

CREATE TABLE `%DB_PREFIX%distribution_order_fee` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键',
  `order_id` int(11) NOT NULL COMMENT '所属的订单ID',
  `distribution_id` int(11) NOT NULL DEFAULT '0' COMMENT '分配的配送驿站id',
  `create_time` int(11) NOT NULL COMMENT '发货时间',
  `total_price` decimal(20,4) NOT NULL COMMENT '订单金额',
  `distribution_fee` decimal(20,4) NOT NULL COMMENT '驿站服务费',
  `is_settlement` int(11) NOT NULL COMMENT '0:未结算，1：已结算',
  `payment_class_name` varchar(255) NOT NULL COMMENT '支付接口类名',
  `order_sn` varchar(255) NOT NULL COMMENT '订单编号',
  PRIMARY KEY (`id`),
  UNIQUE KEY `un_order_id` (`order_id`) USING BTREE
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='驿站服务费结算表';

CREATE TABLE `%DB_PREFIX%user_youhui_statistics` (
	`id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键',
	`youhui_id` int(11) NOT NULL DEFAULT '0' COMMENT '优惠券id',
	`user_id` int(11) NOT NULL COMMENT '会员ID',
	`stat_time` date NOT NULL COMMENT '报表日期',
	`user_count` int(11) NOT NULL COMMENT '会员领取数量',
	`user_day_count` int(11) NOT NULL COMMENT '会员当日领取数量',
	PRIMARY KEY (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='会员领取优惠券统计表';

INSERT INTO `%DB_PREFIX%conf` VALUES ('','IS_DIST_COD_BALANCE','0',8,1,'0,1',1,1,1);

ALTER TABLE `%DB_PREFIX%deal_order`
ADD COLUMN `xpoint` varchar(255) DEFAULT NULL COMMENT '收货地址经度',
ADD COLUMN `ypoint` varchar(255) DEFAULT NULL COMMENT '收货地址纬度';

ALTER TABLE `%DB_PREFIX%withdraw`
ADD COLUMN `is_chargeback` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否已扣款';
update %DB_PREFIX%withdraw set is_chargeback=1 where is_paid=1;

INSERT INTO `%DB_PREFIX%m_config` VALUES ('', 'entry_name_cssh_shop_title', 'wap下载页与门店入驻站点标题', '方维o2o', '0', '基础配置', '27');


update `%DB_PREFIX%deal_order` set is_presell_order=1 where type=8;