...
class IndexAction extends Action{
    ...
    public function install()
    {
    	...
    	$db_config['DB_CHARACTER'] = 'utf8'; // 手动指定
    	...
    	if(mysql_error()=="")
    	{
    		/*$charsetsupport = mysql_query('SHOW CHARSET LIKE "%utf8mb4"');
    		if (!mysql_fetch_array($charsetsupport)) {
    			$return_rs['msg'] = '只支持MySQL 5.5以上的版本';
   ...
   /**
    ...
    private function restore($file,$db_config)
    {
    		...
    		$sql = str_replace("\r", '', $sql);
       		...
       		$file_temp = $this->getRealPath()."/install/install_temp.sql";     		
       		$sql_temp = file_get_contents($file_temp);
       		$sql_temp = str_replace("%DB_PREFIX%",$db_config['DB_PREFIX'],$sql_temp);
       		$db->query($sql_temp);
