        ...
        switch ($status_str) {
            case '待发货':
                ...
                if($order_info['type']==8){
                    $delivery_time=$GLOBALS['db']->getOne("select presell_delivery_time from ".DB_PREFIX."deal where id={$item_info['deal_id']}");
                    if(NOW_TIME<$delivery_time)break;
                }
                $str .= "<a href=\"{$url}\"  >发货</a>";
                break;
...
function get_order_status($s,$order_info)
{
    if($order_info['is_presell_order']){
        return get_presell_order_status($s,$order_info);
    }else{
        // 交易关闭 必须是为结单的
...
function get_presell_order_status($order_status,$order_info=array())
{
    $deal_order_item=unserialize( $order_info['deal_order_item']);
    $deal_order_item=$deal_order_item[0];
    if($order_info['refund_status']==1){
        return "退款中";
    }elseif($order_info['is_delete'] && $order_info['pay_status'] != 2){
        return "交易关闭";
    }elseif($order_info['refund_status']==2){
        return "已退款";
    }elseif($order_info['pay_status']==0){
        return "待付".L("DEPOSIT_MONEY_{$order_info['presell_type']}");
    }elseif($order_info['pay_status']==1){
        return "待付尾款";
    }elseif ($deal_order_item['delivery_status'] == 0) {
        return "待发货";
    } elseif ($deal_order_item['delivery_status'] == 1 && $deal_order_item['is_arrival'] == 0) {
        return "待收货";
    } elseif ($deal_order_item['delivery_status'] == 1 && $deal_order_item['is_arrival'] == 1 && $deal_order_item['dp_id'] == 0) {
        return "待评价";
...
function get_submit_user($uid)
{
		...
		{
			$supplier_id = M("SupplierAccount")->where("id=".$uid)->getField("user_id");
			$uname = M("User")->where("id=".$supplier_id)->getField("user_name");
...
function get_pay_amount($pay_amount, $deal_order)
{
    if($deal_order['is_presell_order']){
        $pay_amount  =$deal_order['total_price']-($deal_order['presell_discount_money']-$deal_order['presell_deposit_money']) - $deal_order['ecv_money'] -  $deal_order['youhui_money'];
    }else{
        $pay_amount  =$deal_order['total_price'] - $deal_order['ecv_money'] -  $deal_order['youhui_money'];
    }

    return format_price($pay_amount);
    	
}

function get_mobile_cfg($nav_cfg,$type){
    foreach($nav_cfg as $k=>$v)
	{
		if($type==2||$type==3){
			unset($nav_cfg[$k]['nav']['uc_order']);
			unset($nav_cfg[$k]['nav']['uc_fx']);
		}
	}
	return $nav_cfg;
}
