...
class FxUserAction extends CommonAction{
	...
	public function deal_index(){
		...
		$list = $GLOBALS['db']->getAll("select d.name,ud.* from ".DB_PREFIX."user_deal as ud left join ".DB_PREFIX."deal as d on ud.deal_id = d.id where ud.user_id = ".$user_id." and d.is_effect = 1 and d.is_delete = 0 order by d.sort asc,d.id asc limit ".$limit);		
	    $user_name=M("User")->where("id=".$user_id)->getField("user_name");
		
		$p = new Page ( $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user_deal as ud left join ".DB_PREFIX."deal as d on ud.deal_id = d.id where ud.user_id = ".$user_id." and d.is_effect = 1 and d.is_delete = 0 "), '' );
		$page = $p->show ();
...
//	    }    
	...
	public function supplier_index(){
		...
		$result=get_supplier($user_id,$limit);
		//echo "<pre>";print_r($result);exit;
		$p = new Page ( $result['count'], '' );
		$page = $p->show ();
		...
		$this->assign ( "nowPage",$p->nowPage);
		$this->assign("user_id",$user_id);
		$this->assign("title_name",$user_name."的下线商家");
