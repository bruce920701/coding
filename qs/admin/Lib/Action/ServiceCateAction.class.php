...
class ServiceCateAction extends CommonAction{
	...
	public function delete() {
		...
		if (isset ( $id )) {
				...
				if(M("ServiceCate")->where(array ('pid' => array ('in', explode ( ',', $id ) )))->count()>0)
				{
					$this->error ("该分类存在子分类，请先删除其子分类",$ajax);
				}
				if(M("CommonService")->where(array ('cate_id' => array ('in', explode ( ',', $id ) )))->count()>0)
				{
					$this->error ("存在商品使用该分类，不能删除",$ajax);
				}
