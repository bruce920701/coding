...
class SupplierAction extends CommonAction{
	public function index()
	{
	    ...
	    // 筛选开启了服务的门店的商家
	    if ($this->is_service_list || intval($_REQUEST['is_service_list']) == 1) {
	    	$locWhere = array(
				'is_effect' => 1,
				'is_close' => 0,
				'is_visiting_service' => 1
			);
			$locSupids = M('SupplierLocation')->where($locWhere)->field('supplier_id')->findAll();
			$supids = array();
			foreach ($locSupids as $loc) {
				$supids[$loc['supplier_id']] = $loc['supplier_id'];
			}
			
			$ex_condition .= ' and id in ('.implode(',', $supids).')';
	    }
	...
	public function edit() {		
		...
		$promote_where = 'supplier_id = ' .$id.' and (class_name="Appdiscount" or class_name="Discountamount")';
		...
		if(defined("FX_LEVEL")&&intval($ref_salary_conf['ref_salary_switch'])){
			$this->assign ( 'is_open_fx', 1 );
		}else{
			$this->assign ( 'is_open_fx', 0 );
		}
		$this->display ();
	}
	...
	public function update() {
		...
		if(floatval($data['publish_verify_balance'])<0.01 || floatval($data['publish_verify_balance'])>100){
		    $this->error("商户结算费率：0.01 - 100");
		}
		
		if($data['is_store_payment'] == 1 && ( floatval($data['store_payment_rate']) < 0.01 ||  floatval($data['store_payment_rate']) > 100 )){
		    $this->error("买单费率：0.01 - 100");
		}
		if ($data['is_store_payment'] == 1 && empty($_REQUEST['location_id'])) {
			$this->error('开启到店支付需要选择支持的门店');
		}
		...
		if (defined('OPEN_XN_TALK') && OPEN_XN_TALK) {
			...
			}elseif($data['open_xn_talk']==2){
                $data['xn_talk_id'] = '';
                $data['xn_talk_login_id'] = '';
                $data['xn_talk_pwd'] = '';
                $data['xn_talk_custom_id'] = '';
                $data['qm_talk_id'] = strim($_REQUEST['qm_talk_id']);
                if(empty($data['qm_talk_id'])){
                    $this->error('七陌accessid不能为空');
                }
            }
		}
	...
	public function docharge()
	{
		...
		if($charge_id>0){
				...
				$result=get_supplier_withdraw_money($supplier_info['supplier_withdraw_cycle'],$supplier_info['id']);
	...
	public function location_service_list()
	{
		$supplier_id = intval($_REQUEST['supplier_id']);
		
		$supplier_location_list = $GLOBALS['db']->getAll("select id,name from ".DB_PREFIX."supplier_location where supplier_id = ".$supplier_id." and is_effect=1 and is_visiting_service=1");
		
		$supplier_location_arr = array(); // 已选门店
		if (intval($_REQUEST['svsid'])) {
			$locations = M('SupplierLocationServiceLink')->field('location_id')->where(array('supplier_vs_id'=>intval($_REQUEST['svsid']), 'is_close'=>0))->findAll();
			if ($locations) {
				foreach ($locations as $location) {
					$supplier_location_arr[] = $location['location_id'];
				}
			}
		}

		M('SupplierLocation')->field('id')->where($where)->findAll();
		...
		$this->ajaxReturn($result['html']);
	}

	private $is_service_list = false;
	public function service_list()
	{
		$this->is_service_list = true;
		$this->assign('main_title', L('SUPPLIER_SERVICE_LIST'));
		$this->assign('is_service_list', 1);
		$this->index();
	}
}
?>