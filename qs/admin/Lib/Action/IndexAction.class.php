...
class IndexAction extends AuthAction{
	...
	public function top()
	{
        ...
        {
            $config_file = APP_ROOT_PATH."system/adm_cfg/".APP_TYPE."/serviceadmnav_cfg.php";
            $navs = array_merge_admnav($navs, $config_file);
        }
		*/
	...
	public function left()
	{
		...
		$navs = array_merge_admnav($navs, $config_file);
		if(!defined("FX_LEVEL")){
		    unset($navs['newmarketing']['groups']['fx_extension']);
		}
		if(!defined("FX_LEVEL")){
			unset($navs['marketing']['groups']['fx']);
			unset($navs['marketing']['groups']['fx_report']);
		}
        if(!defined("IS_PRESELL")||!IS_PRESELL){
            //预售订单屏蔽的是第五个数组，顺序改动请修改该位置
            unset($navs['order']['groups']['order']['nodes'][4]);
            unset($navs['order']['groups']['supplier_order']['nodes'][3]);
        }
        if(!defined("IS_PT")||!IS_PT){
            unset($navs['order']['groups']['order']['nodes'][5]);
            unset($navs['deal']['groups']['shopcate']['nodes'][7]);
            unset($navs['order']['groups']['supplier_order']['nodes'][4]);
        }
		if(defined("DC"))
		{
			$config_file = APP_ROOT_PATH."system/adm_cfg/".APP_TYPE."/dcadmnav_cfg.php";
