...
class DcOrderAction extends CommonAction{
	...
	{	
		...
		switch($order_info['order_status']){ //订单状态
			case 1:  //待接单
				$order_info['order_status'] = '待接单';
				break;
			case 2:  //待取货
				$order_info['order_status'] = '待取货';
				break;
			case 3:  //配送中
				$order_info['order_status'] = '配送中';
				break;
			case 4:  //已完成
			...
			case 5:  //已取消
				...
				break;
			case 7:  //已过期
				$order_info['order_status'] = '已过期';
				break;
			case 8:  //指派单
				$order_info['order_status'] = '指派单';
				break;

			default:
		}
		$order_info['cancel_reason'] = $dada_delivery['cancel_reason']; //订单取消原因,其他状态下默认值为空字符串
		$order_info['cancel_from'] = $dada_delivery['cancel_from']; //订单取消原因来源(1:达达配送员取消；2:商家主动取消；3:系统或客服取消；0:默认值)
		$order_info['dm_name'] = $dada_delivery['dm_name']; //配送员姓名，接单以后会传
		$order_info['dm_mobile'] = $dada_delivery['dm_mobile']; //配送员手机号，接单以后会传

		if($order_info['confirm_status']==0){
			$order_info['delivery_way']="未接单";
		}else{
			if($order_info['delivery_part'] == 1){
				$order_info['delivery_way'] = "商家配送";
			}else if($order_info['delivery_part'] == 2){
				if($order_info['dm_name']){
					$order_info['delivery_way'] = "达达配送（配送员<a style='color: #4AFFFF;text-decoration:underline;'> ".$order_info['dm_name']." ".$order_info['dm_mobile']."</a> ".$order_info['order_status']."）";
				}else{
					$order_info['delivery_way'] = "达达配送";
				}
			}else if($order_info['delivery_part'] == 3){
				$order_info['delivery_way'] = "蜂鸟配送";
			}
		}
		...
		else
		{
			...
			if($order_info['now']-$order_info['create_time'] > $time){
			    $order_info['out_time']=1;
			}else{
	...
	public function do_incharge()
	{
		...
		if($order_info['payment_id']==1)  //货到付款，不允许平台收款
		{
			$this->error(l("货到付款，不允许平台收款"));
		}
		...
		$dc_type=1;
		$consignee_info = M("DcConsignee")->where("user_id=".$order_info['user_id']." and is_main = 1")->find();
