...
class DealAction extends CommonAction{
	...
	private function _shopMap(){
        ...
        $map['is_delete'] = 0;
        $map['_string'] = "1=1 ";
            //自营商城0，自营积分1，商户商城商品2，商户团购商品3  精品区4
        $type=intval($_REQUEST['type']);
        if($type==0){
            $map['supplier_id']=0;
            $map['buy_type']=0;
...
// 		$this->assign("payment_list",$payment_list);
	...
	public function shop_update() {
		...
		if($data['buy_type']==0  || $data['buy_type'] ==2)
		{
			if(intval($data['return_score'])<0)
			{
				$this->error("积分返还不能为负数");
...
// 	    ajax_return($result);
    ...
    public function get_attr_row_html($attr_stock , $key,$supplier_id,$publish_verify_balance){
        ...
        $html .='<td><input  type="hidden" name="presell_discount_money[]" value="' . $attr_stock[$key]['presell_discount_money'] . '" /><input  type="hidden" name="presell_buy_count[]" value="' . $attr_stock[$key]['presell_buy_count'] . '" /><input  type="hidden" name="presell_deposit_money[]" value="' .$attr_stock[$key]['presell_deposit_money'] .'" /><input type="hidden" name="stock_buy_count[]" value="'.$attr_stock[$key]['buy_count'].'" />'.intval($attr_stock[$key]['buy_count']).'</td>';
        return $html;
    }
