...
class CommonAction extends AuthAction{
	 ...
	 * @throws ThinkExecption
     +----------------------------------------------------------
	 */
	protected function _list($model, $map, $sortBy = '', $asc = false, $pageNum=0,$group='',$distinct_field='') {
		//排序字段 默认为主键名
		...
		//取得满足条件的记录数
        if($distinct_field){
            $count = $model->where ( $map )->count ('distinct('.$distinct_field.')');
        }else{
            $count = $model->where ( $map )->count ();
        }

		//$all_list = $model->where($map)->group($group)->findAll ( );
...
//        echo $model->getlastsql();
		...
		if ($count > 0) {
			...
			// 设置当前页面分页
			if ($pageNum) {
			    $p->firstRow = $p->listRows*($pageNum-1);
			}
	...
	public function toogle_status()
	{
		...
		if($field == 'is_index' || $field == 'is_best')
		{
			...
			if($money != 0 || $score != 0 || $point != 0)
			{
				...
				modify_account(array("money"=>$money,"is_admin"=>1,"score"=>$score,"point"=>$point),$user_id,$log_txt);
			}
		}
