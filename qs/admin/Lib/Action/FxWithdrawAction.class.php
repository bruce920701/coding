...
class FxWithdrawAction extends CommonAction{
	...
	public function do_withdrawal()
	{
		...
		if($withdrawal_info['money']<=0) {
			$this->error("提现金额必须大于0");
		}
		$withdrawalMoney = $withdrawal_info['money'] + $withdrawal_info['fee'];
		$result=get_fx_withdraw_money($withdrawal_info['user_id']);
		if($withdrawalMoney>$user_info['fx_money']-$result)$this->error("提现超额");				
		...
		if($withdrawal_info['is_paid']==0)
		{
			...
			if($withdrawal_info['type']==0){//提至余额
				modify_account(array('money'=>$withdrawalMoney,'is_admin'=>1),$withdrawal_info['user_id'],$user_info['user_name'].FX_NAME."提现".format_price($withdrawal_info['money'])."元至余额审核通过。".$log);
				modify_fx_account("-".$withdrawalMoney,$withdrawal_info['user_id'],$user_info['user_name'].FX_NAME."提现".format_price($withdrawal_info['money'])."元至余额审核通过。".$log);
