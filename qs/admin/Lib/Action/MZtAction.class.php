...
class MZtAction extends CommonAction{
	...
	public function add()
	{
		...
		$nav_cfg=get_mobile_cfg($nav_cfg,3);
    ...
    public function zt_img_upload() {
        ...
        if($_REQUEST['url'] && $_REQUEST['mobile_type2']==0){
        
            $regex="/^((ht|f)tps?):\/\/[\w\-]+(\.[\w\-]+)+([\w\-\.,@?^=%&:\/~\+#]*[\w\-\@?^=%&\/~\+#])?$/";
            if(preg_match($regex, $_REQUEST['url'])){
                $str=strstr($_REQUEST['url'],SITE_DOMAIN);
                if($str){
                    $result['status']=0;
                    $result['info']='app自定url不能为内部链接';
                    ajax_return($result);
                }
            }
            else{
                $result['status']=0;
                $result['info']='请填写完整的链接';
                ajax_return($result);
            }
        }
