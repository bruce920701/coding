...
class SupplierLocationAction extends CommonAction{
	...
	public function index()
	{
		...
		$p = new Page ( $total, '' );
		$page = $p->show ();
		if($this->isOpenDC/*defined("DC")*/){
		    $is_dc=1;
		}else{
		    $is_dc=0;
		}	
        foreach($list as $k=>$item){
            $list[$k]['defined_is_dc']=$is_dc;
        }
	...
	public function add()
	{
		...
		//end DC
		$this->assign ( 'is_dc', $this->isOpenDC );
		
		$hasLocationService = 0;
		if ($this->locationVisitServiceCheck()) {
			$hasLocationService = 1;

			$service_time_html = $this->fetch('add_service_time');
			$this->assign('service_time_html', $service_time_html);
		}
		$this->assign('hasLocationService', $hasLocationService);
	...
	public function area_list()
	{
		...
		$area_list = M("Area")->where(array("city_id"=>intval($_REQUEST['city_id']),"pid"=>0))->findAll();
		...
		foreach ($area_list as $t => $v){
		    ...
		    {
		        $area_list[$t]['checked'] = true;
		    }
		    $sub_area_list=$GLOBALS['db']->getAll("select * from ".DB_PREFIX."area where pid=".$v['id']);
		    foreach ($sub_area_list as $tt => $vv){
		        if(in_array($vv['id'], $f_curr_area))
		        {
		            $sub_area_list[$tt]['checked'] = true;
		        }
		    }
		...
		$this->assign("json_area_list",json_encode($area_list));
	...
	public function insert() {
		...
		$deal_cate_id = intval($_REQUEST['deal_cate_id']);
		...
		if ($this->locationVisitServiceCheck() && $data['is_visiting_service'] == 1) {
			$service_time['item_name'] = $_REQUEST['service_item_name'];
			$service_time['limit_num'] = $_REQUEST['service_limit_num'];
			$service_time['begin_time_h']=$_REQUEST['service_begin_time_h'];
			$service_time['begin_time_m']=$_REQUEST['service_begin_time_m'];
			$service_time['end_time_h']=$_REQUEST['service_end_time_h'];
			$service_time['end_time_m']=$_REQUEST['service_end_time_m'];

			$visit_service_time = $this->format_location_service_time($service_time);

			$v_len = count($visit_service_time);
			if ($v_len < 1) {
				$this->error(l('服务起止不能设置相同且最大预约数不允许为空'));
			}
			// 多边形过滤
			if (empty($data['xpoints']) || empty($data['ypoints'])) {
				$this->error(l('请绘制上门服务范围'));
			}
		}
		// END VISITING_SERVICE
		$log_info = $data['name'];
		...
		if (false !== $list) {
			...
			// VISITING_SERVICE
			if ($this->locationVisitServiceCheck() && $data['is_visiting_service'] == 1) {
				foreach ($visit_service_time as &$time) {
					$time['location_id'] = $list;
				}unset($time);
				$GLOBALS['db']->inserts(DB_PREFIX.'supplier_location_service_time', $visit_service_time);
			...
			$area_ids[] = intval($_REQUEST['p_area_id']);
			...
			if($id>0 && $edit_type == 2){ //商户提交审核
			    ...
			    $this->assign("jumpUrl",u("SupplierLocation/edit",array("id"=>$list)));
			}
	...
	public function edit() {		
		...
		//END DC
		$hasLocationService = 0;
		if ($this->locationVisitServiceCheck($id)) {
			$hasLocationService = 1;

			$service_time=M('SupplierLocationServiceTime')->where('location_id='.$id)->findAll();

			$service_time_html = '';
			if(count($service_time)>0) {
				$service_time=array_sort($service_time,'begin_time_h');
	
				$service_time_html=$this->get_template_html('add_service_time',$service_time);
			}

			if (empty($service_time_html)) {
				$service_time_html = $this->fetch('add_service_time');
			}
			$this->assign("service_time_html",$service_time_html);
		...
		if(strpos($_SERVER['HTTP_REFERER'],'a=biz_apply_edit'))
		    $back_url=u("SupplierLocation/publish");
		else
		    $back_url=u("SupplierLocation/index");
	...
	public function update() {
		...
		$data = M(MODULE_NAME)->create ();
		$deal_cate_id = intval($_REQUEST['deal_cate_id']);
        if(!$_REQUEST['is_recommend']){
            $data['is_recommend']=0;
        }
        if(!$_REQUEST['is_verify']){
            $data['is_verify']=0;
        }
		...
		if ($this->locationVisitServiceCheck($data['id']) && $data['is_visiting_service'] == 1) {
			$service_time['item_name'] = $_REQUEST['service_item_name'];
			$service_time['limit_num'] = $_REQUEST['service_limit_num'];
			$service_time['begin_time_h']=$_REQUEST['service_begin_time_h'];
			$service_time['begin_time_m']=$_REQUEST['service_begin_time_m'];
			$service_time['end_time_h']=$_REQUEST['service_end_time_h'];
			$service_time['end_time_m']=$_REQUEST['service_end_time_m'];
			$visit_service_time = $this->format_location_service_time($service_time, $data['id']);
			$v_len = count($visit_service_time);
			if ($v_len < 1) {
				$this->error(l('服务起止不能设置相同且最大预约数不允许为空'));
			}
		...
		// 先获取需要更新的match数据
		$old_data = $GLOBALS['db']->getAll("select id,dada_shop_id, name_match_row, deal_cate_match_row, locate_match_row, tags_match_row from ".DB_PREFIX."supplier_location where id = '".$data['id']."'");
		...
		if (false !== $list) {
		   //DC
			...
			if ($this->locationVisitServiceCheck() && $data['is_visiting_service'] == 1) {
				M('SupplierLocationServiceTime')->where('location_id='.$data['id'])->delete();
				$GLOBALS['db']->inserts(DB_PREFIX.'supplier_location_service_time', $visit_service_time);
				// // logger::write($GLOBALS['db']->getLastSql());
			}
			...
			if($data['is_main']==1)
			{
				M(MODULE_NAME)->where("supplier_id=".$data['supplier_id']." and id <> ".intval($data['id']))->setField("is_main",0);
			}
			if(M(MODULE_NAME)->where("supplier_id=".$data['supplier_id']." and is_main = 1")->count()==0)
			{
				M(MODULE_NAME)->where("id=".intval($data['id']))->setField("is_main",1);
			}
	...
	public function foreverdelete() {
		...
		if (isset ( $id )) {
				...
				// 删除关键字
				require_once(APP_ROOT_PATH."system/model/search_key_words.php");
				deleteKeyWordsApi( explode(',', $id), 4);
...
//         echo $id,$supplier_id;
        ...
        if(count($images)>0){
            M("SupplierLocationImages")->where('supplier_location_id='.$id)->delete();
    ...
    public function refused_apply(){
        ...
        if($location_submit_info && $location_submit_info['admin_check_status'] == 0){
            ...
            $GLOBALS['db']->autoExecute(DB_PREFIX."supplier_location_biz_submit",array("admin_check_status"=>2,"memo"=>$memo),"UPDATE","id=".$id);
            if($GLOBALS['db']->affected_rows()){
    	        $result['status'] = 1;
    	        $result['info'] = "成功拒绝用户申请";
    	        save_log($location_submit_info['name']."商户门店审核被拒绝，原因：".$memo,1);    
	        }
	        else {
	            $result['status'] = 0;
	            $result['info'] = "拒绝审核失败";
	...
	*/
	...
	public function select_supplier_dada(){
		...
		$supplier_info = M("Supplier")->getById(intval($_REQUEST ['id']));
		...
		//查询是否开启达达配送
		$is_open_dada_delivery = $GLOBALS['db']->getOne("select is_open_dada_delivery from ".DB_PREFIX."supplier where id= ".$_REQUEST['id']);
		//开启了达达配送，是否已注册有无账号
	 ...
	 * @return               
	 */
	public function locationVisitServiceCheck($location_id = 0)
	{
		if (defined('IS_VISITING_SERVICE') && IS_VISITING_SERVICE) {
			/*if ($location_id > 0) {
				$count = M('LocationServiceLink')->where(array('location_id' => $location_id))->count();
				if ($count <= 0) {
					return false;
				}
			}*/
	...
	private function format_location_service_time($opentime, $location_id = 0)
	{
		$open_time_data = array();
		foreach($opentime['begin_time_h'] as $k => $begin_time_h_item) {
			if (empty($opentime['limit_num'][$k]) || ($opentime['begin_time_h'][$k] == $opentime['end_time_h'][$k] && $opentime['begin_time_m'][$k] == $opentime['end_time_m'][$k])) {
				continue;
			}
			$open_time_data[$k]['item_name'] = $opentime['item_name'][$k];
			$open_time_data[$k]['limit_num'] = $opentime['limit_num'][$k];
			$open_time_data[$k]['begin_time_h'] = $opentime['begin_time_h'][$k];
			$open_time_data[$k]['begin_time_m'] = $opentime['begin_time_m'][$k];
			$open_time_data[$k]['end_time_h'] = $opentime['end_time_h'][$k];
			$open_time_data[$k]['end_time_m'] = $opentime['end_time_m'][$k];
			if ($location_id) {
				$open_time_data[$k]['location_id'] = $location_id;
			}
		}
		// logger::write(print_r($open_time_data, 1));
		return $open_time_data;
	}
}
?>