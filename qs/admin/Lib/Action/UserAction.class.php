...
class UserAction extends CommonAction{
	...
	public function index()
	{
		...
		if(intval($_REQUEST['is_id_validate'])!=0)
		{
			$map[DB_PREFIX.'user.is_id_validate'] = intval($_REQUEST['is_id_validate'])==-1?0:intval($_REQUEST['is_id_validate']);
		}

        if(!isset($_REQUEST['bdzx_status']))
        {
            $_REQUEST['bdzx_status'] = -1;
        }

        if(isset($_REQUEST['bdzx_status']) && $_REQUEST['bdzx_status'] != -1 )
        {
            $bdzx_status = $_REQUEST['bdzx_status'];
            $map[DB_PREFIX.'user.bdzx_status'] = $bdzx_status;
        }
		...
		//echo json_encode($this->_list($model,$map));
		$this->display ();
	}
	...
	public function edit() {		
		...
		if($vo['is_id_validate']!=0){
			$id_validate = M("IdcardValidate")->where("uid=".$id)->find();
			$this->assign("id_validate",$id_validate);
		}
		$this->display ();
	}
	...
	public function update() {
		...
		if(intval($_REQUEST['is_id_validate'])!=0){
			$update_data['status']=intval($_REQUEST['is_id_validate']);
			$GLOBALS['db']->autoExecute(DB_PREFIX."idcard_validate", $update_data, 'UPDATE', " uid=".$data['id']);
			$GLOBALS['db']->autoExecute(DB_PREFIX."user", array('is_id_validate'=>intval($_REQUEST['is_id_validate'])), 'UPDATE', " id=".$data['id']);
		}
        ...
        foreach($post_data as $k=>$v)
        {
            if ($k == "register_credits" || $k == "avalible_benefit_credits" || $k == "avalible_consume_credits" || $k == "active_code" || $k == "bdzx_status")
            {
                $sql = "update " . DB_PREFIX . "user set " . $k . "=" . $v . " where id=" . $user_id;
	...
	public function modify_account()
	{
        ...
        $frozen_score = intval($_REQUEST['frozen_score']);
		$msg = strim($_REQUEST['msg'])==''?l("ADMIN_MODIFY_ACCOUNT"):strim($_REQUEST['msg']);
        if(app_conf('SCORE_RECHARGE_SWITCH')){
            modify_account(array('money'=>$money,'is_admin'=>1,'score'=>$score,'point'=>$point),$user_id,$msg);
            if($frozen_score){
                modify_account(array('frozen_score'=>$frozen_score),$user_id,"管理员编辑冻结积分");
            }
        }else{
            modify_account(array('money'=>$money,'is_admin'=>1,'score'=>$score,'point'=>$point),$user_id,$msg);
        }
	...
	public function do_withdrawal()
	{
        ...
        if($type==0){
            ...
            if($withdrawal_info['is_paid']==0)
            {
                ...
                M("Withdraw")->where("id=".$id)->setField("pay_time",NOW_TIME);
				M("Withdraw")->where("id=".$id)->setField("is_chargeback",1);
                //M("Withdraw")->where("id=".$id)->setField("money",$withdrawal_info['money']);
                if($withdrawal_info['is_chargeback']==0){
                    modify_account(array('money'=>"-".$withdrawal_info['money'],'is_admin'=>1),$withdrawal_info['user_id'],$user_info['user_name']."提现".format_price($withdrawal_info['money'])."元审核通过。".$log);
                }else{
                    //modify_account(array('money'=>"-".$withdrawal_info['money'],'is_admin'=>1),$withdrawal_info['user_id'],$user_info['user_name']."提现".format_price($withdrawal_info['money'])."元审核通过。".$log);
                }
                ...
                $content=$user_info['user_name']."提现￥".round($withdrawal_info['money'],2)."元审核通过。".$log;
                save_log($content,1);
                require_once(APP_ROOT_PATH.'system/msg/msg.php');
                $msgClass = new Msg;
                $msgClass->send_msg($user_info['id'],$content,"account",array("type"=>5));
                $this->success("确认提现成功");
            }
            else
            {
            ...
            if($withdrawal_info['is_paid']==0)
            {
                //$withdrawal_info['money']=floatval($_REQUEST['money']);
                //if($withdrawal_info['money']<=0)$this->error("提现金额必须大于0");
                M("Withdraw")->where("id=".$id)->setField("is_paid",2);
                //M("Withdraw")->where("id=".$id)->setField("money",$withdrawal_info['money']);
                M("Withdraw")->where("id=".$id)->setField("pay_time",NOW_TIME);
                if($withdrawal_info['is_chargeback']==0){

                }else{
                    modify_account(array('money'=>$withdrawal_info['money']),$withdrawal_info['user_id'],$user_info['user_name']."提现".format_price($withdrawal_info['money'])."元审未通过，金额返还。".$log);
                }
                save_log($user_info['user_name']."提现".format_price($withdrawal_info['money'])."元审核未通过。".$log,1);
                $this->success("拒绝提现成功");
            }else{
                $this->error("已经审核过,无法在审核");
            }
        }
