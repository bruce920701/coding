...
class BaseAction extends Action{
	...
	public function __construct()
	{
		...
		if(IS_VISITING_SERVICE){
		    $config_file = APP_ROOT_PATH."system/mobile_cfg/".APP_TYPE."/vsnav_cfg.php";
		    $mobile_cfg = array_merge_mobile_cfg($mobile_cfg, $config_file);
		}
		
		if(IS_PRESELL)
		{
			$config_file = APP_ROOT_PATH."system/mobile_cfg/".APP_TYPE."/presellwebnav_cfg.php";
			$mobile_cfg = array_merge_mobile_cfg($mobile_cfg, $config_file);
		}
        if(IS_PT){
            $config_file = APP_ROOT_PATH."system/mobile_cfg/".APP_TYPE."/ptwebnav_cfg.php";
            $mobile_cfg = array_merge_mobile_cfg($mobile_cfg, $config_file);
        }
		check_install();
		...
		if (is_file(LANG_PATH.$langSet.'/common.php'))
		{
			...
			if(defined('IS_VISITING_SERVICE') && IS_VISITING_SERVICE == 1 && is_file(LANG_PATH.$langSet.'/visiting_service.php')){
				L(include LANG_PATH.$langSet.'/visiting_service.php');
				$visiting_service_lang= require LANG_PATH.$langSet.'/visiting_service.php';
				$this->lang_pack=array_merge($this->lang_pack,$visiting_service_lang);
			}
		...
		$admin_root=SITE_DOMAIN.$_SERVER['PHP_SELF'];
		$this->assign("admin_root",$admin_root);
