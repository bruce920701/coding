...
class DcResOrderHistoryAction extends CommonAction{
		...
		if ($count > 0) {
			...
			foreach($voList as $k=>$v){
				...
				foreach($rs_list['cart_list'] as $km=>$vm){
					$voList[$k]['table_time_format']='<span class="time_span">'.trim($vm['name']).'</span>'.$vm['table_time_format'];
				}
	...
	{	
		...
		$order_info = M("DcOrderHistory")->where("id=".$id." and type_del = 0 and is_rs = 1")->find();
		...
		$dcver_arr = unserialize($order_info['history_dc_coupon']);	//电子券
		$dcver=$dcver_arr[0];
	...
	public function refund_reason(){
		...
		$order_info = $GLOBALS['db']->getRow("select refund_memo , user_name from ".DB_PREFIX."dc_order_history where id=".$order_id);
	...
	if($voList){ 
				...
				if($m_cart_list){
					...
					$vt['table_time_format']='<span class="time_span">'.trim($vt['name']).'</span>'.$vt['table_time_format'];
					$span='<span class="time_span">';
					$voList[$k]['table_time_format']=str_replace($span,"\n",$vt['table_time_format']);
					$voList[$k]['table_time_format']=strip_tags($voList[$k]['table_time_format']);
						}
	    	...
	    	if($page == 1)
	    	{
		    	$content = iconv("utf-8","gbk","订单信息,预订信息,菜单,会员名称,下单时间,应付总额,已付金额,订单状态");	    		    	
		    	$content = $content . "\n";
	    	}
