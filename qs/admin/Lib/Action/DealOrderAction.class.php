...
class DealOrderAction extends CommonAction{
    ...
    protected function order_list($model, $type=0, $is_supplier=0, $pageNum=0){
        ...
        if(intval($_REQUEST['is_distribution'])!=0){
            if(intval($_REQUEST['is_distribution'])==-1){
                $map['DealOrder.distribution_id']=0;
            }else{
                $map['DealOrder.distribution_id']=array('gt',0);
            }
        }
        if($_REQUEST ['id']){
            $map['DealOrder.id']=array('in',$_REQUEST ['id']);
        }
    ...
    public function bestgoodsOrder(){
        $orderModel = D('SupplierOrderView');
        $this->order_list('', 8);
        $this->assign ('title', '精品区订单');
        $this->display("supplierOrder");
    }
	...
	public function export_csv($parameter)
	{
		...
		if($list)
		{
			...
			if($type==4)
                $order_value = array('sn'=>'""', 'user_name'=>'""', 'deal_name'=>'""','number'=>'""', 'create_time'=>'""', 'total_price'=>'""', 'pay_amount'=>'""', 'consignee'=>'""', 'address'=>'""', 'mobile'=>'""', 'memo'=>'""', 'delivery_status'=>'""','refund_status'=>'""','order_status'=>'""','distribution_id'=>'""');
            else
	    	...
	    	{
	    	    if($type==4)
                    $content = iconv("utf-8","gbk","订单编号,用户名,商品名称,订购数量,下单时间,订单总额,已收金额,收货人,发货地址,手机号码,订单留言,发货状态,退款申请,订单状态,驿站名称");
                else
			...
			foreach($list as $k=>$v)
			{
                ...
                if($type==4){
                    $distribution_name = $GLOBALS['db']->getOne("select name from ".DB_PREFIX."distribution where id=".intval($v['distribution_id']));
                    $order_value['distribution_id'] = '"' . iconv('utf-8','gbk',$distribution_name) . '"';
                }
	...
	public function delivery()
	{
		...
		foreach($order_deal_items as $k=>$v)
    		{
    		    $order_deal_items[$k]['name']=htmlspecialchars_decode($v['name']);
	...
	public function un_do_delivery()
	{
	    ...
	    else
	    {
	        ...
	        if($GLOBALS['db']->affected_rows()){
	           ...
	           if($_REQUEST['memo']){
	               $_REQUEST['memo']="备注：".$_REQUEST['memo'];
	           }
    	        order_log("发货成功    ".$_REQUEST['memo'],$order_id);
    	        if($silent==0)
    	            $this->success($msg);
	        }else {
	            $this->error(l("发货失败"));
...
//			echo $model->getlastsql();
	...
	public function refund()
	{
		...
		if($data)
		{
            ...
            if($order_item_id){
                ...
                $order_info['pay_amount']=get_pay_amount_in_deal_order($order_info);
            }else{
		    ...
		    $order_info['pay_amount']     = format_price_floor( $order_info['pay_amount']);
				...
				if($GLOBALS['db']->affected_rows())
				{
					...
					if ($order_info['pay_status'] < 2 && $refund_money > 0) {
						...
						if($refund_money > 0)
						{
							...
							modify_account(array("money"=>$refund_money,"is_admin"=>1,"supplier_id"=>$supplier_id), $order_info['user_id'],"取消订单，退回余额支付 ");
    ...
    //批量分配驿站订单列表
    public function dist_choose_order()
    {
        $this->order_list('', 4);
        $this->display('exit_dist');
    }
    //批量分配驿站
    public function dist_choose_do()
    {

        $id = $_REQUEST ['check_ids'];
        if (isset ( $id )) {
            $id_arr=explode ( ',', $id );
            $is_err=0;
            $err_info='分配成功';
            ...
            $model = M('DealOrder');
            foreach($id_arr as $v){
                $order = $model->where(array('id' => $v, 'delivery_status' =>0,'pay_status'=>2,'order_status'=>0))->find();
                if(!$order){
                    $is_err=1;
                    $err_info='存在不符合条件的订单';
                    break;
            ...
            if($is_err==0){
                $did = intval($_REQUEST['dist_result']);
                foreach($id_arr as $v){
                    if($order_list[$v]['distribution_id'] != $did){
                        setDistAndAgent($did, $v);
                    }
                }
                $this->success ($err_info,$ajax);
            }else{
                $this->error ($err_info,$ajax);
            }
        } else {
            $this->error (l("INVALID_OPERATION"),$ajax);
        }
    }
    public function distributionOrderFee(){
        $map = array();
        if(strim($_REQUEST ['order_sn'])){
            $map['order_sn']=strim($_REQUEST ['order_sn']);
        }
        $model = M ("DistributionOrderFee");
        if (! empty ( $model )) {
            $this->_list ( $model, $map );
        }
        $this->assign("title",'驿站服务费结算表');
        $this->display ();
    }
}
?>