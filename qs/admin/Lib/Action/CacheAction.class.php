...
class CacheAction extends CommonAction{
	...
	public function index()
	{
		...
		$this->assign("count",6);//4或者5,5的时候开放《同步订单进程状态》按钮
		$this->display();
	}
...
//        $GLOBALS['db']->query("update ".DB_PREFIX."user set path=CONCAT(id,'.') where pid=0 and path=0;");
//        $flag=true;
//        while($flag){
//            $GLOBALS['db']->query("update ".DB_PREFIX."user u1,".DB_PREFIX."user u2 set u1.path=CONCAT(substring_index(u2.path,'.',-4),u1.id,'.') where u2.path!=0 and u1.path=0 and u1.pid=u2.id");
//            if(!$GLOBALS['db']->affected_rows())$flag=false;
//        }
//        $flag=true;
//        while($flag){
//            $GLOBALS['db']->query("update ".DB_PREFIX."user u1,".DB_PREFIX."user u2 set u1.path=CONCAT(u2.id,'.',u1.id,'.') where u1.path=0 and u1.pid=u2.id");
//            if(!$GLOBALS['db']->affected_rows())$flag=false;
//        }
//        $GLOBALS['db']->query("update ".DB_PREFIX."user u1,".DB_PREFIX."user u2 set u1.path=CONCAT(u1.id,'.') where u1.path=0");
        $GLOBALS['db']->query("call update_user_tree();");
        last_sql();
        exit("<div style='line-height:50px; text-align:center; color:#f30;'>树同步成功</div><div style='text-align:center;'><input type='button' onclick='$.weeboxs.close();' class='button' value='关闭' /></div>");

    }
	/**
	 * 同步索引表search_key_words
	 * @author hhcycj
	 */
	public function insertKewWords(){
	    set_time_limit(0);
        require_once(APP_ROOT_PATH."system/model/search_key_words.php");
        insertKeyWords();
        
        header("Content-Type:text/html; charset=utf-8");
        exit("<div style='line-height:50px; text-align:center; color:#f30;'>关键字同步成功</div><div style='text-align:center;'><input type='button' onclick='$.weeboxs.close();' class='button' value='关闭' /></div>");
	}
	...
	public function syn_order_process_status()
	{
		set_time_limit(0);		
		$order_list= $GLOBALS['db']->getAll("select id from " . DB_PREFIX . "deal_order where pay_status=2 and not (is_cancel=0 and is_delete=1)" );
		foreach($order_list as $k=>$v){
			update_order_cache($v['id']);
		}
		header("Content-Type:text/html; charset=utf-8");
       	exit("<div style='line-height:50px; text-align:center; color:#f30;'>订单进程同步完成</div><div style='text-align:center;'><input type='button' onclick='$.weeboxs.close();' class='button' value='关闭' /></div>");
	}

	public function create_schedule_appjs(){
        require APP_ROOT_PATH."system/model/Schedule.php";

        $sch = new Schedule();
        $sch->make_app_js();
        header("Content-Type:text/html; charset=utf-8");
        exit("<div style='line-height:50px; text-align:center; color:#f30;'>计划任务脚本生成成功</div><div style='text-align:center;'><input type='button' onclick='$.weeboxs.close();' class='button' value='关闭' /></div>");
    }
}
?>