$(document).ready(function(){
	...
	var package_start_price = $('input[name="package_start_price"]').val();
	if (Number(package_start_price) <= 0 && Number(package_start_price)!=-1) {
		$('.package_price_box').hide();
	}

	$('input[name="package_start_price"]').bind('input propertychange', function() {
		var val = Number($.trim($(this).val()));
		
		if (val > 0 || val==-1) {
			$('.package_price_box').show();
		} else {
			$('.package_price_box').hide();
		}
	})
	...
	function comment_count() {
		$('input[name="comment"]').bind('input propertychange', function() {
			var val = $.trim($(this).val());
			var count = val.length;
			if (count >= 15) {
				var newval = val.substr(0,15);
				$(this).val(newval);
				count = 15;
			}
...
function change_supplier_name(){
	...
	var ajax_url =ROOT+"?"+VAR_MODULE+"=SupplierLocation&"+VAR_ACTION+"=select_supplier_dada";
