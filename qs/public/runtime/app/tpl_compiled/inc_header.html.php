...
?></span>	
		<span class="f_r">
			<ul class="head_tip">
			<?php if(IS_OPEN_AGENCY==1){    ?>
				<!--<li class="user_tip"><a href="<?php
echo parse_url_tag("u:hiz|index|"."".""); 
?>" target="_blank">代理商中心</a></li>-->
			<?php } ?>	
				<!--<li class="user_tip"><a href="<?php
...
?>" class="city_switch f_l">切换城市</a>
	...
	<div class="search f_r" style="position:relative;">
		<div class="top_search key_words_search_display">
			<form action="<?php
echo parse_url_tag("u:index|searchkeywords#index|"."".""); 
?>" name="search_form" method=get class="key_words_search_display" >
			...
			<input type="text" name="search_keyword" autocomplete="off" class="ui-textbox search_keyword f_l key_words_search_display" holder="请输入您要搜索的关键词" value="<?php echo $this->_var['kw']; ?>" />
			<input type="hidden" name="key_words_url"  value="<?php
echo parse_url_tag("u:index|searchkeywords#keyWordsSearchResult|"."".""); 
?>" />
			<button class="ui-button f_l" rel="search_btn" type="submit">搜索</button>
			</form>
		</div>
		<ul class="search_hot_keyword key_words_search_display">
			<?php $_from = $this->_var['hot_kw']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'kw');if (count($_from)):
    foreach ($_from AS $this->_var['kw']):
?>
			<li><a href="<?php echo $this->_var['kw']['url']; ?>"><?php echo $this->_var['kw']['txt']; ?></a></li>
			<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
		</ul>
