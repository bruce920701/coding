...
?>
					...
					<?php if ($this->_var['filter_row']['list']): ?>
					    <div class="hidden_nav">
							...
							<ul class="clearfix">
								<?php $_from = $this->_var['filter_row']['list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'item');if (count($_from)):
    foreach ($_from AS $this->_var['item']):
?>
								<li>
									<a href="<?php echo $this->_var['item']['url']; ?>" class="font_hover"><?php echo $this->_var['item']['name']; ?></a>
								</li>
...
?>
	...
	<dl class="clearfix <?php if (($this->_foreach['filter_loop']['iteration'] == $this->_foreach['filter_loop']['total'])): ?>bottom<?php endif; ?>">
		...
		<dd class="wrap_filter">
			<ul>
				<?php $_from = $this->_var['filter_item']['list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'item');if (count($_from)):
    foreach ($_from AS $this->_var['item']):
?>
				<li <?php if ($this->_var['item']['current'] == 1): ?>class="current"<?php endif; ?>><a href="<?php echo $this->_var['item']['url']; ?>" ><?php echo $this->_var['item']['name']; ?></a></li>
				<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
			</ul>
		</dd>
		<dd class="expend">
