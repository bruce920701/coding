...
function jh_send_verify_sms($mobile,$code)
{
    $tpl_id = 72728;
    $tp_value = urlencode("#code#=".$code);
    $key = "95040fbf28e1e71fddc71e419add520e";
    $url = "http://v.juhe.cn/sms/send?mobile=".$mobile."&tpl_id=".$tpl_id."&tpl_value=".$tp_value."&key=".$key;
    $ch = curl_init();
...
function get_cart_type($deal_id,$app_index='wap',$is_presll=0){
    
    $deal_info = $GLOBALS['db']->getRow("select id,is_delivery,buy_type,is_shop,supplier_id,presell_begin_time,presell_end_time from ".DB_PREFIX."deal where id=".$deal_id);

    if($deal_info['is_shop']==0 || ( $deal_info['is_shop']==1 && ($deal_info['is_delivery']==0 || IS_PRESELL && $deal_info['presell_begin_time']<NOW_TIME && $deal_info['presell_end_time']>NOW_TIME)) || $deal_info['buy_type']==1 ){//团购，积分，预售商品，无需发货的商品 直接进入提交订单页，不进入购物车页面
        $result['in_cart'] = 0;
        if($app_index=='wap'){
            $result['jump'] = wap_url("index","cart#check",array("id"=>$deal_id,'is_presell'=>$is_presll));
            
        }else{
            $result['jump'] = url("index","cart#check",array("id"=>$deal_id,'is_presell'=>$is_presll));
        }
...
function check_order($order_id){
    $deal_order = $GLOBALS['db']->getRow("select is_main,type,pay_status,create_time,presell_end_time from ".DB_PREFIX."deal_order where id=".$order_id);
    $has_pay=0;
    if($deal_order['is_main']==1){ //如果是主单，则检测子单的支付情况
        ...
        if($deal_order['type']==8){
            require_once APP_ROOT_PATH.'system/model/deal_order.php';
            $result = check_order_allow_pay($deal_order);
...
{
	$distAndAgentSql = 'SELECT id, agency_id,name FROM '.DB_PREFIX.'distribution WHERE id='.$dist_id.' AND is_delete=0 AND status=1 AND disabled=0';
	...
	if ($res) {
        ...
        AppPush::dist_new_order($distAndAgent);
	} else {
...
function getYouhuiStatistics($youhui_id=0,$user_id=0,$is_update=0,$is_new=0) {
    $arr=array('user_count'=>0,'user_day_count'=>0);
    if($youhui_id>0&&$user_id>0){
        $user_youhui_statistics=$GLOBALS['db']->getRow("select id,user_count,user_day_count
            from ".DB_PREFIX."user_youhui_statistics
            where youhui_id=".$youhui_id." and user_id = ".$user_id." and stat_time = '" .to_date(NOW_TIME,"Y-m-d")."'");
        if(!$user_youhui_statistics){
            $is_update=1;
        }
        if($is_update==1){
            $data=array();
            if(intval($user_youhui_statistics['id'])==0){
                $data['youhui_id']=$youhui_id;
                $data['user_id']=$user_id;
                $data['stat_time']=to_date(NOW_TIME,"Y-m-d");
            }
            if($is_new==1){
                $data['user_count']=0;
                $data['user_day_count']=0;
            }else{
                $user_count = $GLOBALS['db']->getOne("select count(*)
                                from ".DB_PREFIX."youhui_log
                                where user_id = ".$user_id." and youhui_id = ".$youhui_id);
                $data['user_count']=$user_count?$user_count:0;

                $date_begin = to_timespan(to_date(NOW_TIME,"Y-m-d"),"Y-m-d");
                $date_end = $date_begin+24*3600;
                $user_day_count=$GLOBALS['db']->getOne("select count(*)
                                from ".DB_PREFIX."youhui_log
                                where user_id = ".$user_id." and youhui_id = ".$youhui_id."
                                and create_time > ".$date_begin." and create_time < ".$date_end);
                $data['user_day_count']=$user_day_count?$user_day_count:0;

            }


            if(intval($user_youhui_statistics['id'])==0){
                $GLOBALS['db']->autoExecute(DB_PREFIX."user_youhui_statistics",$data,'INSERT','','SILENT');
            }else{
                $GLOBALS['db']->autoExecute(DB_PREFIX."user_youhui_statistics",$data,'UPDATE',' id='.intval($user_youhui_statistics['id']),'SILENT');
            }
            $arr['user_count']=$data['user_count'];
            $arr['user_day_count']=$data['user_day_count'];
        }else{
            $arr['user_count']=$user_youhui_statistics['user_count'];
            $arr['user_day_count']=$user_youhui_statistics['user_day_count'];
        }
