...
class Account_payment implements payment {
	...
	{	$payment_notice = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."payment_notice where id = ".$payment_notice_id);	
		...
		{
			$order_sn = $GLOBALS['db']->getOne("select order_sn from ".$payment_result['table_name']." where id = ".$payment_notice['order_id']);
			require_once(APP_ROOT_PATH."system/model/user.php");
			$msg = sprintf('%s订单付款100000000000000,付款单号%s',$order_sn,$payment_notice['notice_sn']);
	...
	public function get_display_code()
	{
		...
		{						
			$html = "<p>帐户余额：<b>".format_price($user_info['money'])."</b>，使用余额支付".					
					" <b id='account_money'></b>，".
					"<label class='ui-checkbox' rel='common_cbo'><input type='checkbox' checked='checked' id='check-all-money' name='all_account_money' value='1'>全额支付</label></p>";
			return $html;
		}
		else
		{
