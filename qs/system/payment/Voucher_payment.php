...
class Voucher_payment implements payment {
	...
	public function get_display_code($total_price=0)
	{
		$sql = "select e.sn as sn,t.name as name ,t.start_use_price from ".DB_PREFIX."ecv as e left join ".DB_PREFIX."ecv_type as t on e.ecv_type_id = t.id where ".
		...
		$html = "<table><tr><td>红包支付：</td><td><select name='ecvsn' class='ui-select voucher_select'>";
		
		foreach($voucher_list as $k=>$v)
		{
		    if($v['start_use_price']==0){
		        $condition='( 无门槛使用 )';
		    }else{
		        $condition='( 满'.round($v['start_use_price'],2).'元使用 )';
		    }
		    
			$html.="<option value='".$v['sn']."'>".$v['name'].$condition."</option>";
		}
		
		$html.="<option value='0'>=不使用红包=</option></select></td></tr></table>";
