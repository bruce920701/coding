 ...
 * 标准：消费券：确认过未点评  实体：已收货未点评  无券无发货：已支付未点评
 * 商品点评表示为购物点评，同步平均价
 * 点评完需同步商品的评分
 *
 * 2.优惠券
 ...
 *
 * 关于商家ID
 * 商品：消费券：验证时会生成验证的门店ID, 实体：配货时会生成配货的门店ID 如ID无法获取，以相关商品支持的门店列表中取总店获随机其中之一
...
function check_dp_status($user_id,$param=array("deal_id"=>0,"youhui_id"=>0,"event_id"=>0,"location_id"=>0,"order_item_id"=>0,"youhui_log_id"=>0,"event_submit_id"=>0))
{
	...
	if($deal_id>0)
	{
		...
		elseif($deal_info['is_delivery']==1)
		{
			...
			else
			{
				$sql = "select n.*,n.location_id as location_id,doi.total_price as avg_price,doi.id as doiid from ".DB_PREFIX."delivery_notice as n left join ".
						DB_PREFIX."deal_order_item as doi on n.order_item_id = doi.id left join ".
						DB_PREFIX."deal_order as do on doi.order_id = do.id ".
						"where doi.deal_id = ".$deal_id." and n.order_item_id = ".$order_item_id." and doi.dp_id = 0 and doi.is_arrival = 1 and do.user_id = ".$user_id;
			}
		}
		else
		{
		...
		$sql="select doi.*,do.order_status from ".DB_PREFIX."deal_order_item as doi left join ".DB_PREFIX."deal_order as do on do.id=doi.order_id where doi.id=".$order_item_id." and (doi.is_arrival = 1 or doi.consume_count > 0 or (doi.is_shop=0 and doi.is_coupon=0)) and doi.dp_id=0" ;
		...
		{
		    if($rs['delivery_status']==5){
		        if($rs['is_shop']==0){
		            //$coupon=$GLOBALS['db']->getRow("select deal_type,end_time from ".DB_PREFIX."deal_coupon where order_deal_id=".$order_item_id." and confirm_time>0");
		            if(($rs['order_status']==0 || $rs['consume_count']==0)&&!$rs['is_coupon']==0){
	                    return array("status"=>false,"info"=>"您没有可以点评的购物记录");
		            }
		        }
		    }
			...
			$avg_price = $rs['unit_price'];
			return array("status"=>true,"info"=>"","supplier_location_id"=>$supplier_location_id,"avg_price"=>$avg_price,"order_item_id"=>$rs['id']);
		}
...
function save_review($user_id,$param=array("deal_id"=>0,"youhui_id"=>0,"event_id"=>0,"location_id"=>0,"order_item_id"=>0,"youhui_log_id"=>0,"event_submit_id"=>0),$content,$dp_point,$dp_image=array(),$tag_group=array(),$point_group=array())
{
	...
	if($order_item_id>0)
	{
		$order_item_info = $GLOBALS['db']->getRow("select deal_id,order_id from ".DB_PREFIX."deal_order_item where id = ".$order_item_id);
		$deal_id=intval($order_item_info['deal_id']);
		$order_id=intval($order_item_info['order_id']);
	}
	else
	{
		$deal_id = intval($param['deal_id']);
	...
	{
		require_once(APP_ROOT_PATH."system/model/deal.php");
		$deal_info = get_deal($deal_id,true);
		if($deal_info)
		{
			//验证是否可以点评
...
function biz_do_reply_dp($account_id,$dp_id,$reply_content,$supplier_id){
    $dp = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."supplier_location_dp where id = ".$dp_id." and supplier_id = ".$supplier_id);
    if($dp){
...
function get_dc_location_dp_list($location_id,$limit)
{
    $orderby = " order by create_time desc ";
    $limit = " limit ".$limit;
    $sql = "SELECT id,title,content,create_time,point,user_id,status,reply_content,images_cache,avg_price,supplier_location_id FROM ".DB_PREFIX."supplier_location_dp where status=1 and event_id=0 and deal_id=0 and youhui_id=0 and supplier_location_id =".$location_id.$orderby.$limit;
    $data = $GLOBALS['db']->getAll($sql);
    $sql_count = "SELECT count(*) FROM ".DB_PREFIX."supplier_location_dp where status=1 and event_id=0 and deal_id=0 and youhui_id=0 and supplier_location_id =".$location_id;
    ...
    if($data){
        foreach ($data as $k=>$v){
            ...
            $v['user_avatar'] = get_abs_img_root(get_muser_avatar($v['user_id'],"big"));
            $v['point'] = round($v['point'], 1);
	        $v['point_percent'] =  $v['point'] / 5 * 100;

            //获取条件关联信息
            $v['create_time_format'] = to_date($v['create_time'], 'Y-m-d');
            $images = unserialize($v['images_cache']);
    ...
    $avg_point = 0;
    if ($total_count > 0) {
    	$avg_point = $total_point / $total_count;
    }
