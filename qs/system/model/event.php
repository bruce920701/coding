...
function get_event_list($limit,$type=array(EVENT_NOTICE,EVENT_ONLINE,EVENT_HISTORY),$param=array("cid"=>0,"aid"=>0,"qid"=>0,"city_id"=>0), $join='', $where='',$orderby = '',$field_append="")
{
	...
	{
		//进行中的
		$condition .= " or ((".$time.">= ".$tname.".submit_begin_time or ".$tname.".submit_begin_time = 0)) ";
	}
	...
	// 新添加的
	$join .= ' LEFT JOIN '.DB_PREFIX.'event_location_link l ON '.$tname.'.id = l.event_id ';
	...
	$field_append = $field_append. ',sl.preview as location_preview, sl.name location_name,sl.id location_id, sl.address location_address, sl.avg_point location_avg_point, sl.district area_name ';
	...
	$condition_in = $condition." AND l.location_id IN ( SELECT location_id FROM
            			(
            				(
            					...
            					FROM
            						`".DB_PREFIX."event` e
            					LEFT JOIN ".DB_PREFIX."event_location_link dl ON dl.event_id = e.id
            					WHERE
            						dl.location_id <> ''
            					    AND ".$condition."
            					GROUP BY
	...
	if($join)
		$sql = "select ".$tname.".*".$field_append." from ".DB_PREFIX."event as ".$tname." ".$join." where  ".$condition." GROUP BY e.id";
	else
		$sql = "select ".$tname.".*".$field_append." from ".DB_PREFIX."event as ".$tname." where  ".$condition." GROUP BY e.id";
	...
	if($events)
	{
		foreach($events as $k=>$event)
		{
            ...
            if($GLOBALS['user_info'])
            {
                ...
                $event['json_url']= SITE_DOMAIN.url("index","event#".$event['id'],array('r'=>base64_encode(intval($GLOBALS['user_info']['id'])),'json'=>base64_encode(json_encode($json))));

            }else{
                $event['json_url']= SITE_DOMAIN.url("index","event#".$event['id'],array('json'=>base64_encode(json_encode($json))));
            }
 ...
 * 获取活动列表
 */
function get_mapi_event_list($limit,$param=array("cid"=>0,"aid"=>0,"qid"=>0,"city_id"=>0), $join='', $where='',$orderby = '',$field_append="")
{
    if(empty($param))
        $param=array("cid"=>0,"aid"=>0,"qid"=>0,"city_id"=>0);
...
function verify_event_submit($submit_id)
{
	...
	{
		do{
			$sn = '3' . substr(NOW_TIME, 1, 9). sprintf('%02s', rand(0, 99));
...
function event_auto_publish($id){
    ...
    if ($submit_data){
        ...
        if (false !== $list) {
            ...
            $GLOBALS['db']->query("delete from ".DB_PREFIX."event_biz_submit where id=".$event_submit_id);
        }
    }
}
