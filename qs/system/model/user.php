...
define("LOGIN_STATUS_TEMP",2); //临时登录
	...
	function auto_create($user_data,$type,$allow_err=false)
	{
		...
		$GLOBALS['db']->query("call update_user_tree()");
		...
		if($user_id)
		{
			...
			//手机与邮箱直接注册需要生成临时用户名
			$user_name = $user_data['user_name']?$user_data['user_name']:"用户";
		...
		else
		{
			...
			else
			{
				...
				//send_msg_new(intval($user_id), "您已成功注册，成为".app_conf('SHOP_TITLE')."的会员。", 'notify', array('type' => 1));
				return array("status"=>true,"info"=>"注册成功","user_data"=>$user_data);
			}
	...
	{		
		...
		if($GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where (user_name = '".trim($user_data['user_name'])."' or email = '".trim($user_data['user_name'])."') and id <> ".intval($user_data['id']))>0)
		{
			$field_item['field_name'] = 'user_name';
...
// 		}
		...
		*/
        ...
        if($user_data['city_code']){
            $user['city_code']=  intval($user_data['city_code']);
        }
        if($user_data['agency_id']){
            $user['agency_id']= intval($user_data['agency_id']);
        }
	...
	function modify_account($data,$user_id,$log_msg='')
	{
        ...
        if(intval($data['frozen_score'])!=0){
            $GLOBALS['db']->query("update ".DB_PREFIX."user set frozen_score = frozen_score + ".intval($data['frozen_score'])." where id =".$user_id);
			if($data['frozen_score']>0)
			{
				$GLOBALS['db']->query("update ".DB_PREFIX."user set total_score = total_score + ".intval($data['frozen_score'])." where id =".$user_id);
			}
        }
		...
		if(floatval($data['money'])!=0)
		{
		    ...
		    $set_money = floatval( $data['money'] );
		    $GLOBALS['db']->query("update ".DB_PREFIX."user set money = money + {$set_money} where ( money + {$set_money} ) >= 0 and id =".$user_id);
		}
		if(intval($data['score'])!=0||floatval($data['money'])!=0||intval($data['point'])!=0||intval($data['frozen_score'])!=0)
		{		
			$user_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where is_delete = 0 and is_effect = 1 and id = ".$user_id);
			//积分变更计算会员组
			$user_current_group = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_group where id = ".intval($user_info['group_id']));
			$user_group = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_group where score <=".intval($user_info['total_score'])." order by score desc");
			if($user_current_group['score']<$user_group['score'])
			{
				$user_info['group_id'] = intval($user_group['id']);
				$GLOBALS['db']->query("update ".DB_PREFIX."user set group_id = ".$user_info['group_id']." where id = ".$user_info['id']);
				$pm_content = "恭喜您，您已经成为".$user_group['name']."。";
				if($user_group['discount']<1)
				{
					$pm_content.="您将享有".($user_group['discount']*10)."折的购物优惠";
				}
				//send_msg_new($user_info['id'], $pm_content, 'notify', array('type' => 2));
				// send_msg($user_info['id'], $pm_content, "notify", 0);
			}
			//经验变更计算会员等级
			$user_current_level = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_level where id = ".intval($user_info['level_id']));
			$user_level = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_level where point <=".intval($user_info['point'])." order by point desc");
			if ($user_current_level['point'] != $user_level['point']) {
				$user_info['level_id'] = intval($user_level['id']);
				$GLOBALS['db']->query("update ".DB_PREFIX."user set level_id = ".$user_info['level_id']." where id = ".$user_info['id']);
				$pm_content = ($user_current_level['point']<$user_level['point'] ? '恭喜您，您已经成为 ' : '很抱歉，您已经降为 ').$user_level['name'].'.';
				//send_msg_new($user_info['id'], $pm_content, 'notify', array('type' => 2));
			}
            ...
            if(app_conf("SCORE_RECHARGE_SWITCH")){
                $log_info['frozen_score'] = intval($data['frozen_score']);
            }
	 ...
	 */
	function auto_do_login_user($user_name_or_email,$user_md5_pwd,$from_cookie = true,$is_wap=false)
	{
...
// 				}
				...
				if($user_current_group['score']<$user_group['score'])
				{
					...
					//send_msg_new($user_data['id'], $pm_content, 'notify', array('type' => 2));
				...
				foreach($medal_list as $medal)
				{
					...
					if(file_exists($file))
					{
						...
						if(class_exists($cls))
						{
							...
							{
								send_msg_new($user_data['id'], $check_result['info'], "notify", array('type' => 7));
				...
				//扫描团购订单，检测结单
				syn_auto_over_status($GLOBALS['user_info']['id']);
				//签到
				if(!$is_wap){  //wap不自动签到
				...
				//查找未登陆时购物车里的物件
				$unlogin_cart=$GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_cart where user_id=0 and session_id='".es_session::id()."'");

				if($unlogin_cart){
				    $user_cart=$GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_cart where user_id=".$GLOBALS['user_info']['id']);
				    if($user_cart){
				        $GLOBALS['db']->query("update ".DB_PREFIX."deal_cart set is_effect=0 where user_id = ".$GLOBALS['user_info']['id']);
				        foreach ($unlogin_cart as $t => $v){
				            $i=0;
				            foreach ($user_cart as $tt => $vv){
				                if(($v['deal_id']==$vv['deal_id'] && !$v['attr_str'])||
				                    ($v['deal_id']==$vv['deal_id'] && $v['attr_str']==$vv['attr_str'])
				                ){
				                    /*
				                    $num=$v['number']+$vv['number'];
				                    $total_price=$v['total_price']+$vv['total_price'];
				                    $return_total_score=$v['return_total_score']+$vv['return_total_score'];
				                    $return_total_money=$v['return_total_money']+$vv['return_total_money'];
				                    $GLOBALS['db']->query("update ".DB_PREFIX."deal_cart set number = ".$num.",total_price=".$total_price.",return_total_money=".$return_total_money.",return_total_score=".$return_total_score." where user_id = ".$GLOBALS['user_info']['id']." and deal_id = ".$vv['deal_id']);
				                    $GLOBALS['db']->query("delete from ".DB_PREFIX."deal_cart where id=".$v['id']);
				                    break;
				                    */
				                    
				                    //如果之前已经有同款商品加入购物车，则更新为最新的，舍弃之前购物记录
				                    $num=$v['number'];
				                    $total_price=$v['total_price'];
				                    $return_total_score=$v['return_total_score'];
				                    $return_total_money=$v['return_total_money'];
				                    $GLOBALS['db']->query("update ".DB_PREFIX."deal_cart set is_effect=1 , number = ".$num.",total_price=".$total_price.",return_total_money=".$return_total_money.",return_total_score=".$return_total_score." where user_id = ".$GLOBALS['user_info']['id']." and deal_id = ".$vv['deal_id']);
				                    $GLOBALS['db']->query("delete from ".DB_PREFIX."deal_cart where id=".$v['id']);
				                    break;
				            ...
				            if(intval($i)>=count($unlogin_cart)){
				                $GLOBALS['db']->query("update ".DB_PREFIX."deal_cart set is_effect=1 , user_id = ".$GLOBALS['user_info']['id']." where user_id = 0 and session_id = '".es_session::id()."'");
				                
				            }
				        }
				    }else{
				        $GLOBALS['db']->query("update ".DB_PREFIX."deal_cart set user_id = ".$GLOBALS['user_info']['id']." where user_id = 0 and session_id = '".es_session::id()."'");
				    }
				}
				
				require_once(APP_ROOT_PATH."system/model/cart.php");
				load_cart_list(0,true);
	 ...
	 * 
	 */
	function do_login_user($user_name_or_email,$user_pwd,$is_wap=false)
	{
		$user_data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where (user_name='".$user_name_or_email."' or email = '".$user_name_or_email."' or mobile = '".$user_name_or_email."') and is_delete = 0");
		...
		else
		{
			...
			else
			{
				...
				//扫描团购订单，检测结单
				syn_auto_over_status($user_data['id']);
				...
				foreach($medal_list as $medal)
				{
					...
					if(file_exists($file))
					{
						...
						if(class_exists($cls))
						{
							...
							if($check_result['status']==0)
							{
								send_msg_msg($user_data['id'], $check_result['info'], "notify", array('type' => 7));
								// send_msg($user_data['id'], $check_result['info'], "notify", 0);
							}
						}
					}
	...
	function refresh_sign_log(){
	    ...
	    if(!$y_sign_data) //签到中断清空记录，未中断不清空
	        $GLOBALS['db']->query("delete from ".DB_PREFIX."user_sign_log where user_id = ".$user_id." and sign_date not between ".$t_begin_time." and ".$t_end_time);
	}
	...
	function send_system_msg($user_id)
	{
		...
		foreach($msg_systems as $msg)
		{
			...
			{
				send_msg_new($user_id, $msg['content'], 'notify', array('type' => 10));
	 ...
	 * 手机号会员合并微信会员限制条件
	 */
	function bind_wx_user_info_check($user_info=array(),$wx_user_info=array())
	{
		$status=1;
		$info='';
		if($user_info['bind_count']>10){
			$status=0;
			$info='合并微信会员次数到达上限';
		}elseif($wx_user_info['is_merchant']==1){
			$status=0;
			$info='微信会员已绑定商家，无法合并';
		}
	 ...
	 * $wx_user_info：微信会员信息
	 * $wx_info：微信公众号信息
	 * $type:微信浏览器0，app1,绑定手机号2
	 */
	function bind_wx_user_info($user_info=array(),$wx_user_info=array(),$wx_info=array(),$type=0)
	{
		if($user_info&&$wx_user_info||$wx_info){
			$GLOBALS['db']->query("delete from ".DB_PREFIX."user where id = ".$wx_user_info['id']);
			//将微信会员的下级推荐人转移到手机号会员的下级
			$update = array();
			$update['pid']=$user_info['id'];
			$GLOBALS['db']->autoExecute(DB_PREFIX.'user', $update, 'UPDATE', 'pid='.$wx_user_info['id'].' and id!='.$user_info['id']);
			$update = array();
			if(ENTRY_NAME=='nzy_o2o'){
				//保留上级推荐人关系
				if($wx_user_info['pid']>0&&$wx_user_info['pid']!=$user_info['id']){
					//微信会员存在推荐人and微信会员的推荐人不是当前手机会员
					$update['pid']=$wx_user_info['pid'];
				}elseif($wx_user_info['pid']==0||$wx_user_info['pid']==$user_info['id']){
					//微信会员不存在推荐人or微信会员的推荐人是当前手机会员
					$update['pid']=0;
				}
				$update['user_name']=$wx_user_info['user_name'];
			}else{
				if($user_info['pid']==0&&$wx_user_info['pid']>0&&$wx_user_info['pid']!=$user_info['id']){
					//保留上级推荐人关系
					$update['pid']=$wx_user_info['pid'];
				}
			}
			...
			if($type==0){
				...
				$update['wx_openid']=$wx_info['wx_openid'];
				$update['m_openid']=$wx_info['m_openid'];
				$update['union_id']=$wx_info['union_id'];
			}
			...
			if($wx_user_info['money']>0||$wx_user_info['score']>0||$wx_user_info['point']>0){
				...
				modify_account($data,$user_info['id'],"在".to_date(NOW_TIME)."合并微信账号".$wx_user_info['user_name']);
			}
			if($wx_user_info['money']){//正常情况下，未绑定手机的会员，无法开通分销资格，，也就不会有分销余额
				//预留
			}
		}
	}
