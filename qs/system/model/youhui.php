...
define("YOUHUI_DOWNLOAD_LIMIT",8); //优惠券领取失败，达上限
...
function get_youhui_list($limit,$type=array(YOUHUI_NOTICE,YOUHUI_ONLINE,YOUHUI_HISTORY),$param=array("cid"=>0,"tid"=>0,"aid"=>0,"qid"=>0,"city_id"=>0), $join='', $where='',$orderby = '',$append_field="")
{
	...
	// 新添加的
	$join .= ' LEFT JOIN '.DB_PREFIX.'youhui_location_link l ON '.$tname.'.id = l.youhui_id ';
	...
	$append_field = $append_field. ', sl.name location_name,sl.id location_id, sl.address location_address, sl.avg_point location_avg_point, sl.district area_name ';
	...
	$condition_in = $condition." AND l.location_id IN ( SELECT location_id FROM
            			(
            				(
            					...
            					FROM
            						`".DB_PREFIX."youhui` y
            					LEFT JOIN ".DB_PREFIX."youhui_location_link dl ON dl.youhui_id = y.id
            					WHERE
            						dl.location_id <> ''
            					    AND ".$condition."
            					GROUP BY
	...
	if($join)
		$sql = "select ".$tname.".*".$append_field." from ".DB_PREFIX."youhui as ".$tname." ".$join." where  ".$condition." GROUP BY y.id";
	else
		$sql = "select ".$tname.".*".$append_field." from ".DB_PREFIX."youhui as ".$tname." where  ".$condition." GROUP BY y.id";
...
function wap_get_youhui_list($limit,$type=array(YOUHUI_NOTICE,YOUHUI_ONLINE,YOUHUI_HISTORY),$param=array("cid"=>0,"tid"=>0,"aid"=>0,"qid"=>0,"city_id"=>0), $join='', $where='',$orderby = '',$append_field="",$user_id="")
{
    if(empty($param))
        $param=array("cid"=>0,"tid"=>0,"aid"=>0,"qid"=>0,"city_id"=>0);
    ...
    // 新添加的
    if($user_id){
        $join .= " LEFT JOIN fanwe_user us ON us.id=".$user_id." ";
    }
    $join .= ' LEFT JOIN '.DB_PREFIX.'youhui_location_link l ON '.$tname.'.id = l.youhui_id ';
    $join .= ' LEFT JOIN '.DB_PREFIX.'supplier_location sl ON sl.id = l.location_id ';
    ...
    $append_field.="case 
     when (y.begin_time+28800) > unix_timestamp(now()) then 4
     when (y.end_time+28800)<unix_timestamp(now()) and y.end_time<>0 then 7
	 when y.total_num<=y.user_count then 6 ";
    if($user_id){
        $append_field.=" when us.score<y.score_limit then 2
		 when us.point<y.point_limit then 3
		 when (SELECT count(*) as count_number from fanwe_youhui_log yl where user_id=71 
	     and (create_time+28800)>UNIX_TIMESTAMP(DATE_FORMAT(CURDATE(),'%Y-%m-%d %H:%i:%s')) and youhui_id=y.id)
            >=y.user_limit and y.user_limit<>0 then 5 ";
    }
    
    $append_field.=" else 1 end order_status ";
    ...
    if($orderby=='')
        $sql.=" order by  order_status ASC,".$tname.".sort desc limit ".$limit;
    else
        $sql.=" order by order_status ASC,".$orderby." limit ".$limit;
	...
	if($youhui_info)
	{
		...
		else
		{
			if($youhui_info['user_everyday_limit']>0)
			{
				...
				{
					$data['status'] = YOUHUI_DOWNLOAD_LIMIT;
					$data['jump'] = wap_url("index","uc_youhui");
					$data['info'] = "每人每天最多领取".$user_day_count."张优惠券";
					return $data;
				}
			}
			...
			{
			    $user_day_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."youhui_log where user_id = ".$user_id." and youhui_id = ".$youhui_info['id']);
			    if($user_day_count>=$youhui_info['user_limit'])
			    {
					$data['status'] = YOUHUI_DOWNLOAD_LIMIT;
					$data['jump'] = wap_url("index","uc_youhui");
			        $data['info'] = "您已经领取了".$user_day_count."张了，留一点给别人吧~";
			        return $data;
			    }
			}
			/* 
			 ...
			 */
			//领取库存验证
			$sql  = "update ".DB_PREFIX."youhui set user_count = user_count + 1 where id = ".$youhui_info['id']." and (user_count + 1 <= total_num or total_num=0)";			
			...
			if($GLOBALS['db']->affected_rows()>0)
			{
				...
				if($youhui_info['expire_day']>0 && $youhui_info['valid_type']==1){
				    ...
				    $log['expire_time'] = $youhui_info['use_end_time'];
			    }else{
			        $log['expire_time'] = 0;
				}
				...
				while(intval($log['id'])==0)
				{
				    ...
				    $log['youhui_sn'] = '2' . substr(NOW_TIME, 1, 9). sprintf('%02s', rand(0, 99));
                ...
                getYouhuiStatistics($youhui_info['id'],$user_id,$is_update=1);
	    ...
	    if ($id>0) {
			...
			unset($data['data']['id']);
	            ...
	            if ($GLOBALS['db']->affected_rows()&&$GLOBALS['db']->error()=='') { 
	                ...
	                $GLOBALS['db']->query("delete from  ".DB_PREFIX."youhui_biz_submit where id=".$id);
	                return true;
 ...
 * @return int     
 */
function youhui_collect_count($id)
{
	$sql = 'SELECT COUNT(*) FROM '.DB_PREFIX.'youhui_sc WHERE youhui_id='.$id;
