...
class DealObject{
    ...
    private $presell_deposit_money=array();
    private $presell_discount_money=array();
    private $presell_buy_count=array();
    /**
     * @param $req
    ...
    public function setParamet($req,$type)
    {
        ...
        if(IS_PRESELL){
            $this->presell_deposit_money=$req['presell_deposit_money'];
            $this->presell_discount_money=$req['presell_discount_money'];
            $this->presell_buy_count=$req['presell_buy_count'];
        }
        //过滤掉改版后不要的数据
    ...
    public function setShop($req)
    {
        ...
        $this->buy_type = intval($req['buy_type']);
        //强制属性
    ...
    public function setScore($req)
    {
        ...
        $this->is_shop = 1;      //标识是否为商城商品 0:否 1:是
        $this->is_pick = 0;      //是否允许上门自提
        $this->buy_type = 1;      //团购商品的类型0：普通(团购、商品)，1：积分商品 2：精品商品
    ...
    private function saveParametAttrAndStock()
    {
        ...
        $presell_deposit_money=$this->presell_deposit_money;
        $presell_discount_money=$this->presell_discount_money;
        $presell_buy_count=$this->presell_buy_count;
        //属性的数量，确定遍历的级别

        //$GLOBALS['db']->query("delete from ".DB_PREFIX."deal_attr where deal_id=".$this->id);
        
        $deal_attr_info=$GLOBALS['db']->getAll("select id,name,goods_type_attr_id from ".DB_PREFIX."deal_attr where deal_id=".$this->id);
        $names=array();
        $type_ids=array();
        foreach ($deal_attr_info as $t => $v){
            $names[]=$v['name'];
            $type_ids[]=$v['goods_type_attr_id'];
        }
        ...
        foreach ($deal_attr as $k=>$v){
            foreach ($v as $kk=>$attr_name){
                if(!in_array($attr_name, $names) || !in_array($k, $type_ids)){
                    $ins_data = array();
        ...
        foreach ($deal_attr_info as $t => $v){
            if(in_array($v['name'],$new_name) && in_array($v['goods_type_attr_id'],$new_type)){
                $ids[] = $v['id'];
            }
        }
        
        $GLOBALS['db']->query("delete from ".DB_PREFIX."deal_attr where id not in  (".implode(",", $ids).") and deal_id=".$this->id);
        ...
        foreach ($group_attr as $k=>$v){
            ...
            $stock_data['presell_deposit_money'] =$presell_deposit_money[$k];
            $stock_data['presell_discount_money'] =$presell_discount_money[$k];
            $stock_data['presell_buy_count']=$presell_buy_count[$k];
