 ...
 */
function get_dc_location_list($type='is_dc',$limit='',$param=array("cid"=>0,"aid"=>0,"qid"=>0,"city_id"=>0),$tag=array(), $where='',$orderby = '',$append_field='',$join,$join_group)
{

	if(empty($param)){
		$param=array("cid"=>0,"aid"=>0,"qid"=>0,"city_id"=>0);
	}
	$time=to_date(NOW_TIME,'H')*3600+to_date(NOW_TIME,'i')*60;
	$tname='sl';
	...
	if($type=='is_dc'){
		...
		$condition =$tname.".is_dc=1 and ".$tname.".is_effect=1 ";
		...
		if($where != '')
		{
		 ...
		 $sql = "select aa.* , max(aa.in_opentime) as is_in_opentime from (select ".$field_str.$append_field.", case when b.begin_time_h is null then 1 when (b.begin_time_h*3600 + b.begin_time_m*60) < ".$time." and (b.end_time_h*3600 + b.end_time_m*60)  >".$time." then 1 else 0 end as in_opentime from ".DB_PREFIX."supplier_location as ".$tname." left join ".DB_PREFIX."dc_supplier_location_open_time as b on ".$tname.".id=b.location_id ".$join." where  ".$condition." ".$join_group." ) aa where aa.distance <= aa.max_delivery_scale or aa.max_delivery_scale=0 group by aa.id";
	...
	$condition2=$sql;
	if($orderby==''){
		$sql.=" order by aa.is_close , is_in_opentime desc, aa.id desc";
	}else{
		$sql.=" order by aa.is_close , is_in_opentime desc ,aa.".$orderby;		
	}
	if($limit!=''){
		$sql.=" limit ".$limit;	
	}
...
function get_lastest_search_name(){
	...
	$dc_search_history=dc_stripslashes(es_cookie::get('dc_search_history'));
 ...
 function dc_order_close($order_id,$is_cancel,$close_reason){
	...
	if($order_info['is_cancel']==0 && $order_info['refund_status']==0){
		...
		if($order_info['is_rs']==1){
			$msg='预订订单'.$order_info['order_sn'].'关闭,已付金额退回';
		}else{
			$msg='外卖订单'.$order_info['order_sn'].'关闭,已付金额退回';
		}
		...
		dc_order_log($order_info['order_sn']."订单关闭, ".format_price($return_money)."已退到会员余额", $order_id);
		}else{
			
		dc_order_log($order_info['order_sn']."订单关闭", $order_id);	
		}
		...
		dc_order_log($order_info['order_sn']."订单关闭, 代金券退还给用户", $order_id);
		}
		
		if($order_info['delivery_part']==2){  //达达配送

		    $GLOBALS['db']->query("update ".DB_PREFIX."dc_order set delivery_operation=2 where id=".$order_id);
...
*/			
			if($order_info['mobile'])
			{
				...
				$msg_data['user_id'] = $order_info['user_id'];
				$msg_data['is_send'] = 0;
...
function dc_confirm_delivery($order_id)
{
	...
	if($order_info)
	{
		if($order_info['refund_status'] > 0 || $order_info['is_cancel'] > 0){
			$result['status']=0;
			$result['info']='订单已关闭,不能确认';
		}else{
			if($order_info['confirm_status']==1){
				if($order_info['is_rs']==1){
					...
					$biz_msg['content']=$order_info['order_sn']."订单已收货,余额增加".$order_info['pay_amount']."元";
					$biz_msg['supplier_id'] = $order_info['supplier_id'];
					$biz_msg['create_time']=NOW_TIME;
					$biz_msg['type']='account';
					$biz_msg['data']=serialize(array('type'=>1,'ctl' => 'withdrawal#money_log', 'title' =>23));
					$GLOBALS['db']->autoExecute(DB_PREFIX.'biz_msg_box', $biz_msg);
					
					
					
					$biz_money_log['log_info']=$order_info['order_sn']."订单已收货";
					$biz_money_log['supplier_id'] = $order_info['supplier_id'];
					$biz_money_log['create_time']=NOW_TIME;
					$biz_money_log['money']=$order_info['pay_amount'];
					$biz_money_log['type']=3;
					$GLOBALS['db']->autoExecute(DB_PREFIX.'dc_supplier_money_log', $biz_money_log);
					
					
					if($order_info['delivery_part'] > 1){
					    if($order_info['delivery_part']==2){
					        $delivery_part = '达达';
					    }elseif($order_info['delivery_part']==3){
					        $delivery_part = '蜂鸟';
					    }
					    $info = '外卖订单：'.$order_info['order_sn'].'收货成功，扣除'.$delivery_part.'配送费';			    
					    dc_balance_delivery_fee($order_info['supplier_id'] , $order_info['thirdpart_delivery_fee'] ,$info);
...
function dcorder_paid_done($order_id)
{
	...
	require_once(APP_ROOT_PATH."system/model/dc.php");
	$order_id = intval($order_id);
	$order_info = $GLOBALS['db']->getRow("select do.*,sl.delivery_type from ".DB_PREFIX."dc_order as do left join ".DB_PREFIX."supplier_location as sl on do.location_id=sl.id where do.id = ".$order_id);
	if($order_info)
	{
		...
		if($order_info['is_rs']==1 && $order_info['is_cancel']==0){
			...
			foreach($location_dc_table_cart['cart_list'] as $k=>$v){
				...
				if(!$rs_info && $total_count==0){
					...
					$data['is_used']=0;
					$data['sn'] = '5' . substr(NOW_TIME, 1, 9). sprintf('%02s', rand(0, 99));
        ...
        if($order_info['is_rs']==1){
            AppPush::biz_new_dc_rs_order($order_info);
        }else{
            AppPush::biz_new_dc_order($order_info);
        }
		...
		//	更新商户销售记录
		$dc_supplier_order_x=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."dc_supplier_order where order_id=".$order_info['id']);
		
		if(!$dc_supplier_order_x){
		...
		do
		{
			...
			$order_id_s = intval($GLOBALS['db']->insert_id());
		}while($order_id_s==0);
		...
		//  更新平台报表
		 ...
		 // $delivery_type'外卖配送方式：0为自动接单，并由商家配送，1为手动接单，手动指派，2为自动接单，并由达达配送',
		$delivery_type = $order_info['delivery_type'];
	    if($delivery_type==0){
	        $GLOBALS['db']->query("update ".DB_PREFIX."dc_order set delivery_part=1  where id = ".$order_id);
	        $GLOBALS['db']->query("update ".DB_PREFIX."dc_supplier_order set delivery_part=1 where order_id = ".$order_id);
	        $rs=$GLOBALS['db']->affected_rows();
	
	    }elseif($delivery_type==2){
	        $GLOBALS['db']->query("update ".DB_PREFIX."dc_order set delivery_part=2  where id = ".$order_id);
	        $GLOBALS['db']->query("update ".DB_PREFIX."dc_supplier_order set delivery_part=2 where order_id = ".$order_id);
	        $rs=$GLOBALS['db']->affected_rows();
	    }

		if($rs > 0){

		    dc_accept_order($order_id);//接单
		}	
...
function dc_supplier_balance($supplier_id){
	...
	$count=0;
	if($balance_total > 0){
		$supplier_info=$GLOBALS['db']->getAll("select balance_price , order_sn,order_id from ".DB_PREFIX."dc_supplier_order where order_status=0 and confirm_status=2 and refund_status=0 and is_cancel=0 and supplier_id=".$supplier_id);
		...
		foreach($supplier_info as $k=>$v){
			...
			$rs=$GLOBALS['db']->affected_rows();
			if($rs>0){
				$info='订单'.$v['order_sn'].'结算成功,结算'.round($v['balance_price'],2)."元";
				dc_modify_supplier_account($v['balance_price'],$supplier_id,3,$info,0);  //商户余额增加
				$count++;
			}
...
function dc_modify_supplier_account($money,$supplier_id,$type,$info,$order_id=0)
{
	if($type>=0&&$type<6)
	{
		...
		$agency_id=intval($GLOBALS['db']->getOne("select agency_id from ".DB_PREFIX."supplier where id=".$supplier_id));
		...
		if($supplier_info)
		{
			...
			/* 
			 *  1.订单下单完成   营业额增加 ，未完成增加， 营业额=未完成=在线支付+活动补贴+代金券   type=0
			...
			if($type==0||($type==3&&$money>0)||$type==4||($type==1 && $money<0))
			{
				...
				if($supplier_stat)
				{
					if($type==0){  //订单下单完成   营业额增加 ，未完成增加， 营业额=未完成=在线支付+活动补贴+代金券
						...
						//代理商佣金增加
						$order_info_x = $GLOBALS['db']->getRow("select total_price,balance_price from ".DB_PREFIX."dc_order where id = ".$order_id);
						modify_agency_account($order_info_x['total_price']-$order_info_x['balance_price'],$agency_id,2,$info);
			...
			/*
			 *  1.订单下单完成   平台营业额增加 ，营业额=在线支付+活动补贴+代金券   type=0
...
function dc_modify_statements($money,$type,$info,$order_id=0)
{
		...
		*/
		...
		if($state_data)
		{
			if($type==0){  //订单下单完成   平台营业额增加 ，营业额=在线支付+活动补贴+代金券   type=0
				$order_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."dc_order where id = ".$order_id);
...
function dc_send_supplier_order($supplier_id,$order_id)
{
		...
		if($order_data)
		{
			...
			$msg_data['order_id']=$order_id;
			$msg_data['order_type']=0;
			$msg_data['content'] = addslashes($msg);
			$msg_data['send_time'] = 0;
			$msg_data['is_send'] = 0;
			$msg_data['create_time'] = NOW_TIME;
			...
			//站内消息
			// 站内消息
			$ctl = ($order_data['is_rs'] ? 'biz_dc_resorder' : 'biz_dc_order').'#view';
			$biz_msg = array(
				'content' => addslashes($msg),
				'supplier_id' => $supplier_id,
				'create_time' => NOW_TIME,
				'type' => 'order',
				'data' => serialize(array('data_id' => $order_id, 'ctl' => $ctl, 'title' => 21, 'ext' => array('lid' => $order_data['location_id']))),
			);
			$GLOBALS['db']->autoExecute(DB_PREFIX.'biz_msg_box', $biz_msg);
	...
	//$dc_type大等于0为预订方式，不享受促销优惠，-1代表享受促销优惠,$paid_promote_amount>0时，代表已经享受过优惠，不再享受优惠
		if($dc_type==-1 && $paid_promote_amount ==0){
			...
			if(count($promote_list)>0){
				...
				//判断是否是首单
				$user_order_count=$GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."dc_supplier_order where user_id=".$user_id);
	...
	{		
		...
		{		$order_menu=unserialize($order_info['order_menu']);
				...
				foreach($cart_list as $kc=>$vc){ //定餐桌时间
					...
					$result['status'] = 1;
					$result['type'] = 5;
					$result['consignee'] = $order_info['consignee'];
					$result['mobile'] = $order_info['mobile'];
					$result['total_price'] = $order_info['total_price'];
					$result['dc_comment'] = $order_info['dc_comment'];
					$result['table_time_format'] = $order_info['table_time_format'];
					$result['table_name'] = $order_info['table_name'];
					$result['location_name'] = $order_info['location_name'];
					$result['menu_list'] = $order_menu['menu_list'];
...
// 					$result['msg'] = $order_info['location_name']."[".$order_info['table_name'].",".$order_info['table_time_format']."]电子券使用成功。\n 使用时间为：".to_date($now);
					$result['msg'] = "消费成功。使用时间为：".to_date($now);
					$GLOBALS['db']->query( "INSERT INTO `".DB_PREFIX."use_coupon_log` (`uid`, `data_id`, `name`, `pwd`, `type`, `create_time`, `location_id`) VALUES ( '{$order_info['user_id']}', '{$order_info['id']}', '【{$order_info['location_name']}】 {$order_info['table_time_format']} {$order_info['table_name']}', '{$sn}', '5', '{$now}', '{$location_id}')" );
...
function get_order_state($order_info,$type='wap',$page=''){
		...
		if($order_info['payment_id']==0){  //在线支付
			...
			if($order_info['pay_status']==0){
				...
				if($order_info['confirm_status']==0){
					...
					$order_state['state']=3;
					if($order_info['is_rs']==0){
					    $order_state['state_format']='制作配送中';
					}else{
...
function get_biz_order_state_new($order_info,$type='mapi'){
            ...
            if($order_info['pay_status']==0){
                ...
                if($order_info['confirm_status']==0){
                    ...
                    if($type=='wap'){
                        ...
                        $act['name']='接单';
                        $act['class']='j-quick';
                        $act['url']=wap_url('biz',$module.'#accept_order',array('id'=>$order_info['id']));
                        $order_state['act'][]=$act;
                        	
                        $act=array();
                        $act['name']='交易关闭';
                        $act['class']='j-cancle';
                        $act['url']=wap_url('biz',$module.'#close_order',array('id'=>$order_info['id']));
                        $act['has_reason']=1;
                        $order_state['act'][]=$act;
                    ...
                    if($type=='wap'){
                        ...
                        if($order_info['is_rs']==0 && NOW_TIME > $over_time){
                            ...
                            $act['class']='j-quick';
                            $act['name']='确认完成';
                            $act['url']=wap_url('biz',$module.'#over_order',array('id'=>$order_info['id']));
                            $order_state['act'][]=$act;
                        }
                        $act=array();
                        $act['name']='交易关闭';
 ...
 */
function timeout_accept_order_process(){

	$order_info_all=$GLOBALS['db']->getAll("select * from ".DB_PREFIX."dc_order where confirm_status=0 and ((pay_status=1 and payment_id=0) or payment_id=1) and is_cancel=0 and refund_status=0");
 ...
 */
function timeout_pay_order_process(){

	$order_info_all=$GLOBALS['db']->getAll("select id from ".DB_PREFIX."dc_order where confirm_status=0 and pay_status=0 and payment_id=0 and is_cancel=0 and refund_status=0 and ".NOW_TIME." - create_time > 900");
...
/**
 * 外卖异常订单
 * 第三方配送端未接单，包括配送员取消订单，长期未接单，订单过期等，此种状态下，商家和平台可选择手动推单给配送端或者商家自己配送
 * @param unknown $order_id
 */
function delivery_part_not_accept_order($order_id,$reason){
    $GLOBALS['db']->query("update ".DB_PREFIX."dc_order set is_delivery_cancel=1 , delivery_cancel_reason='".$reason."' where id=".$order_id);
    $GLOBALS['db']->query("update ".DB_PREFIX."dc_supplier_order set is_delivery_cancel=1 , delivery_cancel_reason='".$reason."' where id=".$order_id);
}

/**
 * 外卖接单，type=0,第一次推单，type=1,重复推单
 * @param unknown $order_id
 * @param number $type
 */
function dc_accept_order($order_id){

    $order_info = $GLOBALS['db']->getRow("select do.* ,s.delivery_money , s.is_open_dada_delivery  from ".DB_PREFIX."dc_order as do 
        left join ".DB_PREFIX."supplier as s on do.supplier_id=s.id where do.id = ".$order_id);
    
    $delivery_min_money = app_conf('DELIVERY_MIN_MONEY');
    $delivery_alarm_money = app_conf('DELIVERY_ALARM_MONEY');

    if($order_info['is_rs']==0 && $order_info['delivery_part']>0){  

        if($order_info['delivery_part']==1 ){  //1为商家配送
            $GLOBALS['db']->query("update ".DB_PREFIX."dc_order set confirm_status=1 ,is_delivery_cancel=0, delivery_part=1 where id=".$order_id);
            $GLOBALS['db']->query("update ".DB_PREFIX."dc_supplier_order set confirm_status=1 ,is_delivery_cancel=0, delivery_part=1 where order_id=".$order_id);
        }elseif($delivery_min_money <= $order_info['delivery_money'] ){  //商家配送余额充足
            if(IS_OPEN_DADA==1 && $order_info['is_open_dada_delivery']==1 && $order_info['delivery_part']==2){  //开通达达，并且选择达达 配送
                //自动接单，并由达达配送
                
                $GLOBALS['db']->query("update ".DB_PREFIX."dc_order set confirm_status=1 , delivery_operation=1 where id=".$order_id);             
                $GLOBALS['db']->query("update ".DB_PREFIX."dc_supplier_order set confirm_status=1 where order_id=".$order_id);
...
//给商家发送配送费充值预警信息
function dc_alarm_supplier_delivery_fee($supplier_id)
{
    ...
    $is_send = 0;
    if($delivery_alarm_money >= $supplier_info['delivery_money'] ){  
        $is_send = 1;
    }elseif($delivery_alarm_money*0.8 >= $supplier_info['delivery_money'] ){  
        $is_send = 1;
    }elseif($delivery_alarm_money*0.6 >= $supplier_info['delivery_money'] ){  
        $is_send = 1;
    }
    ...
    if( $is_send ==1)
    {
        ...
        $msg = '您的配送帐户可用余额已不足'.round($delivery_alarm_money,2).'元，为不影响您的外卖配送，请及时充值.';
        $msg_data['send_type'] = 0;
        $msg_data['dest'] = $account['mobile'];
        $msg_data['order_type']=0;
        $msg_data['content'] = addslashes($msg);
        ...
        if(app_conf("SMS_ON")==1 && $account['mobile'])
        {          	  	
                $msg_data['id'] = $GLOBALS['db']->insert_id();
                send_msg_item($msg_data);
...
function dc_balance_delivery_fee($supplier_id , $money ,$info){
  if($money > 0){
        $sql = "update ".DB_PREFIX."supplier set delivery_money = delivery_money - ".$money." where id = ".$supplier_id;
 ...
 * @param unknown $order_id
 */
function get_dc_delivery_info($order_id){
    $order_info = $GLOBALS['db']->getRow("select do.* ,s.delivery_money , s.is_open_dada_delivery  from ".DB_PREFIX."dc_order as do
        left join ".DB_PREFIX."supplier as s on do.supplier_id=s.id where do.id = ".$order_id);
    ...
    if($order_info){
        if($order_info['is_cancel']==0){
            if($order_info['confirm_status']==0){
                ...
                if($order_info['delivery_part']==1){
                    $delivery_info='商家配送';
                }elseif($order_info['delivery_part']==2){
                    $dada_delivery = unserialize($order_info['dada_delivery']);
//                     $dada_delivery = array('client_id'=>'',  //预留字段，默认为空
//                       'order_id'=>'20170203084328641',       //添加订单接口中的origin_id值
//                       'order_status'=>2,   //订单状态(待接单＝1 待取货＝2 配送中＝3 已完成＝4 已取消＝5 已过期＝7 指派单=8 可参考文末的状态说明）
...
//                       'dm_name'=>'zhang san',        //配送员姓名，接单以后会传
...
//                       );
                    ...
                    $delivery_info='达达配送';
                    if($dada_delivery){
                       $change_button=' <button onclick="change_delivery(this);" data-id="'.$order_info['id'].'">更改</button>';
                       if($dada_delivery['order_status']==1){  //达达待接单
                           $no_confirm_time = ceil((NOW_TIME - $order_info['create_time'] ) / 60);
                           $delivery_info.=' ( 已'.$no_confirm_time.'分钟未接单 )'.$change_button;
                       }elseif($dada_delivery['order_status']==2 || $dada_delivery['order_status']==3){  //2:达达待取货,3:达达配送中
                           if($dada_delivery['order_status']==2){
                             $str = '取货中 ';
                           }else{
                             $str = '配送中 ';
                           }
                           $delivery_info.=' [ 配送员 '.$dada_delivery['dm_name'].' ('. $dada_delivery['dm_mobile'].') '.$str.' ]'.$change_button;
                       }elseif($dada_delivery['order_status']==4){  //达达已完成
                           $delivery_info.=' ( 配送员 '.$dada_delivery['dm_name']. $dada_delivery['dm_mobile'].' 已送达 )';
                       }elseif($dada_delivery['order_status']==5 || $dada_delivery['order_status']==7){  //2:达达待取货,3:达达配送中
                           if($dada_delivery['order_status']==5){
                             $str = '已取消 ';
                           }else{
                             $str = '已过期 ';
                           }
                           $delivery_info.=' ( 订单'.$str.' )';
                       }elseif($dada_delivery['order_status']==8){  //指派单
                           $delivery_info.=' ( 指派单 )';
                       }  
                    }
                }elseif($order_info['delivery_part']==3){
    ...
    $result['confirm_info'] = $confirm_info;
