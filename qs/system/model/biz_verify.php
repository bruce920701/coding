    ...
    switch ($type) {
        case 1:
            ...
            if ($check_result['status'] == 1 ) {
                ...
                $result['coupon_data']['deal_type']     = $check_result['coupon_data']['deal_type'];
                $result['coupon_data']['deal_number']     = $check_result['coupon_data']['deal_number'];
...
function biz_check_coupon($s_account_info,$pwd,$location_id, $no_check_location=0)
{
		...
		$coupon_data['deal_number'] = $coupon_data['number'] ;
...
function biz_check_coupon_batch($s_account_info,$location_id,$coupon_pwds)
{
	...
	foreach($coupon_pwds as $k=>$v){
		...
		if($coupon_data)
		{
			...
			if($coupon_data['is_valid'] == 2 && $is_err ==0){//改团购券因为退款被锁定
...
function biz_check_event($s_account_info,$sn,$location_id, $no_check_location=0){
    ...
    if($s_account_info['id']<=0){
        ...
        if ($event['event_end_time']){
            $result['msg'].= "，该活动的结束时间为:".$event['event_end_time'];
        }
        $result['event_submit'] = $event_submit;
        $end_time=$event_submit['end_time']?to_date($event_submit['end_time']):"永久";
        $result['msg']=$event_submit['name']."<br/>有效期：".$end_time.
