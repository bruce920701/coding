 ...
 * 生成平台的分销报表
 * @param unknown_type $money
 * @param unknown_type $type 0:营业销 1分销佣金 2推广佣金 3分销提现 4开通分销资格佣金 5推荐商家出售商品和团购产生的佣金 6推荐商家到店买单佣金 7推荐商品返佣
 ...
 */
function update_fx_order_item_log($order_item_id,$salary,$level,$user_id=0,$user_name='')
{
	...
	$fx_salary_all[$level] = array('user_id'=>$user_id,'user_name'=>$user_name,'salary'=>$salary);
...
function get_fx_withdraw($limit,$user_id)
{
	...
	$list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."fx_withdraw where user_id = ".$user_id." and is_delete=0 order by create_time desc limit ".$limit);
	foreach($list as $k=>$v)
	{
...
{
	// 先判断订单是否存在并获取用户信息
	$order_sql = 'SELECT user_id FROM '.DB_PREFIX.'deal_order WHERE is_presell_order=0 and id = '.$order_id;
	...
	if (count($order_item) > 0) {
		$user_info_sql = 'SELECT id, pid,user_name FROM '.DB_PREFIX.'user WHERE id='.$user_id;
		...
		if ($current_user['pid'] > 0) {
			...
			for ($i=0; $i < 3; $i++) { 
				...
				foreach ($order_item as $deal) {
					...
					if ($fx_salary) {
						$rate = $fx_salary['fx_salary_type'] ? $deal['total_price'] : $deal['number'];
					}
					...
					if ($salary > 0) {
						...
						$salary_log_format = '推荐会员【%s】购买了【%s】,获得%s级推荐会员佣金';
						$a=array('一','二','三');
						$salary_log = sprintf($salary_log_format, $user_info['user_name'],$deal['name'],$a[$i]);
						...
						if ($send_status) {
							...
							// 更新订单商品的分销佣金信息
							update_fx_order_item_log($deal['id'], $salary, $i + 1,$p_user['id'],$p_user['user_name']);
							if(!$arr[$deal['id']]){
								$arr[$deal['id']]=true;
...
function fx_buy_over_order($order_id)
{
    ...
    if($order_info)
    {
        ...
        if(!empty($pid)&& $order_info['total_price']>0){
            ...
            fx_user_reward($user['pid'],$user['id'],$order_info['rebate']);
        }
...
function send_user_supplier_salary($order_id,$type=0){
	...
	if($type==0 && ($order_info['type']==5||$order_info['type']==6)){  //充值订单不返佣金
		...
		$log_data='推荐商家【'.$supplier_info['name'].'】，售出了【'.$deal_order_items[0]['name'].'】等'.$num.'件商品，获得%s级推荐商家佣金';
		$statements_type=5;
	}elseif($type==1){  //买单订单
		$total_price=$order_info['total_price'];
		$log='推荐商家【'.$supplier_info['name'].'】，成交了买单订单'.$order_info['order_sn'].'，获得推广佣金';
		$log_data='推荐商家【'.$supplier_info['name'].'】，成交了买单订单'.$order_info['order_sn'].'，获得%s级优惠买单佣金';
		$statements_type=6;
	}
	...
	if($user_lever_1_id){
		...
		if($user_lever_1_info['is_fx']==1 && $user_lever_1_info['is_effect']==1 && $user_lever_1_info['is_delete']==0){ //给第一级发放推荐商家入驻佣金
			...
			$is_send1 = send_fx_user_salary_new($user_lever_1_id, $salary,sprintf($log_data,'一'),$total_price);
			if($is_send1){
				fx_user_reward($arr['user_id'],0,$salary,$supplier_info['id']);
				$ref_salary_total+=$salary;
				$arr['salary']=$salary;
				modify_fx_statements($salary,$statements_type,sprintf($log_data,'一'),$user_lever_1_id);
			}
		}
		...
		if($user_lever_1_info['pid']>0){
			...
			if($user_lever_2_info['is_fx']==1 && $user_lever_2_info['is_effect']==1 && $user_lever_2_info['is_delete']==0){ //给第二级发放推荐商家入驻佣金
				...
				$is_send2 = send_fx_user_salary_new($user_lever_2_info['id'], $salary, sprintf($log_data,'二'),$total_price);
				if($is_send2){
					fx_user_reward($arr['user_id'],0,$salary,$supplier_info['id']);
					$ref_salary_total+=$salary;
					$arr['salary']=$salary;
					modify_fx_statements($salary,$statements_type,sprintf($log_data,'二'),$user_lever_2_info['id']);
				}
			}
			...
			if($user_lever_2_info['pid']>0){
				...
				if($user_lever_3_info['is_fx']==1 && $user_lever_3_info['is_effect']==1 && $user_lever_3_info['is_delete']==0){ //给第三级发放推荐商家入驻佣金
					...
					$is_send3 = send_fx_user_salary_new($user_lever_3_info['id'], $salary, sprintf($log_data,'三'),$total_price);
					if($is_send3){
						fx_user_reward($arr['user_id'],0,$salary,$supplier_info['id']);
						$ref_salary_total+=$salary;
						$arr['salary']=$salary;
						modify_fx_statements($salary,$statements_type,sprintf($log_data,'三'),$user_lever_3_info['id']);
					}
				}
...
function send_recommend_user_money($order_item_ids,$deal_coupon_id,$nocoupon=false){
            ...
            if($order_item_id > 0){
                ...
                if($order_item && $deal_data['recommend_user_return_ratio']>0){                   
                    ...
                    if($user_info['is_fx']==1  && $rs){
						...
						modify_fx_statements($recommend_money, 7, $log, $deal_data['recommend_user_id']);
						modify_fx_statements($order_item['total_price'], 0, $log, $deal_data['recommend_user_id']);
                    }
                }
            }
        }
    ...
    if($deal_coupon_id > 0 && !$nocoupon){
        ...
        if($user_info['is_fx']==1 && $deal_data['recommend_user_return_ratio']>0 && $rs){
            ...
            $log = '推荐团购：'. $coupon_data['name'] .'卖出，团购券'.$coupon_data['password']."验证消费成功，获得推荐佣金";
            // modify_fx_account($recommend_money, $deal_data['recommend_user_id'],$log);
            send_recommend_user_money2($coupon_data['coupon_price'], $recommend_money, $deal_data['recommend_user_id'], $log);
			modify_fx_statements($recommend_money, 7, $log, $deal_data['recommend_user_id']);
			modify_fx_statements($coupon_data['coupon_price'], 0, $log, $deal_data['recommend_user_id']);
        }
 ...
 */
function get_fx_withdraw_money($user_id){
	...
	$withdraw_money=floatval($GLOBALS['db']->getOne("select sum(money) as withdraw_money from ".DB_PREFIX."fx_statements_log where type in (2,4,5,6,7) AND stat_time > '".$withdraw_start_day ."' and stat_time<='".$withdraw_day."' and user_id=".intval($user_id)));
	//N天内不可提现金额
 ...
 *  会员下线商家
 */
function get_supplier($user_id,$limit){
	$result['list']= $GLOBALS['db']->getAll("SELECT * from (select s.name,s.id from ".DB_PREFIX."user u,".DB_PREFIX."supplier s where INSTR(u.path,'.".$user_id.".')>0 and u.id!=".$user_id." and u.id=s.user_id) su LEFT JOIN ".DB_PREFIX."fx_user_reward fur on fur.supplier_id=su.id and fur.pid=".$user_id." ORDER BY money desc limit ".$limit);
	$result['count']= $GLOBALS['db']->getOne("SELECT count(*) from (select s.name,s.id from ".DB_PREFIX."user u,".DB_PREFIX."supplier s where INSTR(u.path,'.".$user_id.".')>0 and u.id!=".$user_id." and u.id=s.user_id) su LEFT JOIN ".DB_PREFIX."fx_user_reward fur on fur.supplier_id=su.id ");
	return $result;
}
