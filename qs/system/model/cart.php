...
function load_cart_list($deal_id=0,$wap_show_disable=false)
{
    ...
    $base_sql = "select c.*,d.icon,d.id as did,d.delivery_type,d.current_price,d.uname as duname,d.is_delivery as is_delivery,d.allow_promote as allow_promote,d.user_min_bought as user_min_bought,d.user_max_bought as user_max_bought,d.is_shop,d.allow_user_discount,d.presell_deposit_money,d.presell_discount_money,d.presell_type,d.presell_delivery_time,d.is_presell,d.presell_begin_time,d.presell_end_time from ".DB_PREFIX."deal_cart as c left join ".DB_PREFIX."deal as d on c.deal_id = d.id where ";
...
function count_buy_total($region_id,$consignee_id,$payment,$account_money,$all_account_money,$ecvsn,$ecvpassword,$goods_list,$paid_account_money = 0,$paid_ecv_money = 0,$bank_id = '',$all_score=0,$paid_exchange_money=0,$youhui_ids=array(),$paid_youhui_money=0)
{
	...
	foreach($goods_list as $k=>$v)
	{
		...
		else if($v['buy_type'] == 2)$buy_type=2;
		$deal_user_discount = 0;
		if ($v['allow_user_discount'] && $is_presell==0) {
		...
		$presell_deposit_money=0;
		$presell_discount_money=0;
		$presell_end_time=0;
		$presell_type = 0;
		if($is_presell==1){
		    ...
		    if($v['attr']==0){		        
		        ...
		        $presell_deposit_money = floatval($presell_deposit_money);
		        $presell_discount_money = floatval($presell_discount_money);
	...
	if($is_presell==1){  //预售不可使用红包，积分兑换，优惠劵
	    $ecvsn='';
	    $ecvpassword='';
	    $all_score=0;
	    $youhui_ids=array();
	...
	//logger(__LINE__.":".$is_presell);


	$pay_price = $total_price + $delivery_fee; //加上运费
	$presell_left_price = 0; //商品尾款
	...
	$user_discount = 0;*/
	//logger::write($user_discount);
	$pay_price = $pay_price - $user_discount; //扣除用户折扣
	//余额支付
	$user_money = $GLOBALS['db']->getOne("select register_credits from ".DB_PREFIX."user where id = ".$user_id);
...
function payment_paid($payment_notice_id)
{
	...
	{		
		...
		{
		    if($order_info['is_presell_order']==1 && $order_info['pay_status']==1){//预售订单，并且订金已经支付
		        $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set pay_amount = total_price ,account_money = account_money + ".$payment_notice['money'] ." where id = ".$payment_notice['order_id']." and is_delete = 0 and order_status = 0 ");
		    }else{
...
// 			}
			if($order_info['is_presell_order']==1 && $order_info['pay_status']==1){//预售订单，并且订金已经支付
			    $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set pay_amount = total_price where id = ".$payment_notice['order_id']." and is_delete = 0 and order_status = 0 ");		    
			}else{
...
{	
		...
		if($order['is_presell_order']==1){
		    ...
		    { //预付款已支付
		        ...
		        //增加商品预售数量
		        syn_presell_deal_buy_count($order_id);
		        $result = true;  //订单未支付成功
                if(intval(app_conf("CONF_PRESELL_SEND_MESSAGE"))==1){
                    ...
                    $schedule->send_schedule_plan("sms","消息任务「定时发送手机短信」",array("type"=>0,"dest"=>$order['mobile'],"content"=>"{$name_array['user_name']}您好，您预订的{$name_array['name']}可以开始支付尾款了，请在7天内前往支付尾款"),$order['presell_end_time'],"{$name_array['user_name']}手机:{$order['mobile']}");
                    $schedule->send_schedule_plan("sms","消息任务「定时发送站内信」",array("type"=>1,"dest"=>$order['user_id'],"content"=>"{$name_array['user_name']}您好，您预订的{$name_array['name']}可以开始支付尾款了，请在7天内前往支付尾款"),$order['presell_end_time'],"{$name_array['user_name']}用户ID:{$order['user_id']}");
                }
        ...
        $order_money = $order['total_price'];

        $sql = "select * from ".DB_PREFIX."user where id=".$order['user_id'];
        $user_info = $GLOBALS['db']->getRow($sql);
		if($user_info['register_credits'] < $order_money)
		{
		    return false;
		}
        $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set pay_status = 2,pay_amount=".$order_money." where id =".$order_id." and pay_status <> 2");
		...
		*/
        $new_register_credits = $user_info['register_credits']-$order_money;
        //$sql = "update ".DB_PREFIX."user set register_credits = register_credits - ".$order_money." where  id =".$order['user_id];
        //$GLOBALS['db']->query("update ".DB_PREFIX."user set register_credits = register_credits - {$order_money} where  id =".$order['user_id']);
        $sql = "update ".DB_PREFIX."user set register_credits=".$new_register_credits." where id=".$order['user_id'];
        //echo $sql;
        ...
        {
            $new_avalible_consume_credits = $user_info['avalible_consume_credits'] - abs($order['return_total_score']);
            $sql = "update ".DB_PREFIX."user set avalible_consume_credits=".$new_avalible_consume_credits." where id=".$order['user_id'];
            //echo $sql;
            $GLOBALS['db']->query($sql);
        }
		return $result;
...
function store_pay_order_paid_done($order_id)
{
	...
	$order_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."store_pay_order where id = ".$order_id);
		...
		modify_supplier_account($tatal_money,$order_info['supplier_id'],0,$msg); //商户销售额增加
...
function syn_presell_deal_buy_count($order_id){
     ...
     * @param unknown $order_id
     */
    function syn_presell_deal_stock($order_id){
        //预售订单，把库存加回去
        $order_goods_list = $GLOBALS['db']->getAll("select doi.number , doi.deal_id , doi.attr_str, d.max_bought , ds.stock_cfg from ".DB_PREFIX."deal_order_item as doi left join ".DB_PREFIX."deal as d on doi.deal_id=d.id left join ".DB_PREFIX."deal_stock as ds on doi.deal_id=ds.deal_id where doi.order_id = ".$order_id);
        foreach($order_goods_list as $k=>$v)
        {
            
            if($v['max_bought']>=0){
                $sql = "update ".DB_PREFIX."deal set max_bought = max_bought + ".$v['number']." where id=".$v['deal_id'];            
                $GLOBALS['db']->query($sql);
            }
            if($v['stock_cfg']>=0){
                $sql = "update ".DB_PREFIX."deal_stock set stock_cfg = stock_cfg + ".$v['number']." where deal_id=".$v['deal_id'];
                $GLOBALS['db']->query($sql);
            }            
            $attr_stock = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."attr_stock where deal_id = ".$v['deal_id']." and locate(attr_str,'".$v['attr_str']."')");
            if($attr_stock)
            {
                if($attr_stock['stock_cfg']>=0)
                {
                    $sql = "update ".DB_PREFIX."attr_stock set stock_cfg = stock_cfg + ".$v['number']." where deal_id = ".$v['deal_id'];
                }
                $GLOBALS['db']->query($sql);
            }
        }
