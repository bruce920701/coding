...
function use_coupon($password,$location_id=0,$account_id=0,$send_return=false,$send_notify=false)
{
	...
	{		
        ...
        $user_info=$GLOBALS['db']->getRow("select id,user_name,mobile from ".DB_PREFIX."user where id={$order_info['user_id']}");
        $sub_name=$GLOBALS['db']->getOne("select sub_name from ".DB_PREFIX."deal where id={$coupon_data['deal_id']}");
        AppPush::singleUserPush("use_coupon",$order_info['user_id'],lang("APP_PUSH_ORDER_USE_COUPON",$user_info['user_name'],$sub_name,$order_info['order_sn'],to_date(NOW_TIME)),0);
        //发短信
        $msg_data['dest'] = $user_info['mobile'];
        $msg_data['send_type'] = 0;
        $msg_data['content'] = lang("APP_PUSH_ORDER_USE_COUPON",$user_info['user_name'],$sub_name,$order_info['order_sn'],to_date(NOW_TIME));
        $msg_data['send_time'] = 0;
        $msg_data['is_send'] = 0;
        $msg_data['create_time'] = NOW_TIME;
        $msg_data['user_id'] = $user_info['id'];
        $msg_data['is_html'] = 0;
        send_msg_item_add(0,$user_info,$msg_data);
...
function order_confirm_delivery($delivery_sn,$express_id,$order_id)
{
    ...
    if($order_info)
    {
          ...
          if($GLOBALS['db']->affected_rows())
        {
			...
			require_once(APP_ROOT_PATH."system/model/fx.php");
//            fanwe_require(APP_ROOT_PATH.'system/model/AppPush.php');
//            AppPush::confirmDelivery($order_info);
...
function distribution_confirm_delivery($order_id){
    ...
    if($order_info['type']==4){  //驿站订单
        ...
        if($GLOBALS['db']->affected_rows()){
            ...
            update_order_cache($order_info['id']);
            distribute_order($order_info['id']);
            $is_dist_cod_balance=intval(app_conf('IS_DIST_COD_BALANCE'));
			if($order_info['cod_money']>0&&$is_dist_cod_balance==0){
				//货到付款
			}else{
                ...
                if(1){
                    ...
                    Distribute::distribution_order_fee($type=2,$distribution_fee,$order_info);
                }
			}
...
{	
	...
	if($order_info)
	{
		...
		if(defined("FX_LEVEL")&&$order_info['pay_status']==2 && $order_info['is_presell_order']==0)// 预售订单不参与分销分红
		{
...
{
	$order_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_order where id = ".$order_id." and (order_status = 1 or pay_status = 0)");
	if($order_info)
	{
		unset($order_info['id']);
		...
		if($data['refund_status']!=2)
		{
			...
			{					
			   ...
			   // $scale_discount_price = $scale * $order_info['discount_price'];
			    ...
			    // 当前商品实际支付价格
			    $price = $data['discount_unit_price']*$data['number'] ;
                $price=$real_total_price=get_real_total_price($price,$order_info,$scale_ecv_money,$scale_youhui_money,$scale_exchange_money);
				...
				if($balance_price>0 && $order_info['pay_status']==2)
				{
					require_once(APP_ROOT_PATH."system/model/supplier.php");
...
function refund_item_new($oid, $money, $memo = '')
{
	...
	$order_id = $data['order_id'];
	$order_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_order where id = '".$order_id."'");
	if($data && $data['refund_status'] == 1 && $order_info['order_status'] == 0 && ($order_info['pay_status'] == 2 or $order_info['pay_status'] == 1)) {
		// if ($data['is_coupon'] == 0) {
			...
			// 当前商品实际支付价格
			$real_total_price         = $data['discount_unit_price'] *$data['number'];
            $real_total_price=get_real_total_price($real_total_price,$order_info,$scale_ecv_money,$scale_youhui_money,$scale_exchange_money);
        if ($money > $real_total_price) {
				return 1;//false;
			}
            if($order_info['is_presell_order']==1&&$order_info['pay_status']==1){
                fanwe_require(APP_ROOT_PATH."system/model/cart.php");
                syn_presell_deal_stock($order_info['id']);
            }
 ...
 * @param $item_info
 */
function back_max_bought_by_refund_on_unpay($order_info=array(),$item_info=array()){
    if($order_info['is_presell_order']==1&&$order_info['pay_status']==1){
        if($item_info['attr']){
           $attr_key=str_replace(",","_",$item_info['attr']);
           $GLOBALS['db']->query("update ".DB_PREFIX."attr_stock set stock_cfg=stock_cfg+{$item_info['number']} where attr_key='{$attr_key}'");
        }
        $GLOBALS['db']->query("update ".DB_PREFIX."deal set max_bought=max_bought+{$item_info['number']} where id={$item_info['deal_id']}");
    }
}
/**
 * 退券
...
// 		}
		...
		// 当前商品实际支付价格
		$price = $data['discount_unit_price']*$data['number']  - $scale_ecv_money - $scale_youhui_money - $scale_exchange_money;
...
function get_real_total_price($real_total_price,$order_info,$scale_ecv_money,$scale_youhui_money,$scale_exchange_money){
    if($order_info['is_presell_order']==1){
        if($order_info['pay_status']==1){
            if($order_info['presell_type']){
                $real_total_price=0;
            }else{
                $real_total_price=$order_info['pay_amount'];
            }
        }else if($order_info['pay_status']==2){
            $rest_count=$GLOBALS['db']->getOne("SELECT count(*) - count( DealOrderItem.refund_status = 2 OR NULL ) rest_order FROM ".DB_PREFIX."deal_order_item DealOrderItem WHERE DealOrderItem.order_id ={$order_info['id']}");
            if($rest_count==1){
                $real_total_price+=$order_info['delivery_fee'];
            }
            if($order_info['presell_type']){
                //定金不退
                $real_total_price=$real_total_price-$order_info['presell_discount_money'];
            }else{
                //订金退
                $real_total_price=$real_total_price-($order_info['presell_discount_money']-$order_info['presell_deposit_money']);
            }
        }
    }else{
        $rest_count=$GLOBALS['db']->getOne("SELECT count(*) - count( DealOrderItem.refund_status = 2 OR NULL ) rest_order FROM ".DB_PREFIX."deal_order_item DealOrderItem WHERE DealOrderItem.order_id ={$order_info['id']}");
        ...
        $real_total_price=$real_total_price - $scale_ecv_money - $scale_youhui_money - $scale_exchange_money;
    }
    return  $real_total_price;
}
function get_pay_amount_in_deal_order($order_info){
    if($order_info['is_presell_order']==1){
        if($order_info['pay_status']==2){
            $order_info['pay_amount']=$order_info['pay_amount']-($order_info['presell_discount_money']-$order_info['presell_deposit_money']);
        }
    }else{
        if($order_info['supplier_id']==0){
            $order_info['pay_amount']=$order_info['pay_amount']-$order_info['cod_money'];
        }
...
function do_delivery($param)
{
    ...
    else
    {
        ...
        //app 推送
        AppPush::doDelivery($delivery_sn,$express_name,$order_info);
...
function order_fee_arr($order_info,$show_type=0)
{
	...
	if($show_type){
	    $order_total_price = $order_info['total_price']+$order_info['exchange_money']+$order_info['discount_price'];
	}else{
	    $order_total_price = $order_info['total_price']+$order_info['exchange_money'];
	}
	...
	if($order_info['is_presell_order']==1){
	    $presell_discount_price = $order_info['presell_discount_money']- $order_info['presell_deposit_money'];
	    if($order_info['pay_status']==0){
	        $order_pay_price = $order_info['presell_deposit_money'];
	    }else{
	        $order_pay_price-=$order_info['presell_discount_money'];
	    }
	}
	if($order_info['pay_status']==2){
	    $order_pay_price = $order_info['pay_amount'] - $order_info['ecv_money']-$order_info['youhui_money'] - ($order_info['presell_discount_money']- $order_info['presell_deposit_money']);//$order_total_price - $youhui_price;	    	  
	}
	$feeinfo=array();
	...
	if($order_info['is_presell_order']==1){
	    if($presell_discount_price > 0){
	        $presell_title = "订金：¥".format_price_txt(round($order_info['presell_deposit_money'], 2))."( 抵¥".format_price_txt(round($order_info['presell_discount_money'], 2))." )";	         
	    }else{
	        $presell_title = "订金：¥".format_price_txt(round($order_info['presell_deposit_money'], 2));	         
	    }
	    ...
	    $feeinfo[] = array(
	        "hiz_name" => $presell_title,
	        "pc_name" => $presell_title,
	        "name" => $presell_title,
	        "symbol" =>2,
	        "value" => $pay_status,
	        "buy_type" => 1,
	    ...
	    $presell_title="商品尾款：¥".format_price_txt(round($order_info['presell_left_price'], 2));
	    $color_left=0;
	    if($color_right==1){
	        $color_left=1;
	    }
	    ...
	    if($order_info['pay_status']==2){
	        ...
	        if($order_info['presell_end_time']>NOW_TIME){//未开始
	            $pay_status="未开始";
	            $color_right=0;
	        }else{
	            $pay_status="待付款";
	...
	if($order_info['is_presell_order']==1 && $presell_discount_price>0){	    
	    if($order_info['pay_status']>0){
	        ...
	        $paid[] = array(
	            ...
	            "name" => "订金抵扣优惠",
	            "symbol" => -1,
	            "value" => format_price_txt(round($presell_discount_price, 2)),
	            "buy_type" => 0
	        );
	        $youhui_price+=$order_info['youhui_money'];
	    }
