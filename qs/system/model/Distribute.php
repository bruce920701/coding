...
class Distribute
{
    ...
    public static function distribution_order_fee($type=0,$distribution_fee=0,$order_info=array()){
        $is_insert=0;
        if($type==2){
            $data=array();
            $data['distribution_fee']=$distribution_fee;
            $data['is_settlement']=1;
            $GLOBALS['db']->autoExecute(DB_PREFIX."distribution_order_fee",$data,'UPDATE',' order_id= '.$order_info['id'],'SILENT');
        ...
        if($type==1||$is_insert==1){
            //驿站服务费结算表记录
            $data=array();
            $data['order_id']=$order_info['id'];
            $data['order_sn']=$order_info['order_sn'];
            $data['distribution_id']=$order_info['distribution_id'];//$dist_id;
            $data['create_time']=NOW_TIME;
            $data['total_price']=$order_info['total_price'];
            $data['distribution_fee']=$distribution_fee;
            if($order_info['payment_id']!=0){
                $data['payment_class_name']=$GLOBALS['db']->getRow('select * from '.DB_PREFIX.'payment where id='.$order_info['payment_id']);
            }elseif($order_info['account_money']>0){
                $data['payment_class_name']='Account';
                //$data['payment_id']=$GLOBALS['db']->getRow('select * from ');
            }elseif($order_info['ecv_money']>0){
                $data['payment_class_name']='Voucher';
            }else{
                $data['payment_class_name']='';
            }
            if($is_insert==1){
                $data['is_settlement']=1;
            }else{
            ...
            $GLOBALS['db']->autoExecute(DB_PREFIX."distribution_order_fee",$data,'INSERT','','SILENT');
        }
    }
}