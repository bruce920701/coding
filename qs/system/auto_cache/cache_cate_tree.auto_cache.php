...
class cache_cate_tree_auto_cache extends auto_cache{
	public function load($param)
	{
		...
		{
			//验证参数有效性
			if($param['type']<0||$param['type']>6)$param['type'] = 0;
			$key = $this->build_key(__CLASS__,$param);
			$GLOBALS['cache']->set_dir(APP_ROOT_PATH."public/runtime/data/".__CLASS__."/");
			$nav_list = $GLOBALS['cache']->get($key);
			...
			elseif($param['type']==1)
			{
				...
				foreach($navs as $k=>$v)
				{
					...
					$sub_nav = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."shop_cate where pid = ".$v['id']." and is_effect = 1 order by sort limit 3");
					foreach($sub_nav as $kk=>$vv)
					{
					...
					$pop_nav = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."shop_cate  where pid = ".$v['id']." and is_effect = 1 order by sort limit 50");
					foreach($pop_nav as $kk=>$vv)
					{
			...
			elseif($param['type']==6){
			    $navs = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."service_cate where is_delete = 0 and is_effect = 1 and pid=0 order by sort asc");
			    foreach($navs as $k=>$v)
			    {
			        $navs[$k]['url'] = url("index","visiting_service_list",array("cid"=>$v['id']));
			        $sub_nav = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."service_cate where pid = ".$v['id']." and is_effect = 1 and is_delete = 0 order by sort limit 3");
					foreach($sub_nav as $kk=>$vv)
					{
						$sub_nav[$kk]['url'] = url("index","visiting_service_list",array("cid"=>$vv['id']));
					}
					$navs[$k]['sub_nav'] = $sub_nav;
						
					$pop_nav = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."service_cate  where pid = ".$v['id']." and is_effect = 1 and is_delete = 0 order by sort limit 50");
					foreach($pop_nav as $kk=>$vv)
					{
						$pop_nav[$kk]['url'] = url("index","visiting_service_list",array("cid"=>$vv['id']));
					}
					$navs[$k]['pop_nav'] = $pop_nav;
