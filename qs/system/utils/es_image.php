...
//        @imagestring($im, 5, 5, 3, $randval, $stringColor);
    ...
    static public function water($source,$water,$savename=null,$alpha=80,$position="0")
    {
        ...
        switch (intval($position))
        {
    		...
    		case 6:
    		    ...
    		    $posX=$sInfo["width"]/2-$wInfo["width"]/2;
    		    //生成混合图像
    		    imagecopymerge($sImage, $wImage, $posX, $posY+50, 0, 0, $wInfo['width'],$wInfo['height'],$alpha);
    		    break;
    		//居中偏左
    		case 7:
    		    $posY=$sInfo["height"]/2-$wInfo["height"]/2;
    		    $posX=$sInfo["width"]/2-$wInfo["width"]/2;
    		    //生成混合图像
    		    imagecopymerge($sImage, $wImage, $posX-168, $posY-36, 0, 0, $wInfo['width'],$wInfo['height'],$alpha);
    		    break;
    		...
    		case 8:
    		    ...
    		    $posX=$sInfo["width"]/2-$wInfo["width"]/2;
    		    //生成混合图像
    		    imagecopymerge($sImage, $wImage, $posX+80, $posY-60, 0, 0, $wInfo['width'],$wInfo['height'],$alpha);
    		    break;
			//居中偏上
    		case 9:
    		    $posY=$sInfo["height"]/2-$wInfo["height"]/2;
    		    $posX=$sInfo["width"]/2-$wInfo["width"]/2;
    		    //生成混合图像
    		    imagecopymerge($sImage, $wImage, $posX, $posY-150, 0, 0, $wInfo['width'],$wInfo['height'],$alpha);
    		    break;
            //居中偏上
            case 10:
                $posY=$sInfo["height"]/2-$wInfo["height"]/2;
                $posX=$sInfo["width"]/2-$wInfo["width"]/2;
                //生成混合图像
                imagecopymerge($sImage, $wImage, $posX, 0, 0, 0, $wInfo['width'],$wInfo['height'],$alpha);
                break;
            //居中下
            case 11:
                $posY=$sInfo["height"]/2-$wInfo["height"]/2;
                $posX=$sInfo["width"]/2-$wInfo["width"]/2;
                //生成混合图像
                imagecopymerge($sImage, $wImage, $posX,$posY+230, 0, 0, $wInfo['width'],$wInfo['height'],$alpha);
                break;
            //二维码位置自定义
            case 12:
                $posY=$sInfo["height"]/2-$wInfo["height"]/2;
                $posX=$sInfo["width"]/2-$wInfo["width"]/2;
                //生成混合图像
                imagecopymerge($sImage, $wImage, $posX+15, $posY+145, 0, 0, $wInfo['width'],$wInfo['height'],$alpha);
                break;
            //头像位置
            case 13:
                $posY=$sInfo["height"]/2-$wInfo["height"]/2;
                $posX=$sInfo["width"]/2-$wInfo["width"]/2;
                //生成混合图像
                imagecopymerge($sImage, $wImage, $posX-238, $posY+34, 0, 0, $wInfo['width'],$wInfo['height'],$alpha);
                break;
        }
	...
	static function png_str($file,$filename='',$str,$fontface="simhei.ttf",$position=0,$position_arr=array())
    {
        $fontface=APP_ROOT_PATH.'public/font_bg/'.$fontface;
        $Info=self::getImageInfo($file);
        $im = imagecreatefromjpeg($file);
        $color = imagecolorallocatealpha($im,0,0,0,0);
        $text_size=imagettfbbox(15,0,$fontface,$str);
        $width=abs($text_size[2]-$text_size[0]);

        $height =abs($text_size[5]-$text_size[3]);
        if($position_arr){
            $posY=$position_arr['posY'];
            if($position_arr['posX']){
                $posX=$position_arr['posX'];
            }else{
                $posX= (imagesx($im)-$width)/2;
            }
        }else{
            switch (intval($position))
            {
                case 0:
                    $posY=$Info["height"]*2/5-$height/2-20;
                    $posX=$Info["width"]/2-$width/2-35;
                    break;
                //居中
                case 1:
                    $posY=$Info["height"]*2/5-$height/2+230;
                    $posX=$Info["width"]/2-$width/2-90;
                    break;
                //居中偏下
                case 2:
                    $posY=$sInfo["height"]/2-$wInfo["height"]/2;
                ...
                //居中偏上
                case 3:
                    $posY=$Info["height"]*2/5-$height/2-115;
                    $posX=$Info["width"]/2-$width/2-48;
                    break;
            }
        }

        imagettftext($im,15,0,$posX,$posY,$color,$fontface,$str);
        header("content-type:image/png");
	...
	static function background_png($filename='',$wh=array("width"=>300,"height"=>450),$rgb=array("255","255","255"))
    {
		$image = imagecreatetruecolor($wh['width'], $wh['height']);
	...
	//将图片切为圆形图
	static function circular_png($filename='') {  
	...
	if(strpos($mimetype, "jpg") !== false||strpos($mimetype, "jpeg") !== false){
